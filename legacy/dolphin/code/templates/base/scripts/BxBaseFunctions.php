...
class BxBaseFunctions
{
    ...
    */
    ...
    function getMemberThumbnail($iId, $sFloat = 'none', $bGenProfLink = false, $sForceSex = 'visitor', $isAutoCouple = true, $sType = 'medium', $aOnline = array(), $sTmplSfx = '')
    {
		...
		//$sMemberCountry =  $aProfile['Country'];
		$sMemberCountry  = _t($GLOBALS['aPreValues']['Country'][$aProfile['Country']]['LKey']);
		 $sMemberFlag = genFlag( $aProfile['Country']);
		  $sCity =  $aProfile['City'];
            ...
            'bx_if:profileLink' => array(
                ...
                'content' => array(
                    ...
                    'usr_profile_url' => $sLink,
					 ...
					 // freddy ajout country and flag
		   'MemberCountry' => $sMemberFlag. ' ' .$sCity.','.' '. $sMemberCountry,
		  
		   //'MemberFlag' => $sMemberFlag ,
		   ///////////////////////
                ),
            ),
        ));
