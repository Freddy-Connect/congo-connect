...
class BxDolJoinProcessor
{
    ...
    function showFinishPage( $iMemberId, $sStatus )
    {
		...
		*/
		 ...
		 //DEBUT --- Freddy  ajout code profile edit permet de seconnecter directment sur profile edit apres inscription
/*
bx_login ((int)$iMemberId);
       header("Location: " . BX_DOL_URL_ROOT . "pedit.php?ID=".$iMemberId);
        exit;
		*/

// FIN  ---Freddy  ajout code vers investment add (Denoncer corruption)


/// Ajout freddy pour recuperer l'adresse email du nouveau membre
...
// fin ajout pour recuperer l'adresse email du nouveau membre

       
	   // return '<div class="dbContentHtml bx-def-font-large">' . _t( '_Join complete' ) . '<br />' . _t( $sStatusText ) . '</div>';
	   
	   // Ceci s'aplique unique unique si 
	   return '<div style=" opacity:0;
  transform:translateY(18px);
  animation:cardUp 5s ease-out forwards; background: #FFF;border-radius: 14px; max-width: 640px; padding: 14px; border: 1px solid rgba(0,0,0,0.04);
  box-shadow:
    0 20px 60px rgba(0, 0, 0, 0.25),
    0 8px 20px rgba(0, 0, 0, 0.15);transition: transform .2s ease,
              box-shadow .2s ease;" class="dbContentHtml bx-def-font-large">' 
	   
	  .'<div style=" font-size: 17px; margin-bottom: 10px; color: #1f3c88;">'.'<i class="sys-icon trophy"></i>'.' ' . _t( '_Join complete' ).' '.'<b>'.$aMonsieurMadame.' '.$aRecipient['FirstName'].' '.$aRecipient['LastName'].'</b>'.' ! '.'<br />'. _t( '_Join_Votre_Compte_a_ete_cree_sur' ).' '.'<b>'.$aSiteName.'</b>'.'.'.'</div>'
	   
	
	 .'<div style="  background: #f0f4ff; padding: 12px; border-radius: 6px; font-weight: bold; margin: 0px 0;">'
	  .' <i class="sys-icon envelope"> </i>'.' '.$aRecipient['Email'] 
	  .'<p style="font-size: 14px; line-height: 1.6; margin: 10px 0;font-weight:normal;">'. _t( '_Email_fourni' ). '</p>'
	  .'</div>' 
