...
class BxDolProfilesController
{
	...
	/* Freddy 07/01/2026 modification avec chatgpt pour se connecter sur profile edit apres avoir cliquer sur le lien de verification de mail
	sendConfMail() — VERSION CORRIGÉE

👉 Remplace uniquement le bloc de génération du ConfCode
*/
/* commentaire de la version original
	...
	*/
	...
	/*chatgpt 7 janvier 2026 Version modifiée de sendConfMail($iMemID) à copier-coller, pour l’aligner avec profile_activate.php sur secret_ph, onsecret_ph ici (donc pas de changement de SALT nécessaire).
on profite juste pour sécuriser un peu et rendre le code plus clair.
Pour que ça marche, il faut que dans profile_activate.php qu'on aies bien :crypt($p_arr['Email'], CRYPT_EXT_DES ? "secret_ph" : "se")
*/
function sendConfMail($iMemID)
{
    global $site;
    ...
    // ✅ ConfCode basé sur l'email (SALT = 'secret_ph' DOIT matcher profile_activate.php)
    $sConfCode = base64_encode(
        base64_encode(
            crypt($sEmail, CRYPT_EXT_DES ? 'secret_ph' : 'se')
        )
    );
    ...
    // ✅ Variables disponibles dans le template email (t_Confirmation)
    $aPlus = array(
        'ConfCode' => $sConfCode,
