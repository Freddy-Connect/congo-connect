    ...
    function actionHome()
    {
        ...
        if ($oForm->isSubmittedAndValid())
        {
            if(!(bx_get('ConfID') && bx_get(ConfCode)))
            {
              ...
              // freddy modif   $_page['header'] = $action_result = _t( "_db_pwd_maybe_recognized" );
			  $_page['header'] = $action_result = _t( "_Recognized" );
                $_page['header_text'] = _t( "_RECOGNIZED", $site['title'] );
                // freddy modifecho _t( "_db_pwd_maybe_sent", $site['title'] );
				echo _t( "_MEMBER_RECOGNIZED_MAIL_SENT", $site['title'] );
        ...
        { // Denre new added code
            ...
            if ($member['Password'] <> '' && strcmp($ConfCode, $ConfCodeReal) == 0)
            {
                ...
                $member['Password'] = $this->updateUserNewPwd($member['ID'], $ConfCode);
                ...
                if($p_arr = bx_login($member['ID'], $rememberMe))
                {
					...
					// freddy ajout pour rediriger ver la page d'accueil
					else if(getParam('forgot_redirect') == 'accueil')
                        $forgot_redirect = 'index.php';
    ...
    function genInputs()
    {
        if(!getParam('enable_password_generation') == 'on')
        {
            $inputs[] = array(
                'type' => 'password',
                'name' => 'Password',
                'caption' => _t('Votre_Nouveau_Password'),
        ...
        $inputs[] = array(
            'type' => 'email',
            'name' => 'Email',
            'caption' => _t('Votre_adresse_Email'),
        ...
        if(getParam('forgot_enable_captcha') == 'on')
        {
            $inputs[] = array(
                'type' => 'captcha',
                'name' => 'captcha',
