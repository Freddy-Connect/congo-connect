...
function init() {
    document.getElementById('eventcover_main').addEventListener("touchstart", touchHandler, true);
    document.getElementById('eventcover_main').addEventListener("touchmove", touchHandler, true);
    document.getElementById('eventcover_main').addEventListener("touchend", touchHandler, true);
    document.getElementById('eventcover_main').addEventListener("touchcancel", touchHandler, true);
}
...
function moveimagey(owneris,hashis,xpos,ypos,EventID)
{
 init();
 stopmn=1;
 closemenu();
 ...
 var divheight=$('#eventcover_main').height();	
 var divwidth=$('#eventcover_main').width();
 //get image height
 var imageis=$('#eventcover_main').css('background-image').replace(/url\(|\)$/ig, "");
 ...
 function finish (e)
 {
   ...
   $bg.unbind('mousedown mouseup mouseleave', handle);
	 ...
	 jQuery.ajax({
        type: "POST",
        url: "modules/ibdw/eventcover/updateposition.php",
        data: 'owner='+owner+'&currenthash='+currenthash+'&PositionY='+start.y+'&PositionX=0&boxwidth='+w+'&EventID='+EventID,
        cache: false
   });
	 $("#posX").val(0);
	 $("#posY").val(start.y);
...
function ibdw_cover_remove(owner,currenthash,baseurlis,id)
{
 jQuery.ajax({
        type: "POST",
        url: "modules/ibdw/eventcover/removeimage.php",
        data: 'owner='+owner+'&currenthash='+currenthash+'&id='+id,
        cache: false
       });
 $('#eventcover_main').css('background','url("'+baseurlis+'modules/ibdw/eventcover/templates/base/images/default.jpg") no-repeat scroll transparent');
 $('#removemenu').css('display','none');
 $('#movemenu').css('display','none');
}

function update_eventcore_main(EventID){
  $.ajax({
      type: "POST",
...
function change_album(hashe,id)
{
 $.ajax({
      ...
      url: "modules/ibdw/eventcover/change_album.php",
      success: function(data) {
        update_eventcore_main(id);
      }
 });
}
...
function ibdw_cover_frompc(id_album_predef,user,EventID)
 {
  $("#loading_div").css("display","block");
  $.ajax({
      type: "POST",
      data: "id_album_predef=" + id_album_predef + "&user=" + user + "&EventID="+ EventID,
      url: "modules/ibdw/eventcover/fragment.php",
      success: function(data) 
      {
