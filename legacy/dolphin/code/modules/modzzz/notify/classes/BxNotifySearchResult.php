...
class BxNotifySearchResult extends BxDolTwigSearchResult {

    var $aCurrent = array(
        'name' => 'modzzz_notify',
        'title' => '_modzzz_notify_page_title_browse',
        'table' => 'modzzz_notify_main',
        'ownFields' => array('id', 'author_id', 'title', 'uri', 'created', 'thumb', 'rate', 'desc', 'categories', 'tags', 'comments_count'),
        ...
        'restriction' => array(
            ...
            'public' => array('value' => '', 'field' => 'allow_view_notify_to', 'operator' => '='),
        ),
    ...
    function BxNotifySearchResult($sMode = '', $sValue = '', $sValue2 = '', $sValue3 = '') {   

        switch ($sMode) {

            case 'pending':
                if (isset($_REQUEST['modzzz_notify_filter']))
                    $this->aCurrent['restriction']['keyword'] = array('value' => process_db_input($_REQUEST['modzzz_notify_filter'], BX_TAGS_STRIP), 'field' => '','operator' => 'against');
                $this->aCurrent['restriction']['activeStatus']['value'] = 'pending';
                $this->sBrowseUrl = "administration";
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_pending_approval');
            ...
            case 'search':
                ...
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_search_results') . ' ' . (is_array($sValue2) ? implode(', ',$sValue2) : $sValue2) . ' ' . $sValue;
                unset($this->aCurrent['rss']);
                break;

            case 'user':
                $iProfileId = $GLOBALS['oBxNotifyModule']->_oDb->getProfileIdByNickName ($sValue);
                ...
                $this->aCurrent['title'] = ucfirst(strtolower($sValue)) . _t('_modzzz_notify_page_title_browse_by_author');
            ...
            case 'admin':
                ...
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_admin_notify');
                break;
            ...
            case 'category':
                ...
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_browse_by_category') . ' ' . $sValue;
                break;
            ...
            case 'tag':
                ...
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_browse_by_tag') . ' ' . $sValue;
                break;

            case 'recent':
                $this->sBrowseUrl = 'browse/recent';
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_browse_recent');
                break;

            case 'top':
                $this->sBrowseUrl = 'browse/top';
                $this->aCurrent['sorting'] = 'top';
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_browse_top_rated');
                break;

            case 'popular':
                $this->sBrowseUrl = 'browse/popular';
                $this->aCurrent['sorting'] = 'popular';
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_browse_popular');
                break;                

            case 'featured':
            $this->aCurrent['restriction']['featured'] = array('value' => 1, 'field' => 'featured', 'operator' => '=');
                $this->sBrowseUrl = 'browse/featured';
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_browse_featured');
                break;                                
            ...
            case 'calendar':
                ...
                $this->aCurrent['title'] = _t('_modzzz_notify_page_title_browse_by_day') . sprintf("%04u-%02u-%02u", $sValue, $sValue2, $sValue3);
                break;                                

            case '':
                $this->sBrowseUrl = 'browse/';
                $this->aCurrent['title'] = _t('_modzzz_notify');
                break;
        ...
        $this->aCurrent['paginate']['perPage'] = $oMain->_oDb->getParam('modzzz_notify_perpage_browse');
        ...
        if (isset($_REQUEST['rss']) && $_REQUEST['rss']) {
            ...
            $this->aCurrent['paginate']['perPage'] = $oMain->_oDb->getParam('modzzz_notify_max_rss_num');
        }

        bx_import('Voting', $oMain->_aModule);
        $oVotingView = new BxNotifyVoting ('modzzz_notify', 0);
    ...
    function getAlterOrder() {
		if ($this->aCurrent['sorting'] == 'last') {
			$aSql = array();
			$aSql['order'] = " ORDER BY `modzzz_notify_main`.`created` DESC";
			return $aSql;
		} elseif ($this->aCurrent['sorting'] == 'top') {
			$aSql = array();
			$aSql['order'] = " ORDER BY `modzzz_notify_main`.`rate` DESC, `modzzz_notify_main`.`rate_count` DESC";
			return $aSql;
		} elseif ($this->aCurrent['sorting'] == 'popular') {
			$aSql = array();
			$aSql['order'] = " ORDER BY `modzzz_notify_main`.`views` DESC";
			return $aSql;
		}
	    return array();
    }
    ...
    function displayResultBlock () {
        ...
        if ($s) {
            ...
            return $oFunctions->centerContent ($s, '.modzzz_notify_unit');
        }
        return '';
    }
