...
class BxNotifyPageMy extends BxDolTwigPageMain {

    function BxNotifyPageMy(&$oMain) {

		$this->_oDb = $oMain->_oDb;
        $this->_oConfig = $oMain->_oConfig;
        $this->oMain = $oMain;
 
		parent::BxDolTwigPageMain('modzzz_notify_my', $oMain); 
	}
   ...
   function getBlockCode_Settings() {
    ...
    function getBlockCode_Desc() {
 		...
 		$sHeadSettingC = _t("_modzzz_notify_caption_notify_me"); 
		$sHeadActionsC = _t("_modzzz_notify_actions"); 
  		$sYesC = _t("_modzzz_notify_yes");  
		$sNoC = _t("_modzzz_notify_no");

		if(isset($_REQUEST['submit_form'])) { 
			$this->oDb->saveMemberSettings($this->oMain->_iProfileId);
		}

 		$arrActions = $this->oDb->getNotifyActions(true, 'member');
		...
		foreach($arrActions as $aEachAction) {
			...
			$sNoChecked = "";
 			switch($sAccess){
				case "yes":
					$sYesChecked = "checked='checked'";
				break;
				case "no":
					$sNoChecked = "checked='checked'";
				break; 
			}
			...
			if($sOldGroup != $sNewGroup) {
				...
				$aForm['inputs']["header{$iter}"] = array(
				 ...
				 $aForm['inputs']["ItemHead{$iter}"] = array(
					'type' => 'custom',
					'name' => "ItemHead{$iter}",
					'content' =>  "<div style='width:100%'><div style='float:left;width:50%'><b>{$sHeadActionsC}</b></div><div style='float:left;width:45%'><b>{$sHeadSettingC}</b></div></div><div class='clear_both'></div>",  
					'colspan' => true
				);
			...
			$aForm['inputs']["Item{$iter}"] = array(
				...
				'content' =>  "<div style='width:100%'>
								   ...
								   <div style='float:left;width:45%'>
										<input type=radio name=notify[$iId] value='yes' {$sYesChecked}> {$sYesC} &nbsp; 
										<input type=radio name=notify[$iId] value='no' {$sNoChecked}> {$sNoC} &nbsp;
								   </div>
							  </div>
