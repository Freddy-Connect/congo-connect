...
/*
 * Professional module Data
 */
class BxProfessionalDb extends BxDolTwigModuleDb {	
	 ...
	 * Constructor.
	 */
	function BxProfessionalDb(&$oConfig) {
        parent::BxDolTwigModuleDb($oConfig);

		$this->_oConfig = $oConfig;
 
		/********Schedule ************/
        $this->_sTableSchedule = 'schedule'; 
		/********[END] Schedule ************/

		/********Review ************/
        $this->_sTableReview = 'review_main';
        $this->_sReviewPrefix = 'modzzz_professional_review';
        $this->_sTableReviewMediaPrefix = 'review_';
		/********[END] Review ************/

		/********Client ************/
        $this->_sTableClient = 'client_main';
        $this->_sClientPrefix = 'modzzz_professional_client';
        $this->_sTableClientMediaPrefix = 'client_';
		/********[END] Client ************/

		/********Booking ************/
        $this->_sTableBooking = 'booking_main';
        $this->_sBookingPrefix = 'modzzz_professional_booking';
        $this->_sTableBookingMediaPrefix = 'booking_';
		/********[END] Booking ************/
		
		/********Request ************/
        $this->_sTableRequest = 'request_main';
		/********[END] Request ************/

		/********Service ************/
        $this->_sTableService = 'service_main';
        $this->_sServicePrefix = 'modzzz_professional_service';
        ...
        $this->_sFieldProfessionalId = 'professional_id';
        $this->_sFieldCreated = 'created'; 
        $this->_sTableAdmins = 'admins';
        $this->_sFieldAllowViewTo = 'allow_view_professional_to';  
	}
	...
	function setInvoiceStatus($iItemId, $sStatus) {
 		 ...
 		 $this->query("UPDATE `" . $this->_sPrefix . "invoices` SET `invoice_status`='$sStatus' WHERE `professional_id`='$iItemId'"); 
	}
	...
	function saveClaimRequest($iEntryId, $iProfileId, $sClaimText) {
		...
		$aAllEntries = $this->query("INSERT INTO `" . $this->_sPrefix . "claim` SET `professional_id`='$iEntryId', `member_id`='$iProfileId', `message`='$sClaimText', `processed`=0, `claim_date`=$iTime");  

	}

 	function getClaimById($iEntryId) { 
		$aClaim = $this->getRow("SELECT `id`,`professional_id`,`member_id`,`message`,`processed`,`claim_date`,`assign_date` FROM `" . $this->_sPrefix . "claim` WHERE `id`='$iEntryId'");   
	...
	function assignClaim($iClaimId, $isAdmin=false) { 
		...
		$iClaimantId = (int)$aClaim['member_id'];
 		$iEntryId = (int)$aClaim['professional_id'];
		...
		$this->alertOnAction('modzzz_professional_claim_assign', $iEntryId, $iClaimantId );  
	}
    ...
    function deleteEntryByIdAndOwner ($iId, $iOwner, $isAdmin) {
        if ($iRet = parent::deleteEntryByIdAndOwner ($iId, $iOwner, $isAdmin)) {
			...
			$this->query("DELETE FROM `" . $this->_sPrefix . "claim` WHERE `professional_id`='$iId'");  
            $this->query ("DELETE FROM `" . $this->_sPrefix . "invoices` WHERE `professional_id` = $iId");
    ...
    function saveTransactionRecord($iBuyerId, $iProfessionalId, $sTransNo, $sTransType) {
        $iBuyerId    = (int)$iBuyerId;
        $iProfessionalId  = (int)$iProfessionalId; 
   
		$aDataEntry = $this->getEntryById($iProfessionalId);
		...
		$iOrderId = $this->lastId();
		 ...
		 //if order successful, update expiration date of professional
		 if($iOrderId){
			$aInvoice = $this->getInvoiceByNo($sInvoiceNo);
			$iDays = (int)$aInvoice['days'];
			$this->updateEntryExpiration($iProfessionalId, $iDays);

			$iPackageId = (int)$aInvoice['package_id'];
			$aPackage = $this->getPackageById($iPackageId); 
			$iFeatured = (int)$aPackage['featured'];
			if($iFeatured){
				$this->updateFeaturedStatus($iProfessionalId);
			}
		 }
	...
	function createInvoice($iEntryId, $iPackageId, $fPrice, $iDays, $sStatus='pending'){
		 ...
		 $iInvoiceActiveDays = (int)getParam('modzzz_professional_invoice_valid_days');
    ...
    function getInvoiceByNo($sInvoiceNo) {  
        return $this->getRow("SELECT `invoice_no`, `price`, `days`, `professional_id`, `package_id`, `invoice_status`, `invoice_due_date`, `invoice_expiry_date`, `invoice_date` FROM `" . $this->_sPrefix . "invoices` WHERE `invoice_no` = '{$sInvoiceNo}'"); 
    }
   ...
   /** category functions*/
	...
	function getParentCategoryInfo ($iId){
		$iParentId = (int)$this->getOne("SELECT `parent` FROM `" . $this->_sPrefix . "categ` WHERE `id` = '$iId' ORDER BY `name` ASC");
	...
	function getLocalCategoryCount($iParent, $sCountry='', $sState=''){
		...
		if (!$GLOBALS['logged']['admin']){ 
			if ($GLOBALS['logged']['member']){ 
			    ...
			    $sExtraCheck = " AND professional.`membership_view_filter` IN ('', '$iMembershipId')"; 
			}else{
				$sExtraCheck = "AND professional.`membership_view_filter`=''";
			}
		}
 
		if($sCountry)
			$sExtraCheck .= " AND professional.`country`='$sCountry'";

		if($sState)
			$sExtraCheck .= " AND professional.`state`='$sState'";

		return  $this->getOne("SELECT count(professional.`id`) FROM `" . $this->_sPrefix . "categ` cat LEFT JOIN `" . $this->_sPrefix . "main` professional ON cat.id=professional.`category_id` WHERE cat.`active` = 1 AND cat.`parent`=$iParent AND professional.`status`='approved' {$sExtraCheck}"); 
	}
	...
	function getCategoryCount($iCategoryId){
		...
		return  $this->getOne("SELECT count(professional.`id`) FROM `" . $this->_sPrefix . "categ` cat LEFT JOIN `" . $this->_sPrefix . "main` professional ON cat.id=professional.`category_id` WHERE cat.`active` = 1 AND cat.`id`=$iCategoryId AND professional.`status`='approved' {$sExtraCheck}"); 
	}
	...
	/*featured functions*/ 
    function saveFeaturedTransactionRecord($iBuyerId, $iProfessionalId, $iQuantity, $fPrice, $sTransId, $sTransType) {
        $iBuyerId    = (int)$iBuyerId;
        $iProfessionalId  = (int)$iProfessionalId; 
		...
		if($iOrderId){
			$this->alertOnAction('modzzz_professional_featured_admin_notify', $iProfessionalId, $iBuyerId, $iQuantity, true);

			$this->alertOnAction('modzzz_professional_featured_buyer_notify', $iProfessionalId, $iBuyerId, $iQuantity);
		}
	...
	function processProfessionals(){ 
		$this->processExpiredProfessionals();
			
		$this->processExpiredInvoices();
 
		$this->processFeaturedProfessionals();
	}
	...
	function processExpiredInvoices() { 
		...
		$aProfessionals = $this->getAll("SELECT `professional_id` FROM `" . $this->_sPrefix . "invoices` WHERE `invoice_status`='pending' AND `invoice_expiry_date`>0 AND `invoice_expiry_date`<={$iExpireTime}");
		 
		foreach ($aProfessionals as $aEachProfessional) {
			$iEntryId = (int)$aEachProfessional['professional_id']; 
 
			$this->query("DELETE FROM `" . $this->_sPrefix . "main` WHERE `id`=$iEntryId");  
			
			$this->query("DELETE FROM `" . $this->_sPrefix . "invoices` WHERE `professional_id`=$iEntryId");  
 		}  
 
	}


	function processExpiredProfessionals() { 
 
 		$iTime = time();
 		$SECONDS_IN_DAY = 86400; 
		
        $bFreeProfessional = (getParam('modzzz_professional_paid_active')=='on') ? false : true;  
        $iNumActiveDays = (int)getParam("modzzz_professional_free_expired");
        if($bFreeProfessional && (!$iNumActiveDays))
            return; 

		if(getParam('modzzz_professional_activate_expired') == 'on') {
  			 
			$aProfessionals = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `expiry_date`>0 AND `expiry_date`<={$iTime}");
			 
			foreach ($aProfessionals as $aEachProfessional) {
				$iEntryId = (int)$aEachProfessional['id']; 
				$iRecipientId = (int)$aEachProfessional['author_id']; 

				$this->query("UPDATE `" . $this->_sPrefix . "main` SET `status`='expired'  WHERE `id`=$iEntryId");  
 
		 		$this->alertOnAction('modzzz_professional_expired', $iEntryId, $iRecipientId);
			}  
		}else{ 
  			$this->query("UPDATE `" . $this->_sPrefix . "main` SET `status`='expired' WHERE  `status`='approved' AND `expiry_date`>0 AND `expiry_date`<=$iTime");  
		}
  
		$iNumNotifyDays = (int)getParam("modzzz_professional_email_expiring");
		if($iNumNotifyDays){
			$iNotifyTime = $SECONDS_IN_DAY * $iNumNotifyDays;
			$iTriggerTime = $iTime - $iNotifyTime;

			$aProfessionals = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `expiry_date` >= {$iTriggerTime} AND `expiry_date` < {$iTime}  AND `pre_expire_notify`=0");
			 
			foreach ($aProfessionals as $aEachProfessional) {
				$iEntryId = $aEachProfessional['id'];
				$iRecipientId = $aEachProfessional['author_id'];
		 
				$this->query("UPDATE `" . $this->_sPrefix . "main` SET `pre_expire_notify`=1 WHERE `id` = $iEntryId");

				if(getParam("modzzz_professional_activate_expiring")=='on') { 
					$this->alertOnAction('modzzz_professional_expiring', $iEntryId, $iRecipientId, $iNumNotifyDays);
				}
			}
		}
 			
		$iNumNotifyDays = (int)getParam("modzzz_professional_email_expired");
		if($iNumNotifyDays){
			$iNotifyTime = $SECONDS_IN_DAY * $iNumNotifyDays;
			$iTriggerTime = $iTime - $iNotifyTime;

			$aProfessionals = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='expired' AND `expiry_date` <= {$iNotifyTime} AND `post_expire_notify`=0");
			...
			foreach ($aProfessionals as $aEachProfessional) {
				...
				if(getParam("modzzz_professional_activate_expired")=='on') { 
					$this->alertOnAction('modzzz_professional_post_expired', $iEntryId, $iRecipientId, $iNumNotifyDays);
				}
			}
		}
	...
	function processFeaturedProfessionals(){
        ...
        $aProfessionals = $this->getAll("SELECT `id`, `author_id`, `featured`, `featured_expiry_date` FROM `" . $this->_sPrefix . "main` WHERE `featured`=1 AND `featured_expiry_date`>0 AND `featured_expiry_date` <= $iTime"); 

		foreach($aProfessionals as $aEachList){
  
			$iProfessionalId = (int)$aEachList['id'];
			$iRecipientId = (int)$aEachList['author_id'];

			$this->alertOnAction('modzzz_professional_featured_expire_notify', $iProfessionalId, $iRecipientId  );
		
	        $this->query("UPDATE `" . $this->_sPrefix . "main` SET `featured`=0, `featured_expiry_date`=0, `featured_date`=0  WHERE `id`=$iProfessionalId"); 
		}

	}

	function alertOnAction($sTemplate, $iProfessionalId, $iRecipientId=0, $iDays=0, $bAdmin=false) {
	...
	function getLatestComments($iLimit=5){
		...
		return $this->getAll("SELECT `cmt_object_id`,`cmt_author_id`,`cmt_text`, UNIX_TIMESTAMP(`cmt_time`) AS `date` FROM `" . $this->_sPrefix . "cmts` cmt INNER JOIN `" . $this->_sPrefix . "main` m  ON cmt.`cmt_object_id`= m.`id` WHERE m.`allow_view_professional_to` = '". BX_DOL_PG_ALL ."' AND `cmt_text` NOT LIKE '<object%' ORDER BY `cmt_time` DESC LIMIT $iLimit"); 
	}  
    ...
    function deleteReviewByIdAndOwner ($iId, $iProfessionalId, $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `{$this->_sFieldAuthorId}` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableReview . "` WHERE `{$this->_sFieldId}` = $iId AND `professional_id`=$iProfessionalId $sWhere LIMIT 1")))
            return false;
	...
	function getReviews($sType='', $sqlLimit='', $iEntryId=0) {
		$iEntryId = (int)$iEntryId; 
		if($iEntryId){ 
			$sIdQuery = ' AND `professional_id`='.$iEntryId;
		}
    ...
    function confirmBooking($iId) {
         return $this->query ("UPDATE `" . $this->_sPrefix . $this->_sTableBooking . "` SET `confirmed`=1 WHERE `{$this->_sFieldId}` = $iId");
    }
 
    function getBookingEntryById($iId) { 
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableBooking . "` WHERE `{$this->_sFieldId}` = $iId LIMIT 1");
    }
 
    function getBookingEntryByUri($sUri) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableBooking . "` WHERE `{$this->_sFieldUri}` = '$sUri' LIMIT 1");
    }
 
    function deleteBookingByIdAndOwner ($iId, $iOwner, $isAdmin) {
        $sWhere = '';
        //if (!$isAdmin) 
            //$sWhere = " AND `{$this->_sFieldAuthorId}` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableBooking . "` WHERE `{$this->_sFieldId}` = $iId $sWhere LIMIT 1")))
            return false;
  
        return true;
    } 
	
	function getBookings($sType='', $sqlLimit='', $iEntryId=0) {
		$iEntryId = (int)$iEntryId; 
		if($iEntryId){ 
			$sIdQuery = ' AND `professional_id`='.$iEntryId;
		}
		...
		$aAllEntries = $this->getAll("SELECT * FROM `" . $this->_sPrefix . $this->_sTableBooking . "` WHERE 1 {$sTypeQuery} {$sIdQuery} ORDER BY `created` DESC {$sqlLimit}");

		return $aAllEntries;   
	}

 	function getBookingCount($sType='', $iEntryId=0) {
		$iEntryId = (int)$iEntryId;
		if($iEntryId){ 
			$sIdQuery = ' AND `professional_id`='.$iEntryId;
		}
		...
		$bCount = $this->getOne("SELECT COUNT(`id`) FROM `" . $this->_sPrefix . $this->_sTableBooking . "` WHERE 1  {$sTypeQuery} {$sIdQuery}");
	...
	/***** [begin] Clients **************************************/
    function getClientEntryById($iId) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableClient . "` WHERE `{$this->_sFieldId}` = $iId LIMIT 1");
    }
 
    function getClientEntryByUri($sUri) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableClient . "` WHERE `{$this->_sFieldUri}` = '$sUri' LIMIT 1");
    }
 
    function deleteClientByIdAndOwner ($iId, $iProfessionalId, $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `{$this->_sFieldAuthorId}` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableClient . "` WHERE `{$this->_sFieldId}` = $iId AND `professional_id`=$iProfessionalId $sWhere LIMIT 1")))
            return false;
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableClientMediaPrefix . "files` WHERE `entry_id` = $iId");
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableClientMediaPrefix . "images` WHERE `entry_id` = $iId");
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableClientMediaPrefix . "videos` WHERE `entry_id` = $iId");
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableClientMediaPrefix . "sounds` WHERE `entry_id` = $iId");
 
        return true;
    } 
	
	function getClients($sType='', $sqlLimit='', $iEntryId=0) {
		$iEntryId = (int)$iEntryId; 
		if($iEntryId){ 
			$sIdQuery = ' AND `professional_id`='.$iEntryId;
		}
		...
		$aAllEntries = $this->getAll("SELECT * FROM `" . $this->_sPrefix . $this->_sTableClient . "` WHERE 1 {$sTypeQuery} {$sIdQuery} ORDER BY `created` DESC {$sqlLimit}");

		return $aAllEntries;   
	}

 	function getClientCount($sType='', $iEntryId=0) {
		$iEntryId = (int)$iEntryId;
		if($iEntryId){ 
			$sIdQuery = ' AND `professional_id`='.$iEntryId;
		}
		...
		$bCount = $this->getOne("SELECT COUNT(`id`) FROM `" . $this->_sPrefix . $this->_sTableClient . "` WHERE 1  {$sTypeQuery} {$sIdQuery}");
	...
	/***** [begin] Services **************************************/
    function getRequestEntryById($iId) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableRequest . "` WHERE `{$this->_sFieldId}` = $iId LIMIT 1");
    }
 
    function getRequestEntryByUri($sUri) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableRequest . "` WHERE `{$this->_sFieldUri}` = '$sUri' LIMIT 1");
    } 
	/***** [END] Services **************************************/
    ...
    function getFormServiceEntries($iId) {
         return $this->getPairs ("SELECT `id`, `title` FROM `" . $this->_sPrefix . $this->_sTableService . "` WHERE `{$this->_sFieldProfessionalId}` = $iId and `{$this->_sFieldStatus}`='approved'", 'id','title');
    }
    ...
    function deleteServiceByIdAndOwner ($iId, $iProfessionalId, $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `{$this->_sFieldAuthorId}` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableService . "` WHERE `{$this->_sFieldId}` = $iId AND `professional_id`=$iProfessionalId $sWhere LIMIT 1")))
            return false;
	...
	function getServices($sType='', $sqlLimit='', $iEntryId=0) {
		$iEntryId = (int)$iEntryId; 
		if($iEntryId){ 
			$sIdQuery = ' AND `professional_id`='.$iEntryId;
		}
		...
		$aAllEntries = $this->getAll("SELECT * FROM `" . $this->_sPrefix . $this->_sTableService . "` WHERE 1 {$sTypeQuery} {$sIdQuery} ORDER BY `created` DESC {$sqlLimit}");

		return $aAllEntries;   
	}

 	function getServiceCount($sType='', $iEntryId=0) {
		$iEntryId = (int)$iEntryId;
		if($iEntryId){ 
			$sIdQuery = ' AND `professional_id`='.$iEntryId;
		}
		...
		$bCount = $this->getOne("SELECT COUNT(`id`) FROM `" . $this->_sPrefix . $this->_sTableService . "` WHERE 1  {$sTypeQuery} {$sIdQuery}");
    ...
    function isSubProfileFan($sTable, $iSubEntryId, $iProfileId, $isConfirmed) {
        ...
        $iEntryId = (int)$this->getOne ("SELECT `professional_id` FROM `" . $this->_sPrefix . $sTable . "` WHERE `id` = '$iSubEntryId' LIMIT 1");
	...
	function getAllSubItems($sSubItem, $iProfessionalId){
		$aSubItems = array();
		$iProfessionalId = (int)$iProfessionalId;

		switch($sSubItem){ 
			case 'review':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableReview . "` WHERE `professional_id`=$iProfessionalId");
			break; 			
			case 'service':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableService . "` WHERE `professional_id`=$iProfessionalId");
			break; 
			case 'client':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableClient . "` WHERE `professional_id`=$iProfessionalId");
			break; 
		}
	...
	function getBusinessJobs($iEntryId){ 
		$iEntryId = (int)$iEntryId;
		return $this->getPairs ("SELECT `id` FROM `modzzz_jobs_main` WHERE `company_type`='professional' AND `company_id`=$iEntryId",'id','id'); 
	}  

	function getBusinessJobsById($iEntryId){ 
		$iEntryId = (int)$iEntryId;
		return $this->getAll ("SELECT `id`,`title`,`uri` FROM `modzzz_jobs_main` WHERE `company_type`='professional' AND `company_id`=$iEntryId"); 
	}  

	function getBusinessDeals($iEntryId){ 
		$iEntryId = (int)$iEntryId;
		return $this->getPairs ("SELECT `id` FROM `modzzz_deals_main` WHERE `company_type`='professional' AND `store_id`=$iEntryId",'id','id'); 
	}
	...
	//BEGIN Youtube 
	function getYoutubeVideos($iEntryId, $sType='main', $iLimit=0){
		$iEntryId = (int)$iEntryId;
		...
		if($sType=='main'){
			...
			return $this->getAll("SELECT `id`, `id_entry`, `id_sub_entry`, `title`, `url` FROM `" . $this->_sPrefix . "youtube` WHERE `id_sub_entry`={$iEntryId} {$sQuery}");
		}
	}

 	function addYoutube($iEntryId,$iSubEntryId=0,$sType='main'){
		if(is_array($_POST['video_link'])){ 
			foreach($_POST['video_link'] as $iKey=>$sValue){
				...
				if(trim($sVideoLink)){  
					$this->query("INSERT INTO `" . $this->_sPrefix . "youtube` SET `id_entry`=$iEntryId, `id_sub_entry`=$iSubEntryId, `url`='$sVideoLink', `title`='$sVideoTitle' ");
				}
			} 
		}
	}
	...
	function removeAlbumYoutubeEntries($iEntryId){ 
	   
		$this->query("DELETE FROM `" . $this->_sPrefix . "youtube` WHERE `id_sub_entry`='$iEntryId'");   
	}
    ...
    function getYoutubeIds ($iEntryId, $sType='main') {
		if($sType=='main')
			return $this->getPairs ("SELECT `id` FROM `" . $this->_sPrefix . "youtube`  WHERE `id_entry` = '$iEntryId'", 'id', 'id');
		else
			return $this->getPairs ("SELECT `id` FROM `" . $this->_sPrefix . "youtube`  WHERE `id_sub_entry` = '$iEntryId'", 'id', 'id');
    }
	//END Youtube
	...
	function getPeriods($sPeriod=''){
		...
		$aPeriods = array(
			'am'=>_t('_modzzz_professional_time_am'),
			'pm'=>_t('_modzzz_professional_time_pm'), 
		);
	...
	function getDays($iDay=0, $sType='short'){
 
		$aDays = array(
			1=>_t('_modzzz_professional_'.$sType.'_mon'),
			2=>_t('_modzzz_professional_'.$sType.'_tue'),
			3=>_t('_modzzz_professional_'.$sType.'_wed'),
			4=>_t('_modzzz_professional_'.$sType.'_thur'),
			5=>_t('_modzzz_professional_'.$sType.'_fri'),
			6=>_t('_modzzz_professional_'.$sType.'_sat'),
			7=>_t('_modzzz_professional_'.$sType.'_sun'), 
		);
	...
	function getScheduleFeed($iEntryId, $sType=''){
 		$iEntryId = (int)$iEntryId;

		return $this->getAll("SELECT * FROM `" . $this->_sPrefix . $this->_sTableSchedule . "` WHERE `entry_id`={$iEntryId} AND `type`='main'");
	}
 
 	function addSchedule($iEntryId){
  
		$iEntryId = (int)$iEntryId;

		if(is_array($_POST['schedule_from_day'])){ 
			foreach($_POST['schedule_from_day'] as $iKey=>$sValue){
			
				$iFromDay = (int)process_db_input($sValue);
 				$iToDay = (int)process_db_input($_POST['schedule_to_day'][$iKey]);
 				$iFromHour = process_db_input($_POST['schedule_from_hour'][$iKey]);
 				$iFromMinute = process_db_input($_POST['schedule_from_minute'][$iKey]);
				$iFromPeriod = process_db_input($_POST['schedule_from_period'][$iKey]); 
 				$iToHour = process_db_input($_POST['schedule_to_hour'][$iKey]);
 				$iToMinute = process_db_input($_POST['schedule_to_minute'][$iKey]);
				$iToPeriod = process_db_input($_POST['schedule_to_period'][$iKey]);
 
				if($iFromDay){  
					$this->query("INSERT INTO `" . $this->_sPrefix . $this->_sTableSchedule . "` SET
					`type`='main',
					`entry_id`=$iEntryId, 
					`from_day`='$iFromDay', 
					`to_day`='$iToDay', 
	...
	function removeScheduleEntries($iEntryId){  
		$this->query("DELETE FROM `" . $this->_sPrefix . $this->_sTableSchedule . "` WHERE `entry_id`='$iEntryId'");   
	}

	function removeScheduleEntry($iEntryId, $iScheduleId){ 
		$this->query("DELETE FROM `" . $this->_sPrefix . $this->_sTableSchedule . "` WHERE `entry_id`='$iEntryId' AND `id`='$iScheduleId'");   
	}

    function getScheduleIds ($iEntryId) {
		return $this->getPairs ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableSchedule . "`  WHERE `entry_id` = '$iEntryId'", 'id', 'id');
    } 
	//END Schedule
 
	function triggerAlert($sTemplate, $iSenderId, $iRecipientId, $aProfessional=array(), $aService=array(), $aBooking=array()) {
	   
		$aPlus = array();
 
		$aPlus['ProfessionalName'] = getNickName($aProfessional['author_id']); 
		$aPlus['ProfessionalLink'] = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aProfessional['uri']; 
 
		$aPlus['ServiceName'] = getNickName($aService['author_id']); 
		$aPlus['ServiceLink'] = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'service/view/' . $aService['uri']; 
   
		$aPlus['BookingLink'] = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'booking/view/' . $aBooking['uri']; 

		$aSender = getProfileInfo($iSenderId); 
		$aPlus['SenderName'] = getNickName($iSenderId);
		$aPlus['SenderLink'] = getProfileLink($iSenderId);

		$aRecipient = getProfileInfo($iRecipientId); 
		$aPlus['RecipientName'] = getNickName($iRecipientId);
		$aPlus['RecipientLink'] = getProfileLink($iRecipientId);
