...
class BxProfessionalServiceCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxProfessionalServiceCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aServiceEntry = $oMain->_oDb->getServiceEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableService, $aServiceEntry);
     }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aServiceEntry = $oMain->_oDb->getServiceEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aServiceEntry['professional_id']); 
