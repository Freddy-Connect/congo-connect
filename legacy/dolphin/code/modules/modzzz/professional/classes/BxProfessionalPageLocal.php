...
class BxProfessionalPageLocal extends BxDolTwigPageMain {
 		var $_oDb;
        var $_oConfig; 
		var $sCountry; 
		var $sState; 
    ...
    function BxProfessionalPageLocal(&$oMain, $sCountry, $sState, $sCategory) {
		...
		if($sCountry)
			parent::BxDolTwigPageMain('modzzz_professional_local_state', $oMain); 
		else	
			parent::BxDolTwigPageMain('modzzz_professional_local', $oMain); 

	}
 
    function getBlockCode_StateProfessionals() {
  
		if($this->sState){ 	 
			
			$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'local/'. $this->sCountry .'/'. $this->sState . ($this->sCategory  ? '/'. $this->sCategory : '') . '?';

			$sProfessionals = $this->ajaxBrowse('local_state', $this->oDb->getParam('modzzz_professional_perpage_main_recent'), array(), $this->sCountry, $this->sState, $this->sCategory);
			...
			$sProfessionals = $this->ajaxBrowse('local_country', $this->oDb->getParam('modzzz_professional_perpage_main_recent'), array(), $this->sCountry,   $this->sCategory); 
		} 

		return $sProfessionals;  
	}
    ...
    function getBlockCode_Categories() {
		...
		if($this->sState){  
			...
			$sItemLabel  = _t('_modzzz_professional_category_drilldown_for_item', $sItemName);
    ...
    function getBlockCode_States() {
	    ...
	    return $this->oTemplate->parseHtmlByName('professional_states', $aVars);   
	}
    ...
    function getBlockCode_Region() {
	    ...
	    return $this->oTemplate->parseHtmlByName('professional_regions', $aVars);  
	}
