...
class BxProfessionalClientPageBrowse extends BxDolTwigPageMain {

    function BxProfessionalClientPageBrowse(&$oMain, $sUri) {
        ...
        $this->sUrlStart = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() . 'client/browse/'. $this->sUri;  
        $this->sUrlStart .= (false === strpos($this->sUrlStart, '?') ? '?' : '&');  
 
        $this->sSearchResultClassName = 'BxProfessionalSearchResult';
        $this->sFilterName = 'filter';
		parent::BxDolTwigPageMain('modzzz_professional_clients_browse', $oMain);
	}
