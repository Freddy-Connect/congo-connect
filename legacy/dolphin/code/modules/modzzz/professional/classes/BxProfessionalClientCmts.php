...
class BxProfessionalClientCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxProfessionalClientCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aClientEntry = $oMain->_oDb->getClientEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableClient, $aClientEntry);
     }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aClientEntry = $oMain->_oDb->getClientEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aClientEntry['professional_id']); 
