...
class BxProfessionalReviewFormAdd extends BxDolFormMedia {

    var $_oMain, $_oDb;

    function BxProfessionalReviewFormAdd ($oMain, $iProfileId, $iProfessionalId = 0, $iEntryId = 0, $iThumb = 0) { 
        $this->_oMain = $oMain;
        $this->_oDb = $oMain->_oDb;

		$this->_oDb->_sTableMediaPrefix = $this->_oDb->_sTableReviewMediaPrefix;
  		bx_import ('SubPrivacy', $this->_oMain->_aModule);
		$GLOBALS['oBxProfessionalModule']->_oSubPrivacy = new BxProfessionalSubPrivacy($this->_oMain, $this->_oDb->_sTableReview); 
		...
		$aService = $this->_oDb->getFormServiceEntries($iProfessionalId);
		$aService = array('0' => _t('_modzzz_professional_general_feedback')) + $aService;
        ...
        $aCustomForm = array(
            ...
            'inputs' => array(
                ...
                'header_info' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_professional_form_review_header_info')
                ),                
                ...
                'service_id' => array(
                    'type' => 'select',
                    'name' => 'service_id',
                    'caption' => _t('_modzzz_professional_form_caption_service'),
                    'values' => $aService, 
                    'db' => array (
                        'pass' => 'Xss', 
                    ) 
