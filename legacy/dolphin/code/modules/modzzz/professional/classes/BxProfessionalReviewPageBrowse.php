...
class BxProfessionalReviewPageBrowse extends BxDolTwigPageMain {

    function BxProfessionalReviewPageBrowse(&$oMain, $sUri) {
        ...
        $this->sUrlStart = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() . 'review/browse/'. $this->sUri;  
        $this->sUrlStart .= (false === strpos($this->sUrlStart, '?') ? '?' : '&');  

        $this->sSearchResultClassName = 'BxProfessionalSearchResult';
        $this->sFilterName = 'filter';
		parent::BxDolTwigPageMain('modzzz_professional_reviews_browse', $oMain);
	}
