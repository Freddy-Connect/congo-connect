...
class BxProfessionalPageSubCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxProfessionalPageSubCategory(&$oMain, $sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_professional_subcategory', $oMain);
    ...
    function getBlockCode_SubCategoryProfessionals() {

 		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'subcategories/'.$this->aCategoryInfo['uri'].'?';

        $sCode = $this->ajaxBrowse('subcategories', $this->oDb->getParam('modzzz_professional_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
