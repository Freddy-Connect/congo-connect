...
class BxProfessionalServicePageBrowse extends BxDolTwigPageMain {

    function BxProfessionalServicePageBrowse(&$oMain, $sUri) {
        ...
        $this->sUrlStart = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() . 'service/browse/'. $this->sUri;  
        $this->sUrlStart .= (false === strpos($this->sUrlStart, '?') ? '?' : '&');  

        $this->sSearchResultClassName = 'BxProfessionalSearchResult';
        $this->sFilterName = 'filter';
		parent::BxDolTwigPageMain('modzzz_professional_services_browse', $oMain);
	}
    ...
    function getBlockCode_Browse () {
        ...
        return $this->ajaxBrowseSubProfile(
            'service',
            'services',
            $this->oDb->getParam('modzzz_professional_perpage_browse_subitems'), 
