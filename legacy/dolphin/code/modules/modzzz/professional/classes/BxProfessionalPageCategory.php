...
class BxProfessionalPageCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxProfessionalPageCategory(&$oMain,$sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_professional_category', $oMain);
		...
		if(isset($_GET['ajax']) && ($_GET['ajax']=='sort')){
			echo $this->getBlockCode_CategoryProfessional(); 
			exit;
		}  
	}
    ...
    function getSubCategories() { 
	    ...
	    return $this->oTemplate->parseHtmlByName('professional_subcategories', $aVars);  
	}
 
    function getBlockCode_CategoryProfessional() {

 		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories/'.$this->aCategoryInfo['uri'].'?';

        $sCode = $this->ajaxBrowse('categories', $this->oDb->getParam('modzzz_professional_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
