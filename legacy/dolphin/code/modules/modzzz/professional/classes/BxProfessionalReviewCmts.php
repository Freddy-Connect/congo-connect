...
class BxProfessionalReviewCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxProfessionalReviewCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aReviewEntry = $oMain->_oDb->getReviewEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableReview, $aReviewEntry);
     }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aReviewEntry = $oMain->_oDb->getReviewEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aReviewEntry['professional_id']); 
