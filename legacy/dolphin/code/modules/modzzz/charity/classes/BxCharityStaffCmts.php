...
class BxCharityStaffCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxCharityStaffCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aStaffEntry = $oMain->_oDb->getStaffEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableStaff, $aStaffEntry);
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aStaffEntry = $oMain->_oDb->getStaffEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aStaffEntry['charity_id']); 
