...
class BxCharityMembersCmts extends BxTemplCmtsView {
 

	/**
	 * Constructor
	 */
	function BxCharityMembersCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
        if (!parent::isPostReplyAllowed())
            return false;
        $oMain = $this->getMain();  
		$aMemberEntry = $oMain->_oDb->getMembersEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableMembers, $aMemberEntry);
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain(); 
		$aMemberEntry = $oMain->_oDb->getMembersEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aMemberEntry['charity_id']);
