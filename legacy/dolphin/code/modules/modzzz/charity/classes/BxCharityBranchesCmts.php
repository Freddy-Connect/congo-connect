...
class BxCharityBranchesCmts extends BxTemplCmtsView {
 

	/**
	 * Constructor
	 */
	function BxCharityBranchesCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
        if (!parent::isPostReplyAllowed())
            return false;
        $oMain = $this->getMain();  
		$aBranchEntry = $oMain->_oDb->getBranchesEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableBranches, $aBranchEntry);
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain(); 
		$aBranchEntry = $oMain->_oDb->getBranchesEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aBranchEntry['charity_id']);
