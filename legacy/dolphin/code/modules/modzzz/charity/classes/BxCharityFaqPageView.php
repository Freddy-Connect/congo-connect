...
class BxCharityFaqPageView extends BxDolTwigPageView {	

    function BxCharityFaqPageView(&$oFaqsMain, &$aFaq) {
        parent::BxDolTwigPageView('modzzz_charity_faqs_view', $oFaqsMain, $aFaq); 
	}
   
	function getBlockCode_FaqItems() { 

		$sCode = '';
		$iLimit = 30;//$this->_oDb->getParam('modzzz_charity_perpage_faqs');
		$sUrl = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'home' ;
  
		$iCount = $this->_oDb->getFaqItemCount($this->aDataEntry['id']);  

		if(!$iCount){
			$sCode = MsgBox(_t('_modzzz_charity_faqs_no_items'));
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $this->aInfo = array (
                ...
                'TitleDelete' => $this->_oMain->isAllowedSubDelete($aDataEntry, $aFaqEntry) ? _t('_modzzz_charity_action_title_delete') : '',
             );

            if (!$this->aInfo['TitleDelete'])
                return '';

            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_charity_faq');
        } 
