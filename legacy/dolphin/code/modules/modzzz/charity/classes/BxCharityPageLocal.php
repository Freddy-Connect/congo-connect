...
class BxCharityPageLocal extends BxDolTwigPageMain {
 		var $_oDb;
        var $_oConfig; 
		var $sCountry; 
		var $sState; 

    function BxCharityPageLocal(&$oMain, $sCountry, $sState) {
		...
		if($sCountry)
			parent::BxDolTwigPageMain('modzzz_charity_local_state', $oMain); 
		else	
			parent::BxDolTwigPageMain('modzzz_charity_local', $oMain); 

	}
 
    function getBlockCode_StateCharities() {
  
		if($this->sState){ 
			$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'local/'. $this->sCountry .'/'. $this->sState . '?';
			
			$sCharities = $this->ajaxBrowse('local_state', $this->oDb->getParam('modzzz_charity_perpage_main_recent'), array(), $this->sState, false, false);
		}elseif($this->sCountry){
			$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'local/'. $this->sCountry . '?';

			$sCharities = $this->ajaxBrowse('local_country', $this->oDb->getParam('modzzz_charity_perpage_main_recent'), array(), $this->sCountry, false, false);
		} 
    ...
    function getBlockCode_States() {
	    ...
	    return $this->oTemplate->parseHtmlByName('charity_states', $aVars);   
	}
    ...
    function getBlockCode_Region() {
	    ...
	    return $this->oTemplate->parseHtmlByName('charity_regions', $aVars);  
	}
