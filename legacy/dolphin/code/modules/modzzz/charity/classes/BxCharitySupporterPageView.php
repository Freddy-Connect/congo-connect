...
class BxCharitySupporterPageView extends BxDolTwigPageView {	

    function BxCharitySupporterPageView(&$oSupportersMain, &$aSupporter) {
        parent::BxDolTwigPageView('modzzz_charity_supporters_view', $oSupportersMain, $aSupporter);
	...
	function _blockFiles ($aReadyMedia, $iAuthorId = 0) {        
        ...
        foreach ($aReadyMedia as $iMediaId) {        
            ...
            $aInputBtnDownload = array (
                ...
                'value' => _t ('_download'), 
                'attrs' => array(
                    'onclick' => "window.open ('" . BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . "supporter/download/".$this->aDataEntry[$this->_oDb->_sFieldId]."/{$iMediaId}','_self');",
                ),
            );
    ...
    function getBlockCode_Rate() {
        modzzz_charity_import('SupporterVoting');
        $o = new BxCharitySupporterVoting ('modzzz_charity_supporter', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableSupporter, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_charity_import('SupporterCmts');
        $o = new BxCharitySupporterCmts ('modzzz_charity_supporter', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
			...
			$aSupporterEntry = $this->_oDb->getSupporterEntryById($this->aDataEntry['id']);
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableSupporter,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_photos') : '', 
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableSupporter,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableSupporter,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableSupporter,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_files') : '', 
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_charity_supporter');
        } 
