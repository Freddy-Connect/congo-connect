...
class BxCharityPageCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxCharityPageCategory(&$oMain,$sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_charity_category', $oMain);
		...
		if(isset($_GET['ajax']) && ($_GET['ajax']=='sort')){
			echo $this->getBlockCode_CategoryCharity(); 
			exit;
		} 
    ...
    function getSubCategories() { 
	    ...
	    return $this->oTemplate->parseHtmlByName('charity_subcategories', $aVars);  
	}
 
    function getBlockCode_CategoryCharity() {

		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories/'.$this->aCategoryInfo['uri'].'?';

        $sCode = $this->ajaxBrowse('categories', $this->oDb->getParam('modzzz_charity_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
