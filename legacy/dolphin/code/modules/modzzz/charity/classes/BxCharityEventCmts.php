...
class BxCharityEventCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxCharityEventCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aEventEntry = $oMain->_oDb->getEventEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableEvent, $aEventEntry);
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aEventEntry = $oMain->_oDb->getEventEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aEventEntry['charity_id']); 
