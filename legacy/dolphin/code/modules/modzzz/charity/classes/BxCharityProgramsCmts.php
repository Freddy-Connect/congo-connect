...
class BxCharityProgramsCmts extends BxTemplCmtsView {
 

	/**
	 * Constructor
	 */
	function BxCharityProgramsCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
        if (!parent::isPostReplyAllowed())
            return false;
        $oMain = $this->getMain();  
		$aProgramEntry = $oMain->_oDb->getProgramsEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTablePrograms, $aProgramEntry);
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain(); 
		$aProgramEntry = $oMain->_oDb->getProgramsEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aProgramEntry['charity_id']);
