...
class BxCharityBranchesPageView extends BxDolTwigPageView {	

    function BxCharityBranchesPageView(&$oBranchesMain, &$aBranches) {
        parent::BxDolTwigPageView('modzzz_charity_branches_view', $oBranchesMain, $aBranches);
	...
	function _blockFiles ($aReadyMedia, $iAuthorId = 0) {        
        ...
        foreach ($aReadyMedia as $iMediaId) {        
            ...
            $aInputBtnDownload = array (
                ...
                'value' => _t ('_download'), 
                'attrs' => array(
                    'onclick' => "window.open ('" . BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . "branches/download/".$this->aDataEntry[$this->_oDb->_sFieldId]."/{$iMediaId}','_self');",
                ),
            );
    ...
    function getBlockCode_Rate() {
        modzzz_charity_import('BranchesVoting');
        $o = new BxCharityBranchesVoting ('modzzz_charity_branches', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableBranches, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_charity_import('BranchesCmts');
        $o = new BxCharityBranchesCmts ('modzzz_charity_branches', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableBranches,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_photos') : '', 
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableBranches,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableBranches,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableBranches,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_files') : '', 
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_charity_branches');
        } 
