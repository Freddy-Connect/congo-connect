...
class BxCharityReviewPageView extends BxDolTwigPageView {	

    function BxCharityReviewPageView(&$oReviewsMain, &$aReview) {
        parent::BxDolTwigPageView('modzzz_charity_reviews_view', $oReviewsMain, $aReview);
	...
	function _blockFiles ($aReadyMedia, $iAuthorId = 0) {        
        ...
        foreach ($aReadyMedia as $iMediaId) {        
            ...
            $aInputBtnDownload = array (
                ...
                'value' => _t ('_download'), 
                'attrs' => array(
                    'onclick' => "window.open ('" . BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . "review/download/".$this->aDataEntry[$this->_oDb->_sFieldId]."/{$iMediaId}','_self');",
                ),
            );
    ...
    function getBlockCode_Rate() {
        modzzz_charity_import('ReviewVoting');
        $o = new BxCharityReviewVoting ('modzzz_charity_review', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableReview, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_charity_import('ReviewCmts');
        $o = new BxCharityReviewCmts ('modzzz_charity_review', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
			...
			$iEntryId = $aReviewEntry['charity_id'];
            ...
            $this->aInfo = array (
                ...
                'TitleEdit' => $this->_oMain->isAllowedSubEdit($aDataEntry, $aReviewEntry) ? _t('_modzzz_charity_action_title_edit') : '',
                'TitleDelete' => $this->_oMain->isAllowedSubDelete($aDataEntry, $aReviewEntry) ? _t('_modzzz_charity_action_title_delete') : '',
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_charity_review');
        } 
