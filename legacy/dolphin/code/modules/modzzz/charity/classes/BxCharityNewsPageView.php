...
class BxCharityNewsPageView extends BxDolTwigPageView {	

    function BxCharityNewsPageView(&$oNewsMain, &$aNews) {
        parent::BxDolTwigPageView('modzzz_charity_news_view', $oNewsMain, $aNews);
    ...
    function getBlockCode_Rate() {
        modzzz_charity_import('NewsVoting');
        $o = new BxCharityNewsVoting ('modzzz_charity_news', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableNews, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_charity_import('NewsCmts');
        $o = new BxCharityNewsCmts ('modzzz_charity_news', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableNews,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_photos') : '',  
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableNews,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableNews,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableNews,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_files') : '', 
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_charity_news');
        } 
