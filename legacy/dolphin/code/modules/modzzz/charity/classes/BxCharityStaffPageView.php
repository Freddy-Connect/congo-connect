...
class BxCharityStaffPageView extends BxDolTwigPageView {	

    function BxCharityStaffPageView(&$oStaffsMain, &$aStaff) {
        parent::BxDolTwigPageView('modzzz_charity_staffs_view', $oStaffsMain, $aStaff);
	...
	function _blockFiles ($aReadyMedia, $iAuthorId = 0) {        
        ...
        foreach ($aReadyMedia as $iMediaId) {        
            ...
            $aInputBtnDownload = array (
                ...
                'value' => _t ('_download'), 
                'attrs' => array(
                    'onclick' => "window.open ('" . BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . "staff/download/".$this->aDataEntry[$this->_oDb->_sFieldId]."/{$iMediaId}','_self');",
                ),
            );
    ...
    function getBlockCode_Rate() {
        modzzz_charity_import('StaffVoting');
        $o = new BxCharityStaffVoting ('modzzz_charity_staff', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableStaff, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_charity_import('StaffCmts');
        $o = new BxCharityStaffCmts ('modzzz_charity_staff', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
			...
			$aStaffEntry = $this->_oDb->getStaffEntryById($this->aDataEntry['id']);
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableStaff,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_photos') : '', 
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableStaff,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableStaff,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableStaff,$this->aDataEntry) ? _t('_modzzz_charity_action_upload_files') : '', 
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_charity_staff');
        } 
