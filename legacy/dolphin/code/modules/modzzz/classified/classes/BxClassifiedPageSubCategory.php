...
class BxClassifiedPageSubCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxClassifiedPageSubCategory(&$oMain, $sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_classified_subcategory', $oMain);
    ...
    function getBlockCode_SubCategoryClassifieds() {

		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'subcategories/'.$this->aCategoryInfo['uri'].'?';

        $aBlock = $this->ajaxBrowse('subcategories', $this->oDb->getParam('modzzz_classified_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
