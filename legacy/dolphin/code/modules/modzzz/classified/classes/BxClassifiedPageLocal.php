...
class BxClassifiedPageLocal extends BxDolTwigPageMain {
 		var $_oDb;
        var $_oConfig; 
		var $sCountry; 
		var $sState;   
    ...
    function BxClassifiedPageLocal(&$oMain, $sCountry, $sState, $sCategory) {
		...
		if($sCountry)
			parent::BxDolTwigPageMain('modzzz_classified_local_state', $oMain); 
		else	
			parent::BxDolTwigPageMain('modzzz_classified_local', $oMain); 

	}
 
    function getBlockCode_StateClassifieds() {
		...
		if($this->sState){ 	 
			...
			$aBlock = $this->ajaxBrowse('local_state', $this->oDb->getParam('modzzz_classified_perpage_main_recent'), array(), $this->sCountry, $this->sState, $this->sCategory);

			$sStateName = $this->oDb->getStateName($this->sCountry, $this->sState);
			$aBlock = ($aBlock=='') ? array(MsgBox(_t('_Empty'))) : $aBlock;  
			$aBlock[3] = _t('_modzzz_classified_block_browse_state_classifieds') .' - '. $sStateName;
			...
			$aBlock = $this->ajaxBrowse('local_country', $this->oDb->getParam('modzzz_classified_perpage_main_recent'), array(), $this->sCountry,   $this->sCategory); 

			$sCountryName = $this->oTemplate->getPreListDisplay('Country', $this->sCountry);
			$aBlock = ($aBlock=='') ? array(MsgBox(_t('_Empty'))) : $aBlock;  
			$aBlock[3] = _t('_modzzz_classified_block_browse_country') .' - '. $sCountryName; 
    ...
    function getBlockCode_Categories() {
		...
		if($this->sState){  
			...
			$sItemLabel  = _t('_modzzz_classified_category_drilldown_for_item', $sItemName);
		...
		{	 
 	    ...
 	    $aClassified = array($this->oTemplate->parseHtmlByName('local_categories', $aResult)); 
		$aClassified[3] = $sItemLabel;
    ...
    function getBlockCode_States() {
		...
		$aVars['bx_repeat:entries'] = array();        
 	    ...
 	    $aStates = array($this->oTemplate->parseHtmlByName('block_states_local', $aVars)); 
    ...
    function getBlockCode_Region() {
	    ...
	    return $this->oTemplate->parseHtmlByName('classified_regions', $aVars);  
	}
 
     function ajaxBrowse($sMode, $iPerPage, $aMenu = array(), $sValue = '', $sValue2 = '', $sValue3 = '', $isDisableRss = false, $isPublicOnly = false) {
