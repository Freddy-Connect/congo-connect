...
class BxClassifiedPageCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxClassifiedPageCategory(&$oMain,$sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_classified_category', $oMain);
		...
		if(isset($_GET['ajax']) && ($_GET['ajax']=='sort')){
			echo $this->getBlockCode_CategoryClassified(); 
			exit;
		}  
	}
    ...
    function getSubCategories() { 
	    ...
	    return $this->oTemplate->parseHtmlByName('classified_subcategories', $aVars);  
	}
 
    function getBlockCode_CategoryClassified() {

 		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories/'.$this->aCategoryInfo['uri'].'?';

        $aBlock = $this->ajaxBrowse('categories', $this->oDb->getParam('modzzz_classified_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
 
		$aBlock = ($aBlock=='') ? array(MsgBox(_t('_Empty'))) : $aBlock; 
  
		$aBlock[3] = _t('_modzzz_classified_block_category_classified') .' - '. $this->aCategoryInfo['name'];
