...
/*
 * Classified module Data
 */
class BxClassifiedDb extends BxDolTwigModuleDb {	
	 ...
	 * Constructor.
	 */
	function BxClassifiedDb(&$oConfig) {
        ...
        $this->_sFieldAllowViewTo = 'allow_view_classified_to';
	}
	
	function getFactureInvoiceCount ($iProfileId ) {
        return $this->getOne ("SELECT  COUNT(`f`.`id`) FROM  `modzzz_classified_invoices` AS  `f` RIGHT JOIN  `modzzz_classified_main` AS  `l` ON  `f`.`classified_id` =  `l`.`id`  and `l`.`author_id` = '$iProfileId' and `f`.`invoice_status` = 'pending'");
    }
	// Freddy ajout  Total annonces  ajoutées pour chaque membre
	function getModzzzCountAnnonceHome($iAuthorId){ 
		return $this->getOne("SELECT COUNT(`id`) FROM `modzzz_classified_main` WHERE `author_id`= '$iAuthorId'  AND `status`='approved' ");  
	}
	
	// Freddy nombre des annonces favorites  pour un membre
  
  /*
   function CountFavoriteMemberClassified($iProfileId){
	...
	*/
	function CountFavoriteMemberClassified($iProfileId){
	    $iProfileId = (int)$iProfileId;
		return $this->getOne ("SELECT COUNT(`id_entry`) FROM `modzzz_classified_favorites` AS  `f` RIGHT JOIN  `modzzz_classified_main` AS  `l` ON  `f`.`id_entry` =  `l`.`id`  and  `id_profile`=$iProfileId and `l`.`status` ='approved'  LIMIT 1"); 
  ...
  function getMergedCompanyList($iAuthorId=0) {
        ...
        $arr = array();
        // Freddy
		 //$arr['0'] = ' ' ._t('_modzzz_classified_select_company');
		...
		// Freddy 
		// $arr['-99'] = _t('_modzzz_classified_add_company');

         return $arr;
    }
 

	// Fin Integration Business listing
  
  
  
   //Freddy 08juillet 2015 Business Location
	...
	function getBusinessZip($iAuthorId=0) {
	 ...
	 function getBusinessAddress1($iAuthorId=0) {
  
        
         $aBusinessAddress1 = $this->getOne ("SELECT  `address1` FROM `modzzz_listing_main` WHERE `status` = 'approved' AND `author_id` = '$iAuthorId' LIMIT 1");
 
     return $aBusinessAddress1;
	  }
	  //
	   function getBusinessWebsite($iAuthorId=0) {
  
        
         $aBusinessWebsite = $this->getOne ("SELECT  `businesswebsite` FROM `modzzz_listing_main` WHERE `status` = 'approved' AND `author_id` = '$iAuthorId' LIMIT 1");
 
     return $aBusinessWebsite;
	  }
	  //
	  
	   function getBusinessTelephone($iAuthorId=0) {
  
        
         $aBusinessTelephone = $this->getOne ("SELECT  `businesstelephone` FROM `modzzz_listing_main` WHERE `status` = 'approved' AND `author_id` = '$iAuthorId' LIMIT 1");
 
     return $aBusinessTelephone;
	  }
	  
	// Fin Freddy Business location
	
	 function getBusinessMobile($iAuthorId=0) {
  
        
         $aBusinessTelephone = $this->getOne ("SELECT  `businessfax` FROM `modzzz_listing_main` WHERE `status` = 'approved' AND `author_id` = '$iAuthorId' LIMIT 1");
	...
	function setInvoiceStatus($iItemId, $sStatus) {
 		 ...
 		 $this->query("UPDATE `" . $this->_sPrefix . "invoices` SET `invoice_status`='$sStatus' WHERE `classified_id`='$iItemId'"); 
	}
	...
	function SavePackage($iParent=0){
		...
		$this->query("INSERT INTO `" . $this->_sPrefix . "packages` SET `name`='$sName', `description`='$sDescription', `price`=$fPrice, `days`=$iDays, `sounds`='$iSounds', `files`='$iFiles', `videos`='$iVideos', `photos`='$iPhotos', `featured`=$iFeatured");
    ...
    function deleteEntryByIdAndOwner ($iId, $iOwner, $isAdmin) {
        if ($iRet = parent::deleteEntryByIdAndOwner ($iId, $iOwner, $isAdmin)) {
            ...
            $this->query ("DELETE FROM `" . $this->_sPrefix . "invoices` WHERE `classified_id` = $iId");
    ...
    function saveTransactionRecord($iBuyerId, $iClassifiedId, $sTransNo, $sTransType) {
        $iBuyerId    = (int)$iBuyerId;
        $iClassifiedId  = (int)$iClassifiedId; 
   		$iTime = time();

		$aDataEntry = $this->getEntryById($iClassifiedId);
		...
		$iOrderId = $this->lastId();
		 ...
		 //if order successful, update expiration date of classified
		 if($iOrderId){
			$aInvoice = $this->getInvoiceByNo($sInvoiceNo, false);
			$iDays = (int)$aInvoice['days'];
			$this->updateEntryExpiration($iClassifiedId, $iDays);
		  
			$iPackageId = (int)$aInvoice['package_id'];
			$aPackage = $this->getPackageById($iPackageId); 
			$iFeatured = (int)$aPackage['featured'];
			if($iFeatured){
				$this->updateFeaturedStatus($iClassifiedId);
			}  
		 }
	...
	function createInvoice($iEntryId, $iPackageId, $fPrice, $iDays, $sStatus='pending'){
		 ...
		 $iInvoiceActiveDays = (int)getParam('modzzz_classified_invoice_valid_days');
    ...
    function getInvoiceByNo($sInvoiceNo, $bCheckStatus=true) {  
		...
		return $this->getRow("SELECT `invoice_no`, `price`, `days`, `classified_id`, `package_id`, `invoice_status`, `invoice_due_date`, `invoice_expiry_date`, `invoice_date` FROM `" . $this->_sPrefix . "invoices` WHERE `invoice_no` = '{$sInvoiceNo}' {$sStatusCheck}"); 
    }
   ...
   /** category functions*/
	...
	function getParentCategories(){
		return $this->getAll("SELECT `id`, `parent`, `name`, `uri`, `type`, `custom_field1`, `custom_field2`, `custom_field3`, `custom_field4`, `custom_field5`, `custom_field6`, `custom_field7`, `custom_field8`, `custom_field9`, `custom_field10` FROM `" . $this->_sPrefix . "categ` WHERE `parent` = 0  ORDER BY `name` ASC"); 
	}

 	function getSubCategories($iId){
		return $this->getAll("SELECT `id`, `parent`, `name`, `uri`, `type`, `custom_field1`, `custom_field2`, `custom_field3`, `custom_field4`, `custom_field5`, `custom_field6`, `custom_field7`, `custom_field8`, `custom_field9`, `custom_field10`  FROM `" . $this->_sPrefix . "categ` WHERE `parent` = '$iId' ORDER BY `name` ASC"); 
	}

	function getCategoryInfo ($iId=0, $bGetAll=false){
		if($bGetAll){
			return $this->getAll("SELECT `id`, `name`, `uri`, `type`, `custom_field1`, `custom_field2`, `custom_field3`, `custom_field4`, `custom_field5`, `custom_field6`, `custom_field7`, `custom_field8`, `custom_field10` FROM `" . $this->_sPrefix . "categ` WHERE `active` = 1 AND `parent`=0 ORDER BY `name` ASC");
		}else{
			return $this->getRow("SELECT `id`, `name`, `uri`, `type`, `custom_field1`, `custom_field2`, `custom_field3`, `custom_field4`, `custom_field5`, `custom_field6`, `custom_field7`, `custom_field8`, `custom_field10` FROM `" . $this->_sPrefix . "categ` WHERE `id` = '$iId'");
		}
	}
	...
	function getCategoryByUri($sUri){
		return $this->getRow("SELECT `id`, `name`, `uri`, `type`, `custom_field1`, `custom_field2`, `custom_field3`, `custom_field4`, `custom_field5`, `custom_field6`, `custom_field7`, `custom_field8`, `custom_field10` FROM `" . $this->_sPrefix . "categ` WHERE `uri` = '$sUri'"); 
	}
 
	function getParentCategoryInfo ($iId){
		$iParentId = (int)$this->getOne("SELECT `parent` FROM `" . $this->_sPrefix . "categ` WHERE `id` = '$iId'");
 
		return $this->getRow("SELECT `id`,`name`,`uri`, `type`, `custom_field1`, `custom_field2`, `custom_field3`, `custom_field4`, `custom_field5`, `custom_field6`, `custom_field7`, `custom_field8`, `custom_field10` FROM `" . $this->_sPrefix . "categ` WHERE `id` = '$iParentId'"); 
	}

	function getSubCategoryInfo ($iParentId=0, $iId=0, $bGetAll=false){
		if($bGetAll){ 
			return $this->getAll("SELECT `id`, `name`, `uri`, `type`, `custom_field1`, `custom_field2`, `custom_field3`, `custom_field4`, `custom_field5`, `custom_field6`, `custom_field7`, `custom_field8`, `custom_field10` FROM `" . $this->_sPrefix . "categ` WHERE `active` = 1 AND `parent`='$iParentId' ORDER BY `name` ASC");
		}else{
			return $this->getRow("SELECT `id`, `name`, `uri`, `type`, `custom_field1`, `custom_field2`, `custom_field3`, `custom_field4`, `custom_field5`, `custom_field6`, `custom_field7`, `custom_field8`, `custom_field10` FROM `" . $this->_sPrefix . "categ` WHERE `id` = '$iId'");
		}
	}
	...
	function getCategoryInfoByUri ($sUri){ 
		return $this->getRow("SELECT `id`, `parent`, `name`, `uri`, `type`, `custom_field1`, `custom_field2`, `custom_field3`, `custom_field4`, `custom_field5`, `custom_field6`, `custom_field7`, `custom_field8`, `custom_field10` FROM `" . $this->_sPrefix . "categ` WHERE `active` = 1 AND `uri`='$sUri'"); 
	}
	...
	function getLocalCategoryCount($iParent, $sCountry='', $sState=''){
		...
		if (!$GLOBALS['logged']['admin']){ 
			if ($GLOBALS['logged']['member']){ 
			    ...
			    $sExtraCheck = " AND classified.`membership_view_filter` IN ('', '$iMembershipId')"; 
			}else{
				$sExtraCheck = "AND classified.`membership_view_filter`=''";
			}
		}
 
		if($sCountry)
			$sExtraCheck .= " AND classified.`country`='$sCountry'";

		if($sState)
			$sExtraCheck .= " AND classified.`state`='$sState'";

		return  $this->getOne("SELECT count(classified.`id`) FROM `" . $this->_sPrefix . "categ` cat LEFT JOIN `" . $this->_sPrefix . "main` classified ON cat.id=classified.`category_id` WHERE cat.`active` = 1 AND cat.`parent`=$iParent AND classified.`status`='approved' {$sExtraCheck}"); 
	}
	...
	function getCategoryCount($iCategoryId){
		...
		return  $this->getOne("SELECT count(classified.`id`) FROM `" . $this->_sPrefix . "categ` cat LEFT JOIN `" . $this->_sPrefix . "main` classified ON cat.id=classified.`category_id` WHERE cat.`active` = 1 AND cat.`id`=$iCategoryId AND classified.`status`='approved' {$sExtraCheck}"); 
	}
 	
	function SaveCategory($iParent=0)
	{  
		$sCategoryName = process_db_input($_POST['catname']);
		$sCategoryType = process_db_input($_POST['cattype']);
		$sUri = process_pass_data(strtolower($_POST['catname']),1);

		if(!trim($sCategoryName)){
			return false;
		}
		...
		for($iter=1; $iter<=10; $iter++) { 
			if($_POST['custom_field'.$iter])
				$sExtraAdd .= "`custom_field{$iter}`='".process_db_input($_POST['custom_field'.$iter])."',";
		} 
		 
		$sCatURI = uriGenerate ($sUri,  $this->_sPrefix . "categ", 'uri');
 
		$this->query("INSERT INTO `" . $this->_sPrefix . "categ` SET $sExtraAdd `uri`='$sCatURI', `name`='$sCategoryName', `type`='$sCategoryType'");
	...
	{  
		...
		return $this->query("UPDATE `" . $this->_sPrefix . "categ` SET {$sExtraAdd} `name`='$sCategoryName', `type`='$sCategoryType'  WHERE `id`=$iId");
	}
	...
	/*featured functions*/
   function generateFeaturedPaymentUrl($iClassifiedId, $iQuantity, $fCost) {
        ...
        return  $this ->_oConfig->getPurchaseBaseUrl() . '?cmd=_xclick'
                ...
                . '&return=' .  $this->_oConfig->getFeaturedCallbackUrl() . $iClassifiedId
				. '&business='  . getParam('modzzz_classified_paypal_email')
                . '&item_name=' . getParam('modzzz_classified_featured_purchase_desc')
                . '&item_number=' . $iClassifiedId  
                . '&amount=' . $fCost;
    }
 
    function saveFeaturedTransactionRecord($iBuyerId, $iClassifiedId, $iQuantity, $fPrice, $sTransId, $sTransType) {
        $iBuyerId    = (int)$iBuyerId;
        $iClassifiedId  = (int)$iClassifiedId; 
		...
		if($iOrderId){
			$this->alertOnAction('modzzz_classified_featured_admin_notify', $iClassifiedId, $iBuyerId, $iQuantity, true);

			$this->alertOnAction('modzzz_classified_featured_buyer_notify', $iClassifiedId, $iBuyerId, $iQuantity);
		}
	...
	function processClassifieds(){ 
		$this->processExpiredClassifieds();
		
		$this->processExpiredInvoices();

		$this->processFeaturedClassifieds();
	}
	...
	function processExpiredInvoices() { 
		...
		$aListings = $this->getAll("SELECT `classified_id` FROM `" . $this->_sPrefix . "invoices` WHERE `invoice_status`='pending' AND `invoice_expiry_date`>0 AND `invoice_expiry_date`<={$iExpireTime}");
		 
		foreach ($aListings as $aEachListing) {
			$iEntryId = (int)$aEachListing['classified_id']; 
 
			$this->query("DELETE FROM `" . $this->_sPrefix . "main` WHERE `id`=$iEntryId");  
			
			$this->query("DELETE FROM `" . $this->_sPrefix . "invoices` WHERE `classified_id`=$iEntryId");  
 		}   
	}
  
	function processExpiredClassifieds() { 
 
 		$iTime = time();
 		$SECONDS_IN_DAY = 86400; 
	 
        $bFreeListing = (getParam('modzzz_classified_paid_active')=='on') ? false : true;  
        $iNumActiveDays = (int)getParam("modzzz_classified_free_expired");

		$iExpireTime = $iTime - ($SECONDS_IN_DAY * $iNumActiveDays);

        if($bFreeListing && (!$iNumActiveDays))
            return; 
 
		if(getParam('modzzz_classified_activate_expired') == 'on') {
  			 
			if($bFreeListing)
				$aClassifieds = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `created`<=$iExpireTime"); 
			else
				$aClassifieds = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `expiry_date`>0 AND `expiry_date`<=$iTime");
			 
			foreach ($aClassifieds as $aEachClassified) {
				$iEntryId = (int)$aEachClassified['id']; 
				$iRecipientId = (int)$aEachClassified['author_id']; 

				$this->query("UPDATE `" . $this->_sPrefix . "main` SET `status`='expired'  WHERE `id`=$iEntryId");  
 
		 		$this->alertOnAction('modzzz_classified_expired', $iEntryId, $iRecipientId);
			}  
		}else{ 
  			$this->query("UPDATE `" . $this->_sPrefix . "main` SET `status`='expired' WHERE  `status`='approved' AND `expiry_date`>0 AND `expiry_date`<=$iTime");  
		}
  
		$iNumNotifyDays = (int)getParam("modzzz_classified_email_expiring");
		if($iNumNotifyDays){
   
			if($bFreeListing){
				$iTriggerTime = $iExpireTime + ($SECONDS_IN_DAY * $iNumNotifyDays);

				$aClassifieds = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `created`>$iExpireTime AND `created`<=$iTriggerTime AND `pre_expire_notify`=0");
			}else{

				$iTriggerTime = $iTime - ($SECONDS_IN_DAY * $iNumNotifyDays);

				$aClassifieds = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `expiry_date` >= {$iTriggerTime} AND `expiry_date` < {$iTime}  AND `pre_expire_notify`=0");
			}

			foreach ($aClassifieds as $aEachClassified) {
				$iEntryId = $aEachClassified['id'];
				$iRecipientId = $aEachClassified['author_id'];
		 
				$this->query("UPDATE `" . $this->_sPrefix . "main` SET `pre_expire_notify`=1 WHERE `id` = $iEntryId");

				if(getParam("modzzz_classified_activate_expiring")=='on') { 
					$this->alertOnAction('modzzz_classified_expiring', $iEntryId, $iRecipientId, $iNumNotifyDays);
				}
			}
		}
 			
		$iNumNotifyDays = (int)getParam("modzzz_classified_email_expired");
		if($iNumNotifyDays){
			$iTriggerTime = $iTime - ($SECONDS_IN_DAY * $iNumNotifyDays);

			$aClassifieds = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='expired' AND `expiry_date` <= {$iTriggerTime} AND `post_expire_notify`=0");
			...
			foreach ($aClassifieds as $aEachClassified) {
				...
				if(getParam("modzzz_classified_activate_expired")=='on') { 
					$this->alertOnAction('modzzz_classified_post_expired', $iEntryId, $iRecipientId, $iNumNotifyDays);
				}
			}
		}
	...
	function processFeaturedClassifieds(){
        ...
        $aClassifieds = $this->getAll("SELECT `id`, `author_id`, `featured`, `featured_expiry_date` FROM `" . $this->_sPrefix . "main` WHERE `featured`=1 AND `featured_expiry_date` <= $iTime"); 

		foreach($aClassifieds as $aEachList){

		    $iExistExpireDate = (int)$aEachList['featured_expiry_date']; 
		    if($iExistExpireDate==0)
				continue;

			$iClassifiedId = (int)$aEachList['id'];
			$iRecipientId = (int)$aEachList['author_id'];

			$this->alertOnAction('modzzz_classified_featured_expire_notify', $iClassifiedId, $iRecipientId  );
		
	        $this->query("UPDATE `" . $this->_sPrefix . "main` SET `featured`=0, `featured_expiry_date`=0, `featured_date`=0  WHERE `id`=$iClassifiedId"); 
		}

	}

	function alertOnAction($sTemplate, $iClassifiedId, $iRecipientId=0, $iDays=0, $bAdmin=false) {
    ...
    function markSaleCompleted($iId) {
 		...
 		$iEntryId = (int)$this->getOne("SELECT `classified_id` FROM `" . $this->_sPrefix . "offers` WHERE `id` = $iId LIMIT 1");  
	...
	function getLatestComments($iLimit=5){
		...
		return $this->getAll("SELECT `cmt_object_id`,`cmt_author_id`,`cmt_text`, UNIX_TIMESTAMP(`cmt_time`) AS `date` FROM `" . $this->_sPrefix . "cmts` cmt INNER JOIN `" . $this->_sPrefix . "main` m  ON cmt.`cmt_object_id`= m.`id` WHERE m.`allow_view_classified_to` = '". BX_DOL_PG_ALL ."' AND `cmt_text` NOT LIKE '<object%' ORDER BY `cmt_time` DESC LIMIT $iLimit"); 
	}  
