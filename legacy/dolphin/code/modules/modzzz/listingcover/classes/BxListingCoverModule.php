...
function modzzz_listingcover_import ($sClassPostfix, $aModuleOverwright = array()) {
    global $aModule;
    $a = $aModuleOverwright ? $aModuleOverwright : $aModule;
    if (!$a || $a['uri'] != 'listingcover') {
        $oMain = BxDolModule::getInstance('BxListingCoverModule');
        $a = $oMain->_aModule;
    }
 ...
 *
 */
class BxListingCoverModule extends BxDolTwigModule {
 
    var $_aQuickCache = array ();
 
    function BxListingCoverModule(&$aModule) {

        parent::BxDolTwigModule($aModule);        
        $this->_sFilterName = 'listingcover_filter';
        $this->_sPrefix = 'modzzz_listingcover';

        $GLOBALS['oBxListingCoverModule'] = &$this;
    }
  
    function actionHome () {
        parent::_actionHome(_t('_modzzz_listingcover_page_title_home'));
    }
    ...
    function actionAdministration ($sUrl = '') {
        ...
        $aMenu = array( 
            'settings' => array(
                'title' => _t('_modzzz_listingcover_menu_admin_settings'), 
        ...
        echo $this->_oTemplate->adminBlock ($sContent, _t('_modzzz_listingcover_page_title_administration'), $aMenu);
		$this->_oTemplate->addCssAdmin (array('admin.css', 'unit.css', 'twig.css', 'main.css', 'forms_extra.css', 'forms_adv.css'));
       
        $this->_oTemplate->pageCodeAdmin (_t('_modzzz_listingcover_page_title_administration'));
    }

    function actionAdministrationSettings () {
        return parent::_actionAdministrationSettings ('ListingCover');
    }

    function actionAdministrationManage ($isAdminEntries = false) {
        return parent::_actionAdministrationManage ($isAdminEntries, '_modzzz_listingcover_admin_delete', '_modzzz_listingcover_admin_activate');
    }

    // ================================== events
  
	function isPermalinkEnabled() {
		$bEnabled = isset($this->_isPermalinkEnabled) ? $this->_isPermalinkEnabled : ($this->_isPermalinkEnabled = (getParam('permalinks_listingcover') == 'on'));
		 
        return $bEnabled;
    }
 
    function _defineActions () {
        defineMembershipActions(array('listingcover add cover'));
    }

    function _browseMy (&$aProfile) {        
        parent::_browseMy ($aProfile, _t('_modzzz_listingcover_page_title_my_listingcover'));
    } 
	...
	function serviceCoverBlock($aDataEntry) {
		...
		if(getParam("modzzz_listingcover_active")!='on') return;
    	...
    	if(!empty($aProfileThumbnail) && is_array($aProfileThumbnail)) {
 		...
 		if($aDataEntry['icon']){ 
		    $sThumbnail = $oListing->_oDb->getLogo($aDataEntry['id'], $aDataEntry['icon'], true);
		...
		if(!$sThumbnail){  
			$sThumbnail = $oListing->_oTemplate->getImageUrl('no-image-thumb.png');
		}
		...
		$bShowProfileThumbnail = (getParam('modzzz_listingcover_show_administrator')=='on') ? true : false;
	   ...
	   $this->_oTemplate->addCss(array('unit.css','main.css'));
		...
		$sContent = $this->_oTemplate->parseHtmlByName('unit_cover.html', array(
			...
			'bx_if:show_actions' => array(
				...
				'content' => array(
					'href_cover_add' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'cover/add/' . $aDataEntry[$oListing->_oDb->_sFieldId],
					'href_cover_change' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'cover/browse/' . $aDataEntry[$oListing->_oDb->_sFieldUri],
					'href_cover_delete' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'cover/remove/' . $aDataEntry[$oListing->_oDb->_sFieldUri],
				)
			),   
			...
			'bx_if:businesswebsite' => array( 
								'condition' => $aDataEntry['businesswebsite'],
								'content' => array(
			 
              'businesswebsite' =>  $aDataEntry['businesswebsite'],
								), 
							), 
			 
			 'address1' => $aDataEntry['address1'],
			 'zip' => $aDataEntry['zip'],
			 'businessemail' => $aDataEntry['businessemail'],
			 'businesstelephone' => $aDataEntry['businesstelephone'],
			 'city' => $aDataEntry['city'],
			'country' => _t($GLOBALS['aPreValues']['Country'][$aDataEntry['country']]['LKey']),
			'drapeau' => genFlag($aDataEntry['country']),
		  //  'MemberAvatar' => $GLOBALS['oFunctions']->getMemberThumbnail($aDataEntry['author_id'], 'none'),
			 'MemberAvatar' =>  get_member_thumbnail($aDataEntry['author_id'], 'none', true) ,
			// 'MemberAvatar' => $GLOBALS['oFunctions']->getMemberThumbnail($aDataEntry['ID'], 'left', true),
			 //////////////////////////////////////////////////////////////////
    ...
    function isAllowedAdd ($iEntryId, $isPerformAction = false) {
        ...
        // check admin acl
        $this->_defineActions();
		$aCheck = checkAction($this->_iProfileId, BX_LISTINGCOVER_ADD_COVER, $isPerformAction);
    ...
    function isAllowedUseCover ($iProfileId, $isPerformAction = false) {
        ...
        // check admin acl
        $this->_defineActions();
		$aCheck = checkAction($iProfileId, BX_LISTINGCOVER_ADD_COVER, $isPerformAction);
        return ($aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED); 
    }
    ...
    function actionBrowse ($sUri) {
		...
		$sTitle = '_modzzz_listingcover_page_title_cover_browse';

		$this->_oTemplate->pageStart();

        $GLOBALS['oTopMenu']->setCustomSubHeader($aDataEntry[$oModule->_oDb->_sFieldTitle]);
        $GLOBALS['oTopMenu']->setCustomVar('modzzz_listing_view_uri', $aDataEntry[$oModule->_oDb->_sFieldUri]);
        $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
            _t('_modzzz_listing') => BX_DOL_URL_ROOT . $oModule->_oConfig->getBaseUri() . 'home/',
            $aDataEntry[$oModule->_oDb->_sFieldTitle] => BX_DOL_URL_ROOT . $oModule->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$oModule->_oDb->_sFieldUri],
            _t('_modzzz_listingcover_menu_browse_cover') => '',
        ));
		...
		$aButtons = array(
			'action_cover' => '_modzzz_listingcover_action_make_cover',
			'action_delete' => '_modzzz_listingcover_action_delete',
		);
    ...
    function actionDelete ($sUri) {
		...
		$oListingModule = BxDolModule::getInstance('BxListingModule');
		$aListingEntry = $oListingModule->_oDb->getEntryByUri($sUri);
 
        if (!($this->isAdmin() || $oListingModule->isEntryAdmin($aListingEntry))) {
            $this->_oTemplate->displayAccessDenied ();
            return;
        }
		...
		$sRedirectUrl = BX_DOL_URL_ROOT . $oListingModule->_oConfig->getBaseUri() . 'view/' . $aListingEntry['uri'];
		header ('Location:' . $sRedirectUrl);
		exit; 
    }
 
    function actionAdd ($iListingId) {
  
		$oListing = BxDolModule::getInstance('BxListingModule');  
		 
		if (!($aDataEntry = $oListing->_oDb->getEntryById((int)$iListingId))) {
			$this->_oTemplate->displayPageNotFound ();
			return;
		}	

		$sTitle = '_modzzz_listingcover_page_title_cover_add';
 
        if (!($this->isAdmin() || $oListing->isEntryAdmin($aDataEntry))) {
            $this->_oTemplate->displayAccessDenied ();
            return;
        }
        ...
        $GLOBALS['oTopMenu']->setCustomSubHeader($aDataEntry[$oListing->_oDb->_sFieldTitle]);
        $GLOBALS['oTopMenu']->setCustomVar('modzzz_listing_view_uri', $aDataEntry[$oListing->_oDb->_sFieldUri]);
        $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
            _t('_modzzz_listing') => BX_DOL_URL_ROOT . $oListing->_oConfig->getBaseUri() . 'home/',
            $aDataEntry[$oListing->_oDb->_sFieldTitle] => BX_DOL_URL_ROOT . $oListing->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$oListing->_oDb->_sFieldUri],
            _t($sTitle) => '',
        ));
 
        $this->_addCoverForm($iListingId);

        $this->_oTemplate->addJs ('main.js');
        $this->_oTemplate->addCss ('main.css');
        $this->_oTemplate->addCss ('forms_extra.css');
        $this->_oTemplate->pageCode(_t($sTitle) .' - '. $aDataEntry[$oListing->_oDb->_sFieldTitle]);  
    }
 
    function _addCoverForm ($iListingId) { 
 
 		$oListing = BxDolModule::getInstance('BxListingModule');  
		$aListingEntry = $oListing->_oDb->getEntryById((int)$iListingId);

        bx_import ('FormAdd', $this->_aModule);
        $sClass = $this->_aModule['class_prefix'] . 'FormAdd';
        $oForm = new $sClass ($this, $iListingId);
        $oForm->initChecker();
        ...
        if ($oForm->isSubmittedAndValid ()) {
            ...
            if ($iEntryId) {
				...
				if(!$aListingEntry['cover_id']){
					$this->_oDb->activateCover($aDataEntry['listing_id'], $aDataEntry['thumb']);
				}

				$this->onEventCreate ($iEntryId, $sStatus, $aDataEntry);

				$sRedirectUrl = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'cover/browse/' . $aListingEntry[$oListing->_oDb->_sFieldUri];
	...
	function isListingAdmin ($aDataEntry, $iProfileId=0) {

		$oListing = BxDolModule::getInstance('BxListingModule');  
		$aListingEntry = $oListing->_oDb->getEntryById((int)$aDataEntry['listing_id']);
 
		return ($oListing->isEntryAdmin($aListingEntry, $iProfileId));
	}
    ...
    function isAllowedDelete (&$aDataEntry, $isPerformAction = false) {
		...
		return $this->isListingAdmin($aDataEntry);
    }   
