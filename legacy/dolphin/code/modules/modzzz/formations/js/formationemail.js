function toggleCompanyEmailBlock(sValue){
	if(sValue=='-99'){ 
		$('#company_block').parent().show(); 
		$('#company_email').parent().parent().parent().parent().show(); 
	}else{
		$('#company_block').parent().hide(); 
		$('#company_email').parent().parent().parent().parent().hide();  
	}
}

function formationFormInit(){
...
window.onload = formationFormInit;
