...
class BxFormationsModule extends BxDolTwigModule {
    ...
    function actionPopupLetter ($iId) {
		...
		$aVarsPopup = array (
            'title' => _t('_modzzz_formations_cover_letter'),
            'content' => $sContent,
        );        
    ...
    function _actionApplications ($sUri, $iPerPage, $sUrlBrowse, $sTitle) {
        ...
        foreach ($aProfiles as $aEachApplicant) { 
			...
			if($aEachApplicant['resume']){
				$sDownloadLink = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'download_resume/' . $aEachApplicant['formation_id'] .'/'. $aEachApplicant['resume'];
	  ...
	  'condition' => !in_array($iId, $aFriendsIds) && !isFriendRequest($iUserId, $iId) &&  !$this->isEntryAdmin($aData),
		...
		// [END] Freddy ajout Befirend
			...
			//////////////////////////////////////////
				...
				// Freddy modif 'posted' => date("M d, Y j:s A",$aEachApplicant['created']),
				'posted' => date("d/m/Y H:i ",$aEachApplicant['created']),
			...
			$sRet .= $this->_oTemplate->parseHtmlByName('entry_view_each_applicant', $aVars);   
        }
    ...
    function _actionApply ($iEntryId, $sTitle, $sMsgSent) {
        ...
        if ($oForm->isSubmittedAndValid ()) {
			...
			$this->_oDb->saveApplication($this->_iProfileId, $iEntryId, $sMessage, $sResume, $iResumeId); 
			...
			$sDownloadLink = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'download_resume/' . $iEntryId .'/'. $sResume;
			$sAttachInboxMessage = ($sResume) ? _t('_modzzz_formations_download_resume_online_at', $sDownloadLink) : '';

			if(getParam('modzzz_resume_formation_connect')=='on'){

				if($iResumeId){
					$sAttachMessage = ($sAttachMessage) ? $sAttachMessage.'<br><br>' : '';
					$sAttachInboxMessage = ($sAttachInboxMessage) ? $sAttachInboxMessage.'<br><br>' : '';
					...
					$sAttachInboxMessage .= _t('_modzzz_formations_view_resume_online_at') . $sResumeLink;
				} 
			}
            ...
            $aTemplateVars = array (
                ...
                'AttachInboxMessage' => $sAttachInboxMessage, 
			...
			$this->broadCastToEmployee($iRecipient, $sApplicationTitle, $aTemplateVars);  
     ...
     echo $this->_oTemplate->parseHtmlByName('view_profile_status_link.html',array('uid' => $this->_iProfileId)); 
    ...
    function actionDownloadResume ($iEntryId, $sResume='') {
 
		$aDataEntry = $this->_oDb->getEntryById((int)$iEntryId);
        ...
        $sPath = $this->_oConfig->getResumePath() . $sResume;
        $aPathInfo = pathinfo ($sPath);

        if ($sResume=='' || !file_exists ($sPath)) {
            $this->_oTemplate->displayPageNotFound ();
            exit;
        }
 
        header ("Cache-Control: must-revalidate, post-check=0, pre-check=0");
        header ("Content-type: " . $aPathInfo['mime_type']);
	...
	function broadCastToEmployee($iRecipient, $sApplicationTitle, $aTemplateVars){
		...
		$aTemplate = $oEmailTemplate->getTemplate($this->_sPrefix . '_apply_inbox'); 

		$aMailBoxSettings = array
		(
			'member_id'	 =>  $this->_iProfileId, 
			'recipient_id'	 => $iRecipient, 
			'messages_types'	 =>  'mail',  
		);
		...
		$sMessageBody = $aTemplate['Body'];
		foreach($aTemplateVars as $sKey=>$sValue) 
			$sMessageBody = str_replace("<{$sKey}>", $sValue, $sMessageBody);
  
		$oMailBox -> iWaitMinutes = 0;//turn off anti-spam
		$oMailBox -> sendMessage($sApplicationTitle, $sMessageBody, $iRecipient, $aComposeSettings);  
    }
   ...
   // Freddy Manage Applications
    ...
    function _actionUploadIcon ($iCompanyId=0) {
		...
		if ( 0 < $_FILES['company_icon']['size'] && 0 < strlen( $_FILES['company_icon']['name'] ) ) {
			$sFileName = time();
			$sExt = $this->moveUploadedImage( $_FILES, 'company_icon', $sPath . $sFileName, '', false );
			if( strlen( $sExt ) && !(int)$sExt ) {
