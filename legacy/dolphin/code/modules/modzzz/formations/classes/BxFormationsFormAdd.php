...
class BxFormationsFormAdd extends BxDolFormMedia {

    var $_oMain, $_oDb;

    function BxFormationsFormAdd ($oMain, $iProfileId, $iEntryId = 0, $iThumb = 0) {
		...
		//if(getParam('modzzz_schools_educations_active')=='on')
			...
			$sBusinessEmail = $this->_oDb->getBusinessEmail($aProfileInfoBiz['ID']);
			////////////////////////////////////////////////// 
		...
		$aSubCategory = array();
   		...
   		$bPaidFormation = $this->_oMain->isAllowedPaidFormation (); 
 
		$bListing = false;

		if($iEntryId) {
			$aDataEntry = $this->_oDb->getEntryById($iEntryId);
			$bPaidFormation = $aDataEntry['invoice_no'] ? $bPaidFormation : false;
			...
			if($aDataEntry['company_id']){ 
				...
				// freddy Business remplacé par school
		//if(getParam("modzzz_listing_formations_active")=='on' && $aDataEntry['company_type']=='listing'){ 
			...
			//$sSelCountry = ($_POST['country']) ? $_POST['country'] : getParam('modzzz_formations_default_country'); 
		...
		$sCatUrl = BX_DOL_URL_ROOT . $oMain->_oConfig->getBaseUri() . 'home/'.(getParam('modzzz_formations_permalinks') ? '?' : '&').'ajax=cat&parent=' ;
 
		$sStateUrl = BX_DOL_URL_ROOT . $oMain->_oConfig->getBaseUri() . 'home/'.(getParam('modzzz_formations_permalinks') ? '?' : '&').'ajax=state&country=' ; 
        ...
        if (BxDolRequest::serviceExists('photos', 'perform_photo_upload', 'Uploader'))
            $this->_aMedia['images'] = array (
                ...
                'tag' => BX_FORMATIONS_PHOTOS_TAG,
                'cat' => BX_FORMATIONS_PHOTOS_CAT,
                'thumb' => 'thumb',
                'module' => 'photos',
                'title_upload_post' => 'images_titles',
                'title_upload' => _t('_modzzz_formations_form_caption_file_title'),
        ...
        if (BxDolRequest::serviceExists('videos', 'perform_video_upload', 'Uploader'))
            $this->_aMedia['videos'] = array (
                ...
                'tag' => BX_FORMATIONS_VIDEOS_TAG,
                'cat' => BX_FORMATIONS_VIDEOS_CAT,
                'thumb' => false,
                'module' => 'videos',
        ...
        if (BxDolRequest::serviceExists('sounds', 'perform_music_upload', 'Uploader'))
            $this->_aMedia['sounds'] = array (
                ...
                'tag' => BX_FORMATIONS_SOUNDS_TAG,
                'cat' => BX_FORMATIONS_SOUNDS_CAT,
                'thumb' => false,
                'module' => 'sounds',
        ...
        if (BxDolRequest::serviceExists('files', 'perform_file_upload', 'Uploader'))
            $this->_aMedia['files'] = array (
                ...
                'tag' => BX_FORMATIONS_FILES_TAG,
                'cat' => BX_FORMATIONS_FILES_CAT,
                'thumb' => false,
                'module' => 'files',
		...
		$aExperienceList = $oProfileFields->convertValues4Input('#!FormationExperience');
        ksort($aExperienceList);
        
		$aQualificationList = $oProfileFields->convertValues4Input('#!FormationEducation');
        ksort($aQualificationList);
        
		$aCareerLevelList = $oProfileFields->convertValues4Input('#!FormationCareerLevel');
        ksort($aCareerLevelList);
        
		$aSalaryTypeList = $oProfileFields->convertValues4Input('#!FormationSalaryType');
        ksort($aSalaryTypeList);

		
		
		$aCurrencyList = $oProfileFields->convertValues4Input('#!FormationCurrency');
       // asort($aCurrencyList);
		...
		$aFormationTypeList = $oProfileFields->convertValues4Input('#!FormationType');
        ksort($aFormationTypeList);

		$aFormationStatusList = $oProfileFields->convertValues4Input('#!FormationStatus');
        ksort($aFormationStatusList);
        ...
        $aInputPrivacyViewFans = $this->_oMain->_oPrivacy->getGroupChooser($iProfileId, 'formations', 'view_fans');
        ...
        $aInputPrivacyForum = $this->_oMain->_oPrivacy->getGroupChooser($iProfileId, 'formations', 'post_in_forum');
        ...
        $aInputPrivacyUploadPhotos = $this->_oMain->_oPrivacy->getGroupChooser($iProfileId, 'formations', 'upload_photos');
        $aInputPrivacyUploadPhotos['values'] = $aInputPrivacyCustom2;
        $aInputPrivacyUploadPhotos['db'] = $aInputPrivacyCustom2Pass;

        $aInputPrivacyUploadVideos = $this->_oMain->_oPrivacy->getGroupChooser($iProfileId, 'formations', 'upload_videos');
        $aInputPrivacyUploadVideos['values'] = $aInputPrivacyCustom2;
        $aInputPrivacyUploadVideos['db'] = $aInputPrivacyCustom2Pass;        

        $aInputPrivacyUploadSounds = $this->_oMain->_oPrivacy->getGroupChooser($iProfileId, 'formations', 'upload_sounds');
        $aInputPrivacyUploadSounds['values'] = $aInputPrivacyCustom2;
        $aInputPrivacyUploadSounds['db'] = $aInputPrivacyCustom2Pass;

        $aInputPrivacyUploadFiles = $this->_oMain->_oPrivacy->getGroupChooser($iProfileId, 'formations', 'upload_files');
        $aInputPrivacyUploadFiles['values'] = $aInputPrivacyCustom2;
        $aInputPrivacyUploadFiles['db'] = $aInputPrivacyCustom2Pass;
 
		if(!$GLOBALS['oBxFormationsModule']->isValidAccess(0, $iProfileId, 'post')){ 
			...
			if(getParam('modzzz_formations_post_credits')=='on'){ 
				...
				$iPostCost = (int)$oCredit->_oDb->getActionValue($iProfileId, 'modzzz_formations', 'post');
				...
				$iPostCost = (int)$oPoint->_oDb->getActionValue($iProfileId, 'modzzz_formations', 'post');
			}
        ...
        $aCustomForm = array(
            ...
            'inputs' => array(
                ...
                'header_info' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_info')
                ), 
                'display_business' => array(
                    'type' => 'custom',
                    'name' => 'display_business',
                    'caption' => _t('_modzzz_formations_form_caption_listed_by'),
				...
				'package_name' => array( 
                    ...
                    'name' => 'package_name',
                    'caption' => _t('_modzzz_formations_package'), 
                ),  
                'post_type' => array(
                    'type' => 'select',
                    'name' => 'post_type',
				...
				/*
				 'header_select_company' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_select_company'), 
                    'collapsable' => false,
                    'collapsed' => false,
                ),
				*/ 
                'company_id' => array(
                    'type' => 'select',
                    'name' => 'company_id',
                    'caption' => _t('_modzzz_formations_form_caption_company'),
                    'info' => _t('_modzzz_formations_form_info_company'),
					'attrs' => array(
                    ...
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_company'),
                    ),
                    'values' => $aCompanies,   
                ), 
                ...
                'title' => array(
                    'type' => 'text',
                    'name' => 'title',
                    'caption' => _t('_modzzz_formations_form_caption_title'),
					'info' => _t('_modzzz_formations_form_info_title'),
                    'required' => true,
                    'checker' => array (
                        'func' => 'length',
                        'params' => array(3,150),
                        'error' => _t ('_modzzz_formations_form_err_title'),
                    ),
                    'db' => array (
                        'pass' => 'Xss', 
                    ) 
				 ...
				 'role' => array(
                    'type' => 'text',
                    'name' => 'role', 
                    'caption' => _t('_modzzz_formations_form_caption_role'), 
					'info'  => _t('_modzzz_formations_form_info__date_debut'),
					
                    'required' => true,
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_date_debut'),
                    ),
				 ...
				 'duration' => array(
                    'type' => 'text',
                    'name' => 'duration',
                    'caption' => _t('_modzzz_formations_form_caption_duration'),
					'info'  => _t('_modzzz_formations_form_info__duration'),
                    'required' => true,
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_duration'),
                    ),
                    'db' => array (
                        'pass' => 'Xss', 
                    ),
				  ...
				  'formation_type' => array(
                    'type' => 'select_box',
                    'name' => 'formation_type',
                    'listname' => 'FormationType',
					'values'=> $aFormationTypeList,
                    'caption' => _t('_modzzz_formations_form_caption_formation_type'), 
					 'info'  => _t('_modzzz_formations_form_info_education_type'),
					 'attrs' => array(
					'style' => 'width:240px',
					),
                    ...
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_formation_type'),
                    ),
                    'db' => array (
                      //  'pass' => 'Int', 
					  'pass' => 'Categories', 
				 ...
				 'parent_category_id' => array(
					...
					'info' => _t('_modzzz_formations_form_info_category'),
					'attrs' => array(
                    ...
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_category'),
                    ),
					'db' => array (
                        'pass' => 'Int', 
                    ),			
                ...
                'category_id' => array(
				 ...
				 'header_location' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_location'), 
                    'collapsable' => true,
                    'collapsed' => false,
                ),
                'country' => array(
                   ...
                   'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_country'),
                    ),   
 					'value' => $sSelCountry,
					'attrs' => array(
 				...
 				'state' => array(
                    ...
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_state'),
                    ), 
					'db' => array (
						'pass' => 'Preg', 
						'params' => array('/([a-zA-Z]+)/'),
 	            ...
 	            'city' => array(
                    ...
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_city'),
                    ), 
                    'db' => array (
                        'pass' => 'Xss', 
                    ),
               ...
               /*
			    'zip' => array(
                    ...
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_zip'),
                    ), 
                    'db' => array (
                        'pass' => 'Xss', 
                    ),
				...
				*/
				 ...
				 'desc' => array(
                    ...
                    'checker' => array (
                        'func' => 'length',
                        'params' => array(100,64000),
                        'error' => _t ('_modzzz_formations_form_err_desc'),
                    ),                    
                    'db' => array (
                        'pass' => 'XssHtml', 
                    ),                    
				 ...
				 'skills' => array(
                    'type' => 'textarea',
                    'name' => 'skills',
                    'caption' => _t('_modzzz_formations_form_caption_skills'),
                    'required' => false,
                    'html' => 2, 
                    'db' => array (
                        'pass' => 'XssHtml', 
				 ...
				 // Freddy --- Conditions d'admission
				  'condition_admission' => array(
                    'type' => 'textarea',
                    'name' => 'condition_admission',
                    'caption' => _t('_modzzz_formations_form_caption_admission'),
                    'required' => false,
                    'html' => 2, 
                    'db' => array (
                        'pass' => 'XssHtml', 
				...
				/* 'experience' => array(
                    'type' => 'select',
                    'name' => 'experience',
                    'listname' => 'FormationExperience',
					'values'=> $aExperienceList,
                    'caption' => _t('_modzzz_formations_form_caption_experience'), 
				
					
                     'required' => true,
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_experience'),
                    ),
                    'db' => array (
                        'pass' => 'Int', 
                    ),
                ...
                'qualification' => array(
                    'type' => 'select',
                    'name' => 'qualification',
                    'listname' => 'FormationEducation',
					'values'=> $aQualificationList,
                    'caption' => _t('_modzzz_formations_form_caption_qualification'), 
					
					
                     'required' => true,
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_qualification'),
                    ),
                    'db' => array (
                        'pass' => 'Int', 
                    ),
                ...
                'formation_status' => array(
                    'type' => 'select',
                    'name' => 'formation_status',
                    'listname' => 'FormationStatus',
					'values'=> $aFormationStatusList,
                    'caption' => _t('_modzzz_formations_form_caption_formation_status'), 
                    'required' => true, 
					 'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_formation_status'),
                    ),
                    'db' => array (
                        'pass' => 'Int', 
                    ),
				 ...
				 'career_level' => array(
                    ...
                    'caption' => _t('_modzzz_formations_form_caption_career_level'), 
                    'required' => false, 
                    'db' => array (
                        'pass' => 'Int', 
                    ),
				 ...
				 'vacancies' => array(
                    'type' => 'text',
                    'name' => 'vacancies', 
                    'caption' => _t('_modzzz_formations_form_caption_vacancies'), 
                    'required' => false, 
                    'db' => array (
                        'pass' => 'Int', 
                    ),
				 ...
				 'header_select_salary' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_salary'), 
                    'collapsable' => false,
                    'collapsed' => false,
                ),
				  ...
				  'currency' => array(
					...
					'value'=> $sSelCurrency, 
                    'caption' => _t('_modzzz_formations_form_caption_currency'), 
                    'required' => false, 
                    'db' => array (
                        'pass' => 'Xss', 
                    ),
				  ...
				  'min_salary' => array(
                    'type' => 'text',
                    'name' => 'min_salary', 
                    'caption' => _t('_modzzz_formations_form_caption_min_salary'), 
                    'required' => false, 
                    'db' => array (
                        'pass' => 'Int', 
                    ),
                ...
                'max_salary' => array(
                    'type' => 'text',
                    'name' => 'max_salary', 
                    'caption' => _t('_modzzz_formations_form_caption_max_salary'), 
                    'required' => false, 
                    'db' => array (
                        'pass' => 'Int', 
                    ),
                ...
                'salary_type' => array(
                    ...
                    'caption' => _t('_modzzz_formations_form_caption_salary_type'), 
                    'required' => false, 
                    'db' => array (
                        'pass' => 'Int', 
                    ), 
               ...
               */
			    ...
			    'tags' => array(
                    ...
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_tags'),
                    ),
                    'db' => array (
                        'pass' => 'Tags', 
                    ),
               ...
               'role' => array(
				 ...
				 'header_application' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_application'), 
                    'collapsable' => false,
                    'collapsed' => false,
                ),
				*/
				 ...
				 'application_link' => array(
                    ...
                    'caption' => _t('_modzzz_formations_form_caption_application_link'), 
                    'info' => _t('_modzzz_formations_form_info_application_link'), 
                    'required' => false, 
                    'db' => array (
                        'pass' => 'Xss', 
                    ),
                ...
                'header_company' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_company'),
					'attrs' => array(
						'id' => "company_block",
					),	 
                    'collapsable' => true,
             ...
             /*
			    'header_logo' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_logo'),
                    'collapsable' => true,
                    'collapsed' => false,
                ),
				'presenticon' => array(
				...
				'iconfile' => array(
					'type' => 'file',
					'name' => "iconfile",
					'caption' => _t('_modzzz_formations_form_caption_icon'), 
				),
               ...
               /*
			    'header_images' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_images'),
                    'collapsable' => true,
                    'collapsed' => false,
                ),
				*/
                'thumb' => array(
                    ...
                    'name' => 'thumb',
                    'caption' => _t('_modzzz_formations_form_caption_thumb_choice'),
                    'info' => _t('_modzzz_formations_form_info_thumb_choice'),
                    'required' => false,
                    'db' => array (
                        'pass' => 'Int',
                    ),
                ...
                'images_choice' => array(
                    ...
                    'name' => 'images_choice[]',
                    'caption' => _t('_modzzz_formations_form_caption_images_choice'),
                    'info' => _t('_modzzz_formations_form_info_images_choice'),
                    'required' => false,
                ),
                'images_upload' => array(
                    'type' => 'custom',
                    ...
                    'name' => 'images_upload[]',
                    'caption' => _t('_modzzz_formations_form_caption_images_upload'),
                   // 'info' => _t('_modzzz_formations_form_info_images_upload'),
                    'required' => false,
                ),
               ...
               /* 'header_files' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_files'),
                    'collapsable' => true,
                    'collapsed' => false,
                ),
				*/
                'files_choice' => array(
                    ...
                    'name' => 'files_choice[]',
                    'caption' => _t('_modzzz_formations_form_caption_files_choice'),
                    'info' => _t('_modzzz_formations_form_info_files_choice'),
                    'required' => false,
                ),
                'files_upload' => array(
                    'type' => 'custom',
                    ...
                    'name' => 'files_upload[]',
                    'caption' => _t('_modzzz_formations_form_caption_files_upload'),
                   // 'info' => _t('_modzzz_formations_form_info_files_upload'),
                    'required' => false,
                ),
             ...
             /*
			  'header_youtube' => array(
                   'type' => 'block_header',
                   'caption' => _t('_modzzz_formations_form_header_youtube'),
                   'collapsable' => true,
                   'collapsed' => false,
               ), 
			   */
               ...
               'youtube_choice' => array(
                   ...
                   'name' => 'youtube_choice[]',
                   'caption' => _t('_modzzz_formations_form_caption_youtube_choice'),
                   'info' => _t('_modzzz_formations_form_info_youtube_choice'),
                   'required' => false,
               ), 
               'youtube_attach' => array(
                   ...
                   'name' => 'youtube_upload[]',
                   'caption' => _t('_modzzz_formations_form_caption_youtube_attach'),
                   'info' => _t('_modzzz_formations_form_info_youtube_attach'),
                   'required' => false,
               ),
                ...
                'header_videos' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_videos'),
                    'collapsable' => true,
                    'collapsed' => false,
                ),
                'videos_choice' => array(
                    ...
                    'name' => 'videos_choice[]',
                    'caption' => _t('_modzzz_formations_form_caption_videos_choice'),
                    'info' => _t('_modzzz_formations_form_info_videos_choice'),
                    'required' => false,
                ),
                'videos_upload' => array(
                    'type' => 'custom',
                    ...
                    'name' => 'videos_upload[]',
                    'caption' => _t('_modzzz_formations_form_caption_videos_upload'),
                    'info' => _t('_modzzz_formations_form_info_videos_upload'),
                    'required' => false,
                ),
                ...
                'header_sounds' => array(
                    'type' => 'block_header',
                    'caption' => _t('_modzzz_formations_form_header_sounds'),
                    'collapsable' => true,
                    'collapsed' => false,
                ),
                'sounds_choice' => array(
                    ...
                    'name' => 'sounds_choice[]',
                    'caption' => _t('_modzzz_formations_form_caption_sounds_choice'),
                    'info' => _t('_modzzz_formations_form_info_sounds_choice'),
                    'required' => false,
                ),
                'sounds_upload' => array(
                    'type' => 'custom',
                    ...
                    'name' => 'sounds_upload[]',
                    'caption' => _t('_modzzz_formations_form_caption_sounds_upload'),
                    'info' => _t('_modzzz_formations_form_info_sounds_upload'),
                    'required' => false,
                ),
              ...
              // 'allow_upload_sounds_to' => $aInputPrivacyUploadSounds, 
               ...
               //'allow_upload_files_to' => $aInputPrivacyUploadFiles, 
                ...
                'allow_view_formation_to' => $this->_oMain->_oPrivacy->getGroupChooser($iProfileId, 'formations', 'view_formation'),
              ...
              /// 'allow_post_in_forum_to' => $aInputPrivacyForum, 
                ...
                'allow_join_to' => $this->_oMain->_oPrivacy->getGroupChooser($iProfileId, 'formations', 'join'),  
		...
		if(getParam('modzzz_formations_seeker_activated') != 'on'){ 
       ...
       if (!isset($this->_aMedia['images'])) {
        ...
        if ($GLOBALS['oBxFormationsModule']->isAdmin()) {

            require_once(BX_DIRECTORY_PATH_INC . 'membership_levels.inc.php');
            $aMemberships = getMemberships ();
            unset ($aMemberships[MEMBERSHIP_ID_NON_MEMBER]); // unset Non-member
            $aMemberships = array('' => _t('_modzzz_formations_membership_filter_none')) + $aMemberships;
            $aFormInputsAdminPart = array (
 
				'membership_view_filter' => array(
					'type' => 'select',
					'name' => 'membership_view_filter',
					'caption' => _t('_modzzz_formations_caption_membership_view_filter'), 
					'info' => _t('_modzzz_formations_info_membership_view_filter'), 
					'values' => $aMemberships,
					'value' => '', 
					'checker' => array (
					'func' => 'preg',
					'params' => array('/^[0-9a-zA-Z]*$/'),
					'error' => _t ('_modzzz_formations_err_membership_view_filter'),
					),                                        
					'db' => array (
					'pass' => 'Preg', 
					'params' => array('/([0-9a-zA-Z]*)/'),
		...
		// Freddy   --- Masquer submit si membre n'a pas encore creer une page entreprise// Business listing
			if(count($aCompanies)==0){
				  ...
				  unset ($aCustomForm['inputs'], $aFormInputsAdminPart);
				   unset ($aCustomForm['inputs'], $aFormInputsSubmit); 
   ...
   function processMembershipChecksForMediaUploads (&$aInputs) {
        ...
        defineMembershipActions (array('photos add', 'sounds add', 'videos add', 'files add', 'formations photos add', 'formations sounds add', 'formations videos add', 'formations files add', 'formations allow embed'));
        ...
        $a = array ('images' => 'PHOTOS', 'videos' => 'VIDEOS', 'sounds' => 'SOUNDS', 'files' => 'FILES');
        foreach ($a as $k => $v) {
			if (defined("BX_FORMATIONS_{$v}_ADD"))
				$aCheck = checkAction($_COOKIE['memberID'], constant("BX_FORMATIONS_{$v}_ADD"));
            if ((!defined("BX_FORMATIONS_{$v}_ADD") || $aCheck[CHECK_ACTION_RESULT] != CHECK_ACTION_RESULT_ALLOWED) && !$isAdmin) {
                unset($this->_aMedia[$k]);
                unset($aInputs['header_'.$k]);
                unset($aInputs[$k.'_choice']);
                unset($aInputs[$k.'_upload']);  
		...
		$aCheck = checkAction($_COOKIE['memberID'],  BX_FORMATIONS_ALLOW_EMBED);
