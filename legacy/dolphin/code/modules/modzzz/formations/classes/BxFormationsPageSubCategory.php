...
class BxFormationsPageSubCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxFormationsPageSubCategory(&$oMain, $sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_formations_subcategory', $oMain);
    ...
    function getBlockCode_SubCategoryFormations() {

		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'subcategories/'.$this->aCategoryInfo['uri'].'?';
 
        $sCode = $this->ajaxBrowse('subcategories', $this->oDb->getParam('modzzz_formations_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
