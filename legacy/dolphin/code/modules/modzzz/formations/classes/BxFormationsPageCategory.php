...
class BxFormationsPageCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxFormationsPageCategory(&$oMain,$sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_formations_category', $oMain);
		...
		if(isset($_GET['ajax']) && ($_GET['ajax']=='sort')){
			echo $this->getBlockCode_CategoryFormations(); 
			exit;
		} 
    ...
    function getSubCategories() { 
	    ...
	    return $this->oTemplate->parseHtmlByName('formations_subcategories', $aVars);  
	}
 
    function getBlockCode_CategoryFormations() {

		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories/'.$this->aCategoryInfo['uri'].'?';
  
        $sCode = $this->ajaxBrowse('categories', $this->oDb->getParam('modzzz_formations_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
