...
class BxFormationsPageView extends BxDolTwigPageView {	

	function BxFormationsPageView(&$oMain, &$aDataEntry) {
		parent::BxDolTwigPageView('modzzz_formations_view', $oMain, $aDataEntry);
	...
	function getBlockCode_Company() {
		...
		//Freddy mise en commenatire pour afficher le bloc company pour ceux qui n ont pas poster le formation en tant qu individuel
		//if(!$aData['company_id']) return;
			  
		if($aData['company_type']=='formation'){
	...
	/*
		...
		*/
		...
		if($aData['vacancies']=='1'){
			$aVacancies = _t('_modzzz_formations_vacancies_single') ; 
		}
		else{
			$aVacancies = _t('_modzzz_formations_vacancies') ; 
		}
      ...
      // Freddy pour afficher les candidatures que lorsque le count est superier à 10
       /* $application_count= $this->_oDb->getApplicationsCount($aData['id']);
		...
		if ( $application_count >='10') {
			$aApplicant_be_the_first_or_not = '<strong>'. $application_count .'</strong>'. ' ' ._t('_modzzz_formations_form_unit_view_applications');
		}
		else {
			$aApplicant_be_the_first_or_not= _t('_modzzz_formations_Be_one_of_the_first_ten_applicants');
		}
		*/
		...
		$application_count= $this->_oDb->getApplicationsCount($aData['id']);
		  ...
		  else{ $smessage_applicant= '<i class="sys-icon user"></i>'.' '._t('_modzzz_formations_Be_one_of_the_first_ten_applicants');
		  }
		 ...
		 // freddy pour ne pas afficher le bouton si le membre a deja postuler
		  ...
		  $aAPPLY =  _t('_modzzz_formations_easy_apply');
	    ...
	    $aVars = array (
		 'AddToFavorite' => $this->_oMain->isAllowedMarkAsFavorite($aData) ? ($this->_oMain->isFavorite($this->_oMain->_iProfileId, $aData['id']) ? _t('_modzzz_formations_action_remove_from_favorite') : _t('_modzzz_formations_action_add_to_favorite')) : '',
		...
		'FullName'=> $sAuthorFirst_Name_Last_Name,
		 ...
		 // 'Applicant_be_the_first_or_not' => $aApplicant_be_the_first_or_not,
			...
			'bx_if:DisplayApply' => array( 
				...
				'content' => array(
		         'applyLink' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'apply/' . $aData['id'],
				 'message_title' => _t('_modzzz_formations_candidater') ? _t('_modzzz_formations_candidater') : '',
		 ...
		 'title' => $aData['title'],
		   'city' => $aData['city'],
			...
			'vacancies_caption' => $aVacancies,
			 ...
			 // freddy stage or formation seeker or provider
		/*
		 'bx_if:post_type_stage' => array( 
								'condition' =>($aData['post_type']=='seeker'),
								'content' => array(
             'post_type_stage' => _t('_modzzz_formations_post_type_stage'), 
			  'formation_status' => $aFormationStatus,
			 'formation_type' => $aFormationType,
								), 
							),
							
			 'bx_if:post_type_formation' => array( 
								'condition' =>($aData['post_type']=='provider'),
								'content' => array(
              'post_type_formation' => _t('_modzzz_formations_post_type_formation'),  
			  'formation_status' => $aFormationStatus,
			 'formation_type' => $aFormationType,
								), 
							),
			...
			'bx_if:career_level' => array( 
								...
								'content' => array(
             'career_level' => _t($GLOBALS['aPreValues']['FormationCareerLevel'][$aData['career_level']]['LKey']),
								), 
							),
    ...
    function _blockInfo ($aData) {
		...
		// [END] Freddy ajout Befirend
		 ...
		 // [END] Freddy Contact formation poster
		 'bx_if:contacter_url' => array(
			...
			'bx_if:featured' => array( 
				...
				'content' => array(    
					'featured' => $aData['featured_expiry_date'] ? _t('_modzzz_formations_featured_until') .' '. $this->_oTemplate->filterCustomDate($aData['featured_expiry_date']) : _t('_modzzz_formations_featured_formations'),  
				),  		
			), 
        );
    ...
    function getBlockCode_Rate() {
        modzzz_formations_import('Voting');
        $o = new BxFormationsVoting ('modzzz_formations', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRate($this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_formations_import('Cmts');
        $o = new BxFormationsCmts ('modzzz_formations', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $aSubscribeButton = $oSubscription->getButton($this->_oMain->_iProfileId, 'modzzz_formations', '', (int)$this->aDataEntry['id']);
            ...
            $aInfo = array (
                ...
                'TitleApply' => $this->_oMain->isAllowedApply($this->aDataEntry) ? _t('_modzzz_formations_action_title_apply') : '',
				'TitleEdit' => $this->_oMain->isAllowedEdit($this->aDataEntry) ? _t('_modzzz_formations_action_title_edit') : '',
                'TitleDelete' => $this->_oMain->isAllowedDelete($this->aDataEntry) ? _t('_modzzz_formations_action_title_delete') : '',
				'TitleInvite' => $this->_oMain->isAllowedSendInvitation($this->aDataEntry) ? _t('_modzzz_formations_action_title_invite') : '',
                'TitleShare' => $this->_oMain->isAllowedShare($this->aDataEntry) ? _t('_modzzz_formations_action_title_share') : '',
                'TitleJoin' => $this->_oMain->isAllowedJoin($this->aDataEntry) ? ($isFan ? _t('_modzzz_formations_action_title_leave') : _t('_modzzz_formations_action_title_join')) : '',
                'IconJoin' => $isFan ? 'signout' : 'signin',
				'TitleBroadcast' => $this->_oMain->isAllowedBroadcast($this->aDataEntry) ? _t('_modzzz_formations_action_title_broadcast') : '',
 				
				//Freddy Manage Applications 08 juillet 2015
				'TitleManageApplications' => $this->_oMain->isAlloweManageApplications($this->aDataEntry) ? _t('_modzzz_formations_action_title_manage_applicants') : '',
				
				'TitleBroadcastApplicants' => $this->_oMain->isAllowedBroadcast($this->aDataEntry) ? _t('_modzzz_formations_action_title_broadcast_applicants') : '',
				'AddToFeatured' => $this->_oMain->isAllowedMarkAsFeatured($this->aDataEntry) ? ($this->aDataEntry['featured'] ? _t('_modzzz_formations_action_remove_from_featured') : _t('_modzzz_formations_action_add_to_featured')) : '',
				'TitlePurchaseFeatured' => '',
				'AddToFavorite' => $this->_oMain->isAllowedMarkAsFavorite($this->aDataEntry) ? ($this->_oMain->isFavorite($this->_oMain->_iProfileId, $this->aDataEntry['id']) ? _t('_modzzz_formations_action_remove_from_favorite') : _t('_modzzz_formations_action_add_to_favorite')) : '',
				'TitlePurchaseFeatured' => '',
				/*'TitlePurchaseFeatured' => $this->_oMain->isAllowedPurchaseFeatured($this->aDataEntry) ? ($this->aDataEntry['featured'] ? _t('_modzzz_formations_action_title_extend_featured') : _t('_modzzz_formations_action_title_purchase_featured')) : '',*/
                'TitleRelist' => $this->_oMain->isAllowedRelist($this->aDataEntry) ? _t('_modzzz_formations_action_title_relist') : '',
                'TitleExtend' => $this->_oMain->isPaidListing($this->aDataEntry['id']) ? ($this->_oMain->isAllowedExtend($this->aDataEntry) ? _t('_modzzz_formations_action_title_extend') : '') : '',
                'TitlePremium' => $this->_oMain->isAllowedPremium($this->aDataEntry) ? ($this->_oMain->isPaidListing($this->aDataEntry['id']) ? '' : _t('_modzzz_formations_action_title_premium')) : '',

				'TitleManageFans' => $this->_oMain->isAllowedManageFans($this->aDataEntry) ? _t('_modzzz_formations_action_manage_fans') : '',
			    'AddToToday' => $this->_oMain->isAllowedMarkAsToday($this->aDataEntry) ? ($this->aDataEntry['today_formation'] ? _t('_modzzz_formations_action_remove_from_today') : _t('_modzzz_formations_action_add_to_today')) : '',
                'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotos($this->aDataEntry) ? _t('_modzzz_formations_action_upload_photos') : '',
	            'TitleEmbed' => $this->_oMain->isAllowedEmbed($this->aDataEntry) ? _t('_modzzz_formations_action_embed_video') : '',  
                'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideos($this->aDataEntry) ? _t('_modzzz_formations_action_upload_videos') : '',
                'TitleUploadSounds' => $this->_oMain->isAllowedUploadSounds($this->aDataEntry) ? _t('_modzzz_formations_action_upload_sounds') : '',
                'TitleUploadFiles' => $this->_oMain->isAllowedUploadFiles($this->aDataEntry) ? _t('_modzzz_formations_action_upload_files') : '',
            );
            ...
            return $oSubscription->getData() . $oFunctions->genObjectsActions($aInfo, 'modzzz_formations');
        } 
	...
	function getBlockCode_Local() {   
		...
		return $this->ajaxBrowse('local', $this->_oDb->getParam('modzzz_formations_perpage_main_recent'),array(),$sCity,$this->aDataEntry['id']); 
	}
	...
	function _blockCustomDisplay($aDataEntry, $sType) {
		...
		{  
			case "statistics":
				$aAllow = array('role', 'vacancies', 'experience', 'qualification', 'formation_type', 'formation_status');
			break;
			case "location":
	...
	function getBlockCode_Other() {    
		...
		return $this->ajaxBrowse('other', $this->_oDb->getParam('modzzz_formations_perpage_main_recent'),array(),$this->aDataEntry['company_id'],$this->aDataEntry['id']); 
	}
	...
	/*	if($this->aDataEntry['post_type'] == 'seeker')
        ...
        return parent::_blockFans ($this->_oDb->getParam('modzzz_formations_perpage_view_fans'), 'isAllowedViewFans', 'getFans');
    }            
	...
	/*	if($this->aDataEntry['post_type'] == 'seeker')
        ...
        return parent::_blockFansUnconfirmed (BX_FORMATIONS_MAX_FANS);
    }
	...
	function getBlockCode_Forum() {
		...
		$aVars['bx_repeat:entries'] = array();
  		foreach($aPosts as $aEachPost){
			...
			$sFormationsName = $aEachPost['title']; 
			...
			$sFormationsUrl = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() .'view/' . $sForumUri;
			$sTopicUrl = BX_DOL_URL_ROOT . 'forum/formations/topic/'.$sTopicUri.'.htm';

			$aVars['bx_repeat:entries'][] = array( 
							'topic_url' => $sTopicUrl, 
							'topic' => $sTopic, 
							'snippet_text' => $sPostText, 
							'item_title' => $sFormationsName, 
							'item_url' => $sFormationsUrl, 
							'created' => $sDate,
		...
		$sCode = $this->_oTemplate->parseHtmlByName('formation_forum', $aVars);  
    ...
    function getCode() {
        ...
        $this->_oMain->_processFansActions ($this->aDataEntry, BX_FORMATIONS_MAX_FANS);
	...
	function getBlockCode_ManageFormations() {
	...
	$edit_formations_url = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'edit/' . $this->aDataEntry['id'];
     ...
     $supprimer_edit_url = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'delete/' . $this->aDataEntry['id'];
		...
		$aVars = array(  
			...
			'manage_applicants_url' => $manage_applicants_url, 
			 ...
			 'TitleEdit_Url' =>  $edit_formations_url, 
			  
			 
			 'Photos_Url' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'upload_photos/' . $this->aDataEntry['uri'],
			 'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotos($this->aDataEntry) ? _t('_modzzz_formations_action_upload_photos') : '',
			 
		     'DocumentUrl' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'upload_files/' . $this->aDataEntry['uri'], 
		       'TitleUploadFiles' => $this->_oMain->isAllowedUploadFiles($this->aDataEntry) ? _t('_modzzz_formations_action_upload_files') : '',
			   
			   
			 'YoutubeUrl' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'embed/' . $this->aDataEntry['uri'],
			  'TitleEmbed' => $this->_oMain->isAllowedEmbed($this->aDataEntry) ? _t('_modzzz_formations_action_embed_video') : '',  
		...
		return $this->_oTemplate->parseHtmlByName('block_manage_formation', $aVars);   
	}
...
function getBlockCode_ManageApplyandFavorite() {
	 if (null == $this->_oMain)
		...
		if($aCompany['icon']){ 
		    $sImage = $this->_oDb->getLogoBusinessListing($aCompany['id'], $aCompany['icon'], true);
			 ...
			 $a = array ('ID' => $aCompany['author_id'], 'Avatar' => $aCompany['thumb']);
			  $aImage = BxDolService::call('photos', 'get_image', array($a, 'browse'), 'Search');
			 $sImage = $aImage['file'] ? $aImage['file'] : $this->_oTemplate->getImageUrl('formation-offered-thumb') ;
		  }
		  else{
			$sImage = $this->_oTemplate->getImageUrl('formation-offered-thumb');
		} 
		...
		if($aData['icon']){ 
			...
			$sNoImage = ($aData['post_type']=='seeker') ? $this->_oTemplate->getImageUrl('formation-wanted-thumb.png') : $this->_oTemplate->getImageUrl('formation-offered-thumb.png');
		}	
		...
		$aVars = array(  
			 'AddToFavorite' => $this->_oMain->isAllowedMarkAsFavorite($aData) ? ($this->_oMain->isFavorite($this->_oMain->_iProfileId, $aData['id']) ? _t('_modzzz_formations_action_remove_from_favorite') : _t('_modzzz_formations_action_add_to_favorite')) : '',
			  ...
			  'id' => $aData['id'],
			   ...
			   'TitleShare' => $this->_oMain->isAllowedShare($aData) ? _t('_modzzz_formations_action_title_share') : '',
			...
			'company_name' => $sCompanyName,
			  ...
			  'bx_if:views' => array( 
				...
				'content' => array(
					'views' => '<i class="sys-icon eye"></i>' .' '. $aData['views'] . ' '. _t('_modzzz_formations_views'),
