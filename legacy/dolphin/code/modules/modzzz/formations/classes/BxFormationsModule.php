...
function modzzz_formations_import ($sClassPostfix, $aModuleOverwright = array()) {
    global $aModule;
    $a = $aModuleOverwright ? $aModuleOverwright : $aModule;
    if (!$a || $a['uri'] != 'formations') {
...
define ('BX_FORMATIONS_PHOTOS_CAT', 'Formations');
define ('BX_FORMATIONS_PHOTOS_TAG', 'formations');

define ('BX_FORMATIONS_VIDEOS_CAT', 'Formations');
define ('BX_FORMATIONS_VIDEOS_TAG', 'formations');

define ('BX_FORMATIONS_SOUNDS_CAT', 'Formations');
define ('BX_FORMATIONS_SOUNDS_TAG', 'formations');

define ('BX_FORMATIONS_FILES_CAT', 'Formations');
define ('BX_FORMATIONS_FILES_TAG', 'formations');
define ('BX_FORMATIONS_MAX_FANS', 1000);
 

/*
 * Formations module
 *
 * This module allow users to create user's formations, 
 * users can rate, comment and discuss formation.
 * Formation can have photos, videos, sounds and files, uploaded
 * by formation's admins.
 *
 * 
 *
 * Profile's Wall:
 * 'add formation' event is displayed in profile's wall
 *
 *
 *
 * Spy:
 * The following qactivity is displayed for content_activity:
 * add - new formation was created
 * change - formation was chaned
 * rate - somebody rated formation
 * commentPost - somebody posted comment in formation
 *
 *
 *
 * Memberships/ACL:
 * formations view formation - BX_FORMATIONS_VIEW_FORMATION
 * formations browse - BX_FORMATIONS_BROWSE
 * formations search - BX_FORMATIONS_SEARCH
 * formations add formation - BX_FORMATIONS_ADD_FORMATION
 * formations apply - BX_FORMATIONS_APPLY
 * formations comments delete and edit - BX_FORMATIONS_COMMENTS_DELETE_AND_EDIT
 * formations edit any formation - BX_FORMATIONS_EDIT_ANY_FORMATION
 * formations delete any formation - BX_FORMATIONS_DELETE_ANY_FORMATION
 * formations mark as featured - BX_FORMATIONS_MARK_AS_FEATURED
 * formations approve formations - BX_FORMATIONS_APPROVE_FORMATIONS
 * formations broadcast message - BX_FORMATIONS_BROADCAST_MESSAGE
 *
 * 
 *
 * Service methods:
 *
 * Homepage block with different formations
 * @see BxFormationsModule::serviceHomepageBlock
 * BxDolService::call('formations', 'homepage_block', array());
 *
 * Profile block with user's formations
 * @see BxFormationsModule::serviceProfileBlock
 * BxDolService::call('formations', 'profile_block', array($iProfileId));
 *
 * Formation's forum permissions (for internal usage only)
 * @see BxFormationsModule::serviceGetForumPermission
 * BxDolService::call('formations', 'get_forum_permission', array($iMemberId, $iForumId));
 *
 * Member menu item for formations (for internal usage only)
 * @see BxFormationsModule::serviceGetMemberMenuItem
 * BxDolService::call('formations', 'get_member_menu_item', array());
 *
 *
 *
 * Alerts:
 * Alerts type/unit - 'modzzz_formations'
 * The following alerts are rised
 *
 * 
 *
 *  add - new formation was added
 *      $iObjectId - formation id
 *      $iSenderId - creator of a formation
 *      $aExtras['Status'] - status of added formation
 *
 *  change - formation's info was changed
 *      $iObjectId - formation id
 *      $iSenderId - editor user id
 *      $aExtras['Status'] - status of changed formation
 *
 *  delete - formation was deleted
 *      $iObjectId - formation id
 *      $iSenderId - deleter user id
 *
 *  mark_as_featured - formation was marked/unmarked as featured
 *      $iObjectId - formation id
 *      $iSenderId - performer id
 *      $aExtras['Featured'] - 1 - if formation was marked as featured and 0 - if formation was removed from featured 
 *
 */
class BxFormationsModule extends BxDolTwigModule {

    var $_oPrivacy;
    var $_aQuickCache = array ();

    function BxFormationsModule(&$aModule) {

        parent::BxDolTwigModule($aModule);        
        $this->_sFilterName = 'filter';
        $this->_sPrefix = 'modzzz_formations';

        bx_import ('Privacy', $aModule);
        $this->_oPrivacy = new BxFormationsPrivacy($this);

        bx_import ('CompanyPrivacy', $aModule);
        $this->_oCompanyPrivacy = new BxFormationsCompanyPrivacy($this);
  
	    $this->_oConfig->init($this->_oDb);

        $GLOBALS['oBxFormationsModule'] = &$this;
    ...
    function actionHome () {
        parent::_actionHome(_t('_modzzz_formations_page_title_home'));
    }

    function actionFiles ($sUri) {
        parent::_actionFiles ($sUri, _t('_modzzz_formations_page_title_files'));
    }

    function actionSounds ($sUri) {
        parent::_actionSounds ($sUri, _t('_modzzz_formations_page_title_sounds'));
    }

    function actionVideos ($sUri) {
        parent::_actionVideos ($sUri, _t('_modzzz_formations_page_title_videos'));
    }

    function actionPhotos ($sUri) {
        parent::_actionPhotos ($sUri, _t('_modzzz_formations_page_title_photos'));
    }

    function actionComments ($sUri) {
        parent::_actionComments ($sUri, _t('_modzzz_formations_page_title_comments'));
    }
 
    function actionBrowseFans ($sUri) {
        parent::_actionBrowseFans ($sUri, 'isAllowedViewFans', 'getFansBrowse', $this->_oDb->getParam('modzzz_formations_perpage_browse_fans'), 'browse_fans/', _t('_modzzz_formations_page_title_fans'));
    }
    ...
    function actionBrowseApplications ($sUri, $iEntryId=0, $iId=0, $sAction='') {
	   ...
	   $this->_actionApplications ($sUri, $this->_oDb->getParam('modzzz_formations_perpage_browse_applications'), 'browse_applications/', _t('_modzzz_formations_page_title_applications'));
    }
    ...
    function actionPopupLetter ($iId) {
		...
		$aVarsPopup = array (
            ...
            'title' => '<div style="text-transform:none">'._t('_modzzz_formations_cover_letter'). ' '.$aFirstName .' '. $aLastName .'</div>',
    ...
    {
        $sTitle = _t('_modzzz_formations_aucune_candidature');
	...
	function isValidAccess($iEntryId, $iProfileId, $sAction) {
		...
		if(getParam('modzzz_formations_post_credits')!='on' && getParam('modzzz_formations_post_points')!='on')
			return true;
    ...
    function actionView ($sUri) {
		...
		if($this->isValidAccess($iEntryId, $this->_iProfileId, 'view')){ 
			...
			parent::_actionView ($sUri, _t('_modzzz_formations_msg_pending_approval'));
		}else{
    ...
    function showViewPaymentForm ($sUri) {
		...
		if(getParam('modzzz_formations_view_credits')=='on'){ 
 
			$sPaymentUnit = 'credits'; 
			$sMoneyBalanceLabelC = _t("_modzzz_formations_form_caption_credits_balance");
			$sMoneyTypeC = _t("_modzzz_formations_credits");
			...
			$iViewCost = (int)$oCredit->_oDb->getActionValue('modzzz_formations', 'view');
			if($iMoneyBalance < $iViewCost){
				$sCode = _t('_modzzz_formations_msg_insufficient_credits_view', number_format($iMoneyBalance,0), number_format($iViewCost,0));
				$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyCreditsLink);
 			}
		}elseif(getParam('modzzz_formations_view_points')=='on'){
			
 			$sPaymentUnit = 'points';
			$sMoneyBalanceLabelC = _t("_modzzz_formations_form_caption_points_balance");
			$sMoneyTypeC = _t("_modzzz_formations_points");
			...
			$iViewCost = (int)$oPoint->_oDb->getActionValue($this->_iProfileId, 'modzzz_formations', 'view');
			if($iMoneyBalance < $iViewCost){
				$sCode = _t('_modzzz_formations_msg_insufficient_points_view', number_format($iMoneyBalance,0), number_format($iViewCost,0));
				$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyPointsLink);
 			}
		}
		...
		$aCustomForm = array(
			...
			'inputs' => array (
				...
				'header_info' => array(
					'type' => 'block_header',
					'caption' => _t("_modzzz_formations_form_caption_view", $sTitle),
				), 
				'money_balance' => array(
					'type' => 'custom',
				...
				'view_cost' => array(
					'type' => 'custom',
 					'caption' => _t("_modzzz_formations_form_caption_cost_to_view"),
					'content' => number_format($iViewCost,0) .' '. $sMoneyTypeC,
				),    
				'submit_form' => array(
					'type' => 'submit',
					'name' => 'submit_form',
					'value' => _t("_modzzz_formations_form_caption_submit"),
				),
			)
		);
    ...
    function actionCompanyView ($sUri) {
        $this->_actionCompanyView ($sUri, _t('_modzzz_formations_msg_pending_approval'));
    }
    ...
    function _actionCompanyView ($sUri, $sMsgPendingApproval) {
        ...
        new BxDolViews('modzzz_formations_company', $aDataEntry[$this->_oDb->_sFieldId]);
    }



    function actionUploadPhotos ($sUri) {        
        parent::_actionUploadMedia ($sUri, 'isAllowedUploadPhotos', 'images', array ('images_choice', 'images_upload'), _t('_modzzz_formations_page_title_upload_photos'));
    }

    function actionUploadVideos ($sUri) {
        parent::_actionUploadMedia ($sUri, 'isAllowedUploadVideos', 'videos', array ('videos_choice', 'videos_upload'), _t('_modzzz_formations_page_title_upload_videos'));
    }

    function actionUploadSounds ($sUri) {
        parent::_actionUploadMedia ($sUri, 'isAllowedUploadSounds', 'sounds', array ('sounds_choice', 'sounds_upload'), _t('_modzzz_formations_page_title_upload_sounds')); 
    }

    function actionUploadFiles ($sUri) {
        parent::_actionUploadMedia ($sUri, 'isAllowedUploadFiles', 'files', array ('files_choice', 'files_upload'), _t('_modzzz_formations_page_title_upload_files')); 
    }

    function actionBroadcast ($iEntryId) {
        parent::_actionBroadcast ($iEntryId, _t('_modzzz_formations_page_title_broadcast'), _t('_modzzz_formations_msg_broadcast_no_recipients'), _t('_modzzz_formations_msg_broadcast_message_sent'));
    }

    function actionBroadcastApplicants ($iEntryId) {
        $this->_actionBroadcastApplicants ($iEntryId, _t('_modzzz_formations_applicants_page_title_broadcast'), _t('_modzzz_formations_applicants_msg_broadcast_no_recipients'), _t('_modzzz_formations_applicants_msg_broadcast_message_sent'));
    }
    ...
    function _actionBroadcastApplicants ($iEntryId, $sTitle, $sMsgNoRecipients, $sMsgSent) {
        ...
        $aRecipients = $this->_oDb->getFormationApplicants ($iEntryId);
        if (!$aRecipients) {
    ...
    function isAllowedMarkAsFavorite ($aDataEntry, $isPerformAction = false) {
        if ($this->isAdmin()) 
            return true;
        $this->_defineActions();
        $aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_MARK_AS_FAVORITE, $isPerformAction);
    ...
    function actionMarkFavorite ($iEntryId) {
        $this->_actionMarkFavorite ($iEntryId, _t('_modzzz_formations_msg_added_to_favorite'), _t('_modzzz_formations_msg_removed_from_favorite'));
    }
	...
	function actionApply ($iEntryId) {
        $this->_actionApply ($iEntryId, _t('_modzzz_formations_page_title_apply'),  _t('_modzzz_formations_msg_application_sent'));
    }
	
	/*
	$aDataEntry = $this->_oDb->getEntryById($iEntryId);
	$iEntryFormationCountry =  $aDataEntry['country'];
	$iEntryFormationRegion = ($aDataEntry['state']) ? $this->_oDb->getStateName($iEntryFormationCountry, $aDataEntry['state']) : '';
	
		$iEntryFormationTitle = '<div class="Formation-Apply-Formation-Title">'. $aDataEntry['title'].'</div>'.'<div class="infoUnit infoUnitFontIcon  bx-twig-unit-line bx-def-font-small bx-def-font-grayed Formation-Apply-Formation-Title-Location"> '.'<i class="sys-icon map-marker"></i>'.'&nbsp;'.$aDataEntry['zip'].'&nbsp;'.$aDataEntry['city'].','.'&nbsp;'.$iEntryFormationRegion.'</div>';
		
		$iEntryFormationCountry =  $aDataEntry['country'];
		
		$iEntryFormationZip =  $aDataEntry['zip'];
		$iEntryFormationCity =  $aDataEntry['city'];
		
		
       
	    $this->_actionApply ($iEntryId, $iEntryFormationTitle,  _t('_modzzz_formations_msg_application_sent'));
    }
	*/
	////////////////////////////////
    ...
    function _actionApply ($iEntryId, $sTitle, $sMsgSent) {
        ...
        if ($oForm->isSubmittedAndValid ()) {
			...
			$sAttachMessage = ($sResume) ? _t('_modzzz_formations_apply_attach_message') : '';

			if(getParam('modzzz_resume_formation_connect')=='on'){
				...
				if($iResumeId){
					...
					$sAttachMessage .= _t('_modzzz_formations_view_resume_online_at') . $sResumeLink;
				} 
			}
			...
			$sApplicationTitle = _t('_modzzz_formations_apply_letter_title', $aDataEntry[$this->_oDb->_sFieldTitle]);
            ...
            $aTemplateVars = array (
                ...
                'EntryTitle' => $aDataEntry[$this->_oDb->_sFieldTitle],
                'EntryUrl' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri],  
				'Applicants_URL' => BX_DOL_URL_ROOT .  'm/formations/browse_applications/' . $aDataEntry[$this->_oDb->_sFieldUri],

				
				//Freddy ajout avar de la personne qui invite
				// 'InviterAvatar' =>  get_member_thumbnail($this->_iProfileId, 'none', true) ,
		...
		$iEntryFormationCountry =  $aDataEntry['country'];
	    $iEntryFormationRegion = ($aDataEntry['state']) ? $this->_oDb->getStateName($iEntryFormationCountry, $aDataEntry['state']) : '';
        $iEntryFormationTitle = $aDataEntry['title'];
     	$iEntryFormationZip =  $aDataEntry['zip'];
		$iEntryFormationCity =  $aDataEntry['city'];
		$iEntryFormationUri =  $aDataEntry['uri'];
		//////////////////////////////////////////////////////////////
		...
		$icoThumbApply = get_member_thumbnail($aProfileApplyForm['ID'], 'none', true);
		...
		// freddy oliger candidat à completer son profil avant de postuler si la formation est vide
		//if($aLinkedinProfileExperience <1 || $aLinkedinProfileEducation<1 || $aLinkedinProfileSkills <1 ) {
			if( $aLinkedinProfileEducation<1  ) {
		...
		$aUrlUpdateProfileIncomplet ='<div class="Update-Profile">'. '<i class="sys-icon edit"> </i>'.' '. _t('_modzzz_formations_profile_update'). '</div>';
		$aUrlUpdateProfileComplet ='<i class="sys-icon edit"> </i>'.' '. _t('_modzzz_formations_profile_update');
		 
      echo $this->_oTemplate->parseHtmlByName('view_profile_status_link.html',
	  array(
     	
		'Formation_Region_Apply' =>  $iEntryFormationRegion,
		'Formation_Title_Apply' =>   $iEntryFormationTitle,
		'Formation_Zip_Apply' =>  $iEntryFormationZip,
		'Formation_City_Apply' =>  $iEntryFormationCity,
		'Retour_Annonce' =>  BX_DOL_URL_ROOT.'m/'.'formations/'.'view/'.$iEntryFormationUri,
	 ...
	 'Profil_Edit_Global' =>  $aProfil_Edit_Global,
	  ...
	  'bx_if:Message_Profil_Complet' => array( 
								//'condition' => $aLinkedinProfileExperience >0 && $aLinkedinProfileEducation >0 && $aLinkedinProfileSkills >0,
								'condition' => $aLinkedinProfileEducation >0 ,
								'content' => array(
	  ...
	  /* 'bx_if:Alert_Applicant_Experience' => array( 
		...
		*/
	    ...
	    array('Retour_Annonce' =>  BX_DOL_URL_ROOT.'m/'.'formations/'.'view/'.$iEntryFormationUri,)
		); 
		// Freddy integration Linkedin Profile
     /*
	 echo $this->_oTemplate->parseHtmlByName('view_profile_status_link.html',
    ...
    function actionInvite ($iEntryId) {
        parent::_actionInvite ($iEntryId, 'modzzz_formations_invitation', $this->_oDb->getParam('modzzz_formations_max_email_invitations'), _t('_modzzz_formations_msg_invitation_sent'), _t('_modzzz_formations_msg_no_users_to_invite'), _t('_modzzz_formations_page_title_invite'));
    }

    function _getInviteParams ($aDataEntry, $aInviter) {
        return array (
                'FormationName' => $aDataEntry['title'],
                'FormationLocation' => _t($GLOBALS['aPreValues']['Country'][$aDataEntry['country']]['LKey']) . (trim($aDataEntry['city']) ? ', '.$aDataEntry['city'] : '') . ', ' . $aDataEntry['zip'],
                'FormationUrl' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry['uri'],
                'InviterUrl' => $aInviter ? getProfileLink($aInviter['ID']) : 'javascript:void(0);',
                //Freddy modif
				//'InviterNickName' => $aInviter ? $aInviter['NickName'] : _t('_modzzz_formations_user_unknown'),
				'InviterNickName' => $aInviter ? $aInviter['FirstName'].' '.$aInviter['LastName'] : _t('_modzzz_formations_user_unknown'),
                'InvitationText' => stripslashes(strip_tags($_POST['inviter_text'])),
            );        
    }    

    function actionCalendar ($iYear = '', $iMonth = '') {
        parent::_actionCalendar ($iYear, $iMonth, _t('_modzzz_formations_page_title_calendar'));
    }

    // Freddy Ajout Formation/Stage---post_type 
  // function actionSearch ($sKeyword = '', $sCategory = '', $sCountry = '', $sState = '', $sCity = '') {
    ...
    // if ($sCountry || $sCategory || $sKeyword || $sState || $sCity ) {
        ...
        modzzz_formations_import ('FormSearch');
        $oForm = new BxFormationsFormSearch ();
        $oForm->initChecker();
        ...
        if ($oForm->isSubmittedAndValid ()) {
		  ...
		  //  $o = new BxFormationsSearchResult('search', $oForm->getCleanValue('Keyword'), $oForm->getCleanValue('Category'), $oForm->getCleanValue('City'), $oForm->getCleanValue('State'), $oForm->getCleanValue('Country') );
		    $o = new BxFormationsSearchResult('search', $oForm->getCleanValue('Keyword'), $oForm->getCleanValue('Category'), $oForm->getCleanValue('City'), $oForm->getCleanValue('State'), $oForm->getCleanValue('Country'), $oForm->getCleanValue('Country'), $oForm->getCleanValue('Type'), $oForm->getCleanValue('Parent') );
        ...
        echo $oForm->getCode ();
        $this->_oTemplate->addCss ('main.css');
        $this->_oTemplate->pageCode(_t('_modzzz_formations_caption_search'));
    } 
 
    function actionAdd () {
        parent::_actionAdd (_t('_modzzz_formations_page_title_add'));
    }

    function _addForm ($sRedirectUrl) {
  
		$bPaidFormation = $this->isAllowedPaidFormation (); 
		if( $bPaidFormation && (!isset($_POST['submit_form'])) ){
			...
			if(getParam('modzzz_formations_post_credits')=='on'){ 
				...
				$sBuyCreditsLink = BX_DOL_URL_ROOT . $oCredit->_oConfig->getBaseUri() . 'purchase_credits'; 
				$iMoneyBalance = (int)$oCredit->getMemberCredits($this->_iProfileId);
				$iPostCost = (int)$oCredit->_oDb->getActionValue('modzzz_formations', 'post');
				if($iMoneyBalance < $iPostCost){
					$sCode = _t('_modzzz_formations_msg_insufficient_credits_post', number_format($iMoneyBalance,0), number_format($iPostCost,0));
					$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyCreditsLink);
				}
			}elseif(getParam('modzzz_formations_post_points')=='on') {
				...
				$sBuyPointsLink = BX_DOL_URL_ROOT . $oPoint->_oConfig->getBaseUri() . 'purchase_points'; 
				$iMoneyBalance = $oPoint->_oDb->getMemberPoints($this->_iProfileId);
				$iPostCost = (int)$oPoint->_oDb->getActionValue($this->_iProfileId, 'modzzz_formations', 'post');
				if($iMoneyBalance < $iPostCost) {
					$sCode = _t('_modzzz_formations_msg_insufficient_points_post', number_format($iMoneyBalance,0), number_format($iPostCost,0));
					$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyPointsLink);
				}
			}
		...
		$aCustomForm = array(
			...
			'inputs' => array (
				...
				'header_payment' => array(
					'type' => 'block_header',
					'caption' => _t("_modzzz_formations_form_caption_post", $sTitle),
				), 
				'money_balance' => array(
					'type' => 'custom',
					'caption' => $sMoneyBalanceLabelC,
				...
				'post_cost' => array(
					'type' => 'custom',
 					'caption' => _t("_modzzz_formations_form_caption_cost_to_post"),
					'content' => number_format($iPostCost,0) .' '. $sMoneyTypeC .' '. _t("_modzzz_formations_form_per_day"),
				),   
                'quantity' => array(
                    'caption'  => _t('_modzzz_formations_caption_num_post_days'),
                    'type'   => 'select',
                    'name' => 'quantity',
                    'values' => $aQuantity,
                    'required' => true,
                    'checker' => array (  
                        'func'   => 'Preg',
                        'params' => array('/^[0-9]+$/'),
                        'error'  => _t('_modzzz_formations_caption_err_post_days'),
                    ),
                ), 
				'submit_form' => array(
					'type' => 'submit',
		...
		if ( $oForm->isSubmittedAndValid() ) {
			...
			if(getParam('modzzz_formations_post_credits')=='on'){ 
				...
				if($iMoneyBalance < $iTotalCost){
					$sCode = _t('_modzzz_formations_msg_insufficient_credits_post', number_format($iMoneyBalance,0), number_format($iTotalCost,0));
					$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyCreditsLink);
					echo MsgBox($sCode);
					return;
				}
				...
				if($iMoneyBalance < $iTotalCost){
					$sCode = _t('_modzzz_formations_msg_insufficient_points_post', number_format($iMoneyBalance,0), number_format($iTotalCost,0));
					$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyPointsLink);
					echo MsgBox($sCode);
					return;
				}
			...
			if($bSuccess){
				...
				$sResultMsg = _t("_modzzz_formations_msg_post_success") ; 
			}else{
				$sResultMsg = _t("_modzzz_formations_msg_post_failure") ; 
			}
  ...
  // Fin Freddy Integration Business Listing
		...
		$aForm = array(
            ...
            'inputs' => array( 
                ...
                'package_id' => array( 
                    ...
                    'name' => 'package_id',
					'values'=> $aPackage,
                    'caption' => _t('_modzzz_formations_form_caption_package'), 
                    'required' => true,
					'attrs' => array(
                    ...
                    'checker' => array (
                        'func' => 'avail',
                        'error' => _t ('_modzzz_formations_form_err_package'),
                    ),   
                ),  
				...
				'package_desc' => array( 
                    ...
                    'name' => 'package_desc',
                    'caption' => _t('_modzzz_formations_package_desc'),  
                ), 
				
                'submit' => array(
                    'type'  => 'submit',
                    'value' => _t('_modzzz_formations_continue'),
                    'name'  => 'submit_package',
                ),
            ),
        );
		...
		// NB Ce bout de code ne fonctionne seulement lorsque Activate Paid Formations listings est activé 
		...
		if(count($aCompanies)==0){
			...
			echo MsgBox (_t('_modzzz_formations_add_business_buton_register'));
    ...
    function showAddForm($sRedirectUrl) {
        ...
        if ($oForm->isSubmittedAndValid ()) {
			...
			if($this->isValidAccess(0, $this->_iProfileId, 'post')){ 
 			...
 			$bPaidFormation = $this->isPaidPackage($oForm->getCleanValue('package_id')); 
			...
			if(!($oForm->getCleanValue('city') || $oForm->getCleanValue('zip'))){
				...
				}elseif($aValsAdd['company_type'] == 'formation'){
            ...
            if ($iEntryId) {
				...
				if($bPayPointsCredits){
				...
				}elseif($this->isAllowedPaidFormation()){
					...
					$iNumActiveDays = (int)getParam("modzzz_formations_free_expired");
			...
			$this->_oTemplate->addJs ('formation.js'); 
    ...
    function actionEdit ($iEntryId) {
        $this->_actionEdit ($iEntryId, _t('_modzzz_formations_page_title_edit'));
    }
  ...
  function _actionEdit ($iEntryId, $sTitle) { 
    ...
    function actionDelete ($iEntryId) {
        parent::_actionDelete ($iEntryId, _t('_modzzz_formations_msg_formation_was_deleted'));
    }

    function actionMarkFeatured ($iEntryId) {
        parent::_actionMarkFeatured ($iEntryId, _t('_modzzz_formations_msg_added_to_featured'), _t('_modzzz_formations_msg_removed_from_featured'));
    }
 
    function actionMarkToday ($iEntryId) {
        $this->_actionMarkToday ($iEntryId, _t('_modzzz_formations_msg_added_to_today'), _t('_modzzz_formations_msg_removed_from_today'));
    }
    ...
    function _actionMarkToday ($iEntryId, $sMsgSuccessAdd, $sMsgSuccessRemove) {
        ...
        if ($this->_oDb->markAsToday($iEntryId)) {
            ...
            $sRedirect = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri];
            $sJQueryJS = genAjaxyPopupJS($iEntryId, 'ajaxy_popup_result_div', $sRedirect);
            echo MsgBox($aDataEntry[$this->_oDb->_sFieldDailyFormation] ? $sMsgSuccessRemove : $sMsgSuccessAdd) . $sJQueryJS;
            exit;
        }        
    ...
    function actionJoin ($iEntryId, $iProfileId) {
        ...
        parent::_actionJoin ($iEntryId, $iProfileId, _t('_modzzz_formations_msg_joined_already'), _t('_modzzz_formations_msg_joined_request_pending'), _t('_modzzz_formations_msg_join_success'), _t('_modzzz_formations_msg_join_success_pending'), _t('_modzzz_formations_msg_leave_success'));
    }    

    function actionSharePopup ($iEntryId) {
        parent::_actionSharePopup ($iEntryId, _t('_modzzz_formations_caption_share_formation'));
    }

    function actionCompanySharePopup ($iEntryId) {
        $this->_actionCompanySharePopup ($iEntryId, _t('_modzzz_formations_caption_share_company'));
    }
    ...
    function actionManageFansPopup ($iEntryId) {
        parent::_actionManageFansPopup ($iEntryId, _t('_modzzz_formations_caption_manage_fans'), 'getFans', 'isAllowedManageFans', 'isAllowedManageAdmins', BX_FORMATIONS_MAX_FANS);
    }

    function actionTags() {
        parent::_actionTags (_t('_modzzz_formations_page_title_tags'));
    }    
    ...
    function actionCategories ($sUri='') { 
        ...
        echo $oPage->getCode();
		$this->_oTemplate->addCss (array('unit.css', 'main.css', 'twig.css')); 
        $this->_oTemplate->pageCode(_t('_modzzz_formations_page_title_categories'), false, false);
    }
    ...
    function actionSubCategories ($sUri='') {
		...
		$this->_oTemplate->addCss (array('unit.css', 'main.css', 'twig.css'));
         ...
         $this->_oTemplate->pageCode(_t('_modzzz_formations_page_title_subcategories'), false, false);
    }
    ...
    function serviceHomepageBlock () {
        ...
        $o = new BxFormationsPageMain ($this);
        $o->sUrlStart = BX_DOL_URL_ROOT . 'index.php?';
		
		$this->_oTemplate->addCss (array('unit.css', 'twig.css','main.css'));
  
        $sDefaultHomepageTab = $this->_oDb->getParam('modzzz_formations_homepage_default_tab');
        ...
        return $o->ajaxBrowse(
            $sBrowseMode,
            $this->_oDb->getParam('modzzz_formations_perpage_homepage'), 
            array(
                _t('_modzzz_formations_tab_featured') => array('href' => BX_DOL_URL_ROOT . 'index.php?filter=featured', 'active' => 'featured' == $sBrowseMode, 'dynamic' => true),
                _t('_modzzz_formations_tab_recent') => array('href' => BX_DOL_URL_ROOT . 'index.php?filter=recent', 'active' => 'recent' == $sBrowseMode, 'dynamic' => true),
                _t('_modzzz_formations_tab_top') => array('href' => BX_DOL_URL_ROOT . 'index.php?filter=top', 'active' => 'top' == $sBrowseMode, 'dynamic' => true),
                _t('_modzzz_formations_tab_popular') => array('href' => BX_DOL_URL_ROOT . 'index.php?filter=popular', 'active' => 'popular' == $sBrowseMode, 'dynamic' => true),
            )
        );
    }
    ...
    function serviceProfileBlock ($iProfileId) {
        ...
        return $o->ajaxBrowse(
            'user', 
            $this->_oDb->getParam('modzzz_formations_perpage_profile'), 
            array(),
     ...
     * @return html to display member formations in account page a block
     */ 
    function serviceAccountPageBlockOLD () {
        ...
        return $o->ajaxBrowse(
            'user', 
            $this->_oDb->getParam('modzzz_formations_perpage_account'), 
            array(),
            $aProfile['NickName'],
            true,
            false 
    ...
    function serviceAccountPageBlock () {
     ...
     /**
     * Account block with different events
     * @return html to display local formations in account page a block
     */  
    ...
    function serviceGetMemberMenuItem () {
        parent::_serviceGetMemberMenuItem (_t('_modzzz_formations'), _t('_modzzz_formations'), 'formations.png');
    }
    ...
    function serviceGetSpyPost($sAction, $iObjectId = 0, $iSenderId = 0, $aExtraParams = array()) {
        return parent::_serviceGetSpyPost($sAction, $iObjectId, $iSenderId, $aExtraParams, array(
            'add' => '_modzzz_formations_spy_post',
            'change' => '_modzzz_formations_spy_post_change',
            'join' => '_modzzz_formations_spy_join',
            'rate' => '_modzzz_formations_spy_rate',
            'commentPost' => '_modzzz_formations_spy_comment',
        ));
    }
    ...
    function serviceGetSubscriptionParams ($sAction, $iEntryId) {
        ...
        $a = array (
            'change' => _t('_modzzz_formations_sbs_change'),
            'commentPost' => _t('_modzzz_formations_sbs_comment'),
            'rate' => _t('_modzzz_formations_sbs_rate'),
            'join' => _t('_modzzz_formations_sbs_join')  
        );
    ...
    function actionLocal ($sCountry='', $sState='', $sCategory='') { 
		...
		$sTitle = _t('_modzzz_formations_page_title_local');
    ...
    function actionLocalCountry ($sCountry='', $sCategory='') { 
        ...
        $this->_oTemplate->pageCode(_t('_modzzz_formations_page_title_local'), false, false);
    }
...
/*
        ...
        echo $this->_oTemplate->adminBlock ($sContent, _t('_modzzz_formations_admin_links'));
*/

        $aMenu = array(
            'pending_approval' => array(
                'title' => _t('_modzzz_formations_menu_admin_pending_approval'), 
            ...
            'admin_entries' => array(
                'title' => _t('_modzzz_formations_menu_admin_entries'), 
			...
			'invoices' => array(
                'title' => _t('_modzzz_formations_menu_manage_invoices'), 
			...
			'orders' => array(
                'title' => _t('_modzzz_formations_menu_manage_orders'), 
			...
			'packages' => array(
                'title' => _t('_modzzz_formations_menu_manage_packages'), 
           ...
           'categories' => array(
                'title' => _t('_modzzz_formations_menu_admin_manage_categories'), 
            ...
            'subcategories' => array(
                'title' => _t('_modzzz_formations_menu_admin_manage_subcategories'), 
            ...
            'create' => array(
                'title' => _t('_modzzz_formations_menu_admin_add_entry'), 
            ...
            'settings' => array(
                'title' => _t('_modzzz_formations_menu_admin_settings'), 
        ...
        echo $this->_oTemplate->adminBlock ($sContent, _t('_modzzz_formations_page_title_administration'), $aMenu);
 
		$this->_oTemplate->addCssAdmin (array('admin.css', 'unit.css', 'twig.css', 'main.css', 'forms_extra.css', 'forms_adv.css'));
	
		if($sUrl == 'create') 
			$this->_oTemplate->addAdminJs ('formation.js'); 
			
			
			 
			

        $this->_oTemplate->pageCodeAdmin (_t('_modzzz_formations_page_title_administration'));
    }

    function actionAdministrationSettings () {
        return parent::_actionAdministrationSettings ('Formations');
    }
    ...
    function actionAdministrationCategories ($sParam1='') {
		...
		$sContent = $GLOBALS['oAdmTemplate']->parseHtmlByName('top_block_select.html', array(
			'name' => _t('_modzzz_formations_categories'),
	    ...
	    if($iCategory){
			$sControls = BxTemplSearchResult::showAdminActionsPanel($sFormName, array(
				'action_edit' => _t('_modzzz_formations_categ_btn_edit'),
				'action_delete' => _t('_modzzz_formations_categ_btn_delete'), 
				'action_add' => _t('_modzzz_formations_categ_btn_add'),
				'action_sub' => _t('_modzzz_formations_categ_btn_subcategories'), 
			), 'pathes', false);
	    }else{
			$sControls = BxTemplSearchResult::showAdminActionsPanel($sFormName, array(
				'action_save' => _t('_modzzz_formations_categ_btn_save')
			), 'pathes', false);	 
	    }
    ...
    function actionAdministrationSubCategories ($sParam1='', $sParam2='') {
		...
		if(!$iCategory){
			$sContent = MsgBox(_t('_modzzz_formations_manage_subcategories_msg')); 
		...
		$sContent = $GLOBALS['oAdmTemplate']->parseHtmlByName('top_block_select.html', array(
			'name' => $sCategoryName .': '. _t('_modzzz_formations_subcategories'),
    ...
    function actionAdministrationManage ($isAdminEntries = false) {
        return parent::_actionAdministrationManage ($isAdminEntries, '_modzzz_formations_admin_delete', '_modzzz_formations_admin_activate');
    }
    ...
    function onEventMarkAsToday ($iEntryId, $aDataEntry) {
        ...
        // arise alert
		$oAlert = new BxDolAlerts($this->_sPrefix, 'mark_as_today', $iEntryId, $this->_iProfileId, array('Daily' => $aDataEntry[$this->_oDb->_sFieldDailyFormation]));
		$oAlert->alert();
    }   
    ...
    function isAllowedCompanyView ($aDataEntry, $isPerformAction = false) {
        ...
        // check admin acl
        $this->_defineActions();
		$aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_VIEW_FORMATION, $isPerformAction);
    ...
    function isAllowedView ($aDataEntry, $isPerformAction = false) {
        ...
        // check user formation 
	    $isAllowed =   $this->_oPrivacy->check('view_formation', $aDataEntry['id'], $this->_iProfileId); 
    ...
    function isAllowedBrowse ($isPerformAction = false) {
        if ($this->isAdmin()) 
            return true;
        $this->_defineActions();
		$aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_BROWSE, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    }
    ...
    function isAllowedSearch ($isPerformAction = false) {
        if ($this->isAdmin()) 
            return true;
        $this->_defineActions();
		$aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_SEARCH, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    }
    ...
    function isAllowedAdd ($isPerformAction = false) {
        ...
        if (!$GLOBALS['logged']['member']) 
            return false;
        $this->_defineActions();
		$aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_ADD_FORMATION, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    } 
    ...
    function isAllowedEdit ($aDataEntry, $isPerformAction = false) {
        ...
        // check acl
        $this->_defineActions();
		$aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_EDIT_ANY_FORMATION, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    } 
    ...
    function isAllowedMarkAsToday ($aDataEntry, $isPerformAction = false) {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_MARK_AS_TODAY, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;        
    }
    ...
    function isAllowedMarkAsFeatured ($aDataEntry, $isPerformAction = false) {
        if ($this->isAdmin()) 
            return true;
        $this->_defineActions();
        $aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_MARK_AS_FEATURED, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;        
    }
	...
	//freddy  Commenter pour broadcaster stage
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_BROADCAST_MESSAGE, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;        
    }
    ...
    function isAllowedApply ($aDataEntry, $isPerformAction = false) {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_APPLY, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;        
    }
    ...
    function isAllowedDelete (&$aDataEntry, $isPerformAction = false) {
        if ($this->isAdmin() || ($GLOBALS['logged']['member'] && $aDataEntry['author_id'] == $this->_iProfileId && isProfileActive($this->_iProfileId))) 
            return true;
        $this->_defineActions();
		$aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_DELETE_ANY_FORMATION, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    }     

    function onEventJoinRequest ($iEntryId, $iProfileId, $aDataEntry) {
        parent::_onEventJoinRequest ($iEntryId, $iProfileId, $aDataEntry, 'modzzz_formations_join_request', BX_FORMATIONS_MAX_FANS);
    }

    function onEventJoinReject ($iEntryId, $iProfileId, $aDataEntry) {
        parent::_onEventJoinReject ($iEntryId, $iProfileId, $aDataEntry, 'modzzz_formations_join_reject');
    }

    function onEventFanRemove ($iEntryId, $iProfileId, $aDataEntry) {        
        parent::_onEventFanRemove ($iEntryId, $iProfileId, $aDataEntry, 'modzzz_formations_fan_remove');
    }

    function onEventFanBecomeAdmin ($iEntryId, $iProfileId, $aDataEntry) {        
        parent::_onEventFanBecomeAdmin ($iEntryId, $iProfileId, $aDataEntry, 'modzzz_formations_fan_become_admin');
    }

    function onEventAdminBecomeFan ($iEntryId, $iProfileId, $aDataEntry) {        
        parent::_onEventAdminBecomeFan ($iEntryId, $iProfileId, $aDataEntry, 'modzzz_formations_admin_become_fan');
    }

    function onEventJoinConfirm ($iEntryId, $iProfileId, $aDataEntry) {
        parent::_onEventJoinConfirm ($iEntryId, $iProfileId, $aDataEntry, 'modzzz_formations_join_confirm');
    }
    ...
    function isAllowedCreatorCommentsDeleteAndEdit (&$aDataEntry, $isPerformAction = false) {
        if ($this->isAdmin()) 
            return true;
        if (getParam('modzzz_formations_author_comments_admin') && $this->isEntryAdmin($aDataEntry))
            return true;
        $this->_defineActions();
		$aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_COMMENTS_DELETE_AND_EDIT, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    }
    ...
    function isAllowedCompanyCreatorCommentsDeleteAndEdit (&$aDataEntry, $isPerformAction = false) {
        if ($this->isAdmin()) 
            return true;
        if (getParam('modzzz_formations_author_comments_admin') && $this->isCompanyEntryAdmin($aDataEntry))
            return true;
        $this->_defineActions();
		$aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_COMMENTS_DELETE_AND_EDIT, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;
    ...
    function isEntryAdmin($aDataEntry, $iProfileId = 0) {
        ...
        if (($GLOBALS['logged']['member'] || $GLOBALS['logged']['admin']) && $aDataEntry['author_id'] == $iProfileId && isProfileActive($iProfileId))
            return true;
        return $this->_oDb->isFormationAdmin ($aDataEntry['id'], $iProfileId) && isProfileActive($iProfileId);
    }
    ...
    function _defineActions () {
        defineMembershipActions(array('formations purchase','formations mark as favorite', 'formations relist', 'formations extend', 'formations purchase featured', 'formations view formation', 'formations apply', 'formations browse', 'formations search', 'formations add formation', 'formations comments delete and edit', 'formations edit any formation', 'formations delete any formation', 'formations mark as featured','formations mark as today', 'formations approve formations', 'formations broadcast message'));
    }

    function _browseMy (&$aProfile) {        
        parent::_browseMy ($aProfile, _t('_modzzz_formations_page_title_my_formations'));
    }    
    ...
    function _actionUploadResume ($iFormationId=0) {
    ...
    function _actionUploadIcon ($iCompanyId=0) {
		...
		$sPath = $this->_oConfig->getCompanyIconsPath();	
 		...
 		$iImageWidth = getParam("modzzz_formations_company_image_width");
		$iImageHeight = getParam("modzzz_formations_company_image_height"); 

		$iIconWidth = getParam("modzzz_formations_company_icon_width");
		$iIconHeight = getParam("modzzz_formations_company_icon_height");
    ...
    function actionCompanyEdit ($iEntryId) {
        $this->_actionCompanyEdit ($iEntryId, _t('_modzzz_formations_page_company_title_edit'));
    }

    function actionCompanyDelete ($iEntryId) {
        $this->_actionCompanyDelete ($iEntryId, _t('_modzzz_formations_company_msg_formation_was_deleted'));
    }

    function actionCompanyMarkFeatured ($iEntryId) {
        $this->_actionCompanyMarkFeatured ($iEntryId, _t('_modzzz_formations_company_msg_added_to_featured'), _t('_modzzz_formations_company_msg_removed_from_featured'));
    }

    function actionCompanyBroadcast ($iEntryId) {
        $this->_actionCompanyBroadcast ($iEntryId, _t('_modzzz_formations_page_company_title_broadcast'), _t('_modzzz_formations_company_msg_broadcast_no_recipients'), _t('_modzzz_formations_msg_broadcast_message_sent'));
    }

    function actionCompanyInvite ($iEntryId) {
        $this->_actionCompanyInvite ($iEntryId, 'modzzz_formations_company_invitation', $this->_oDb->getParam('modzzz_formations_max_email_invitations'), _t('_modzzz_formations_msg_invitation_sent'), _t('_modzzz_formations_msg_no_users_to_invite'), _t('_modzzz_formations_page_company_title_invite'));
    }
   ...
   function _actionCompanyBroadcast ($iEntryId, $sTitle, $sMsgNoRecipients, $sMsgSent) {
	...
	function processFormations() { 
		...
		if (getParam('modzzz_formations_paid_active')=='on') 
			$this->_oDb->processFormations(); 
		else
			$this->processFreeFormations();
	}

	function processFreeFormations() { 
		...
		foreach($aEntries as $aEachEntry){
			...
			$this->_oDb->alertOnAction('modzzz_formations_expired', $iEntryId, $aEachEntry['author_id']); 
		...
		$this->_oDb->processExpiringFormations();   
	}
    ...
    function actionPackages () { 
        ...
        $this->_oTemplate->pageCode(_t('_modzzz_formations_page_title_packages'), false, false);
    }
   ...
   function actionAdministrationPackages ($sParam1='') {
		...
		$sContent = $GLOBALS['oAdmTemplate']->parseHtmlByName('top_block_select.html', array(
			'name' => _t('_modzzz_formations_packages'),
    ...
    function actionAdministrationOrders () {
		...
		$sContent = $this->_manageOrders ('order', '', true, 'bx_twig_admin_form', array(
			'action_activate' => '_modzzz_formations_admin_activate',
    ...
    function _manageOrders ($sMode, $sValue, $isFilter, $sFormName, $aButtons, $sAjaxPaginationBlockId = '', $isMsgBoxIfEmpty = true, $iPerPage = 14, $bActionsPanel = true) {
 	...
 	function isPaidPackage($iPackageId){
		...
		if(!$this->isAllowedPaidFormation())
			return false;

		return $this->_oDb->isPaidPackage($iPackageId);  
	}
 
    function isAllowedPaidFormation ($bCheckAdmin=true) {
    ...
    function actionPaypalProcess($iProfileId, $iFormationId) {
        $sPostData = '';
        $sPageContent = '';
        ...
        if($aData) {
			...
			$aDataEntry = $this->_oDb->getEntryById ($iFormationId);
			$sRedirectUrl = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri];
        	...
        	if((int)$aResponse['code'] !== 0){
 				$this->actionPostPurchase(_t('_modzzz_formations_purchase_failed')); 
 				return;
			}
			...
			//if (($aData['receiver_email'] != trim(getParam('modzzz_formations_paypal_email'))) || ($aData['txn_type'] != 'web_accept')) {

			if($aData['txn_type'] != 'web_accept') {
				$this->actionPostPurchase(_t('_modzzz_formations_purchase_failed'));
			}else{ 
				if($this->_oDb->isExistPaypalTransaction($iProfileId, $aData['txn_id'])) { 
					$this -> actionPostPurchase(_t('_modzzz_formations_transaction_completed_already', $sRedirectUrl)); 
				} else {
					if( $this->_oDb->saveTransactionRecord($iProfileId, $iFormationId, $aData['txn_id'], 'Paypal Purchase')) { 
						
						$this->_oDb->setItemStatus($iFormationId, 'approved'); 
						
						$this->_oDb->setInvoiceStatus($iFormationId, 'paid'); 
					} else {
						$this -> actionPostPurchase(_t('_modzzz_formations_trans_save_failed'));
					}
				}
			}
	...
	function initializeCheckout($iFormationId, $fTotalCost, $iQuantity=1, $bFeatured=0, $sTitle='') {
		...
		if($bFeatured){
			$sNotifyUrl = $this->_oConfig->getFeaturedCallbackUrl() . $this->_iProfileId .'/'. $iFormationId;
			$sItemDesc = _t('_modzzz_formations_paypal_featured_item_desc', $sTitle);
 		}else{
			$sNotifyUrl = $this->_oConfig->getPurchaseCallbackUrl() . $this->_iProfileId .'/'. $iFormationId;
			$sItemDesc = _t('_modzzz_formations_paypal_item_desc', $sTitle);
		}
        ...
        $aFormData = array_merge($aFormData, array(
			'business' => getParam('modzzz_formations_paypal_email'), 
            'item_name' => $sItemDesc,
			'amount' => $fTotalCost, 
            'item_number' => $iFormationId,
            'quantity' => $iQuantity,
    	...
    	Redirect($this->_oConfig->getPurchaseBaseUrl(), $aFormData, 'post', "Formation Listing");
    	exit();
	}
    ...
    function actionPostPurchase($sTransMessage = '') {
        ...
        $this->_oTemplate->addCss ('main.css'); 
        $this->_oTemplate->pageCode(_t('_modzzz_formations_post_purchase_header')); 
    }
 

	//modzzz.com
    function actionPurchaseFeatured($iFormationId, $sTransMessage = '') {
		...
		$iPerDayCost = getParam('modzzz_formations_featured_cost');
		...
		$aForm = array(
            'form_attrs' => array(
                ...
                'method' => 'post', 
                'action' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'purchase_featured/'.$iFormationId,
            ),
            'params' => array (
                'db' => array(
            ...
            'inputs' => array( 
                ...
                'title' => array(
                    'type' => 'custom',
                    'name' => 'title',
					'caption'  => _t('_modzzz_formations_form_caption_current_item'),
                    'content' => $sTitle,
                ), 
                'cost' => array(
                    'type' => 'custom',
                    'name' => 'cost',
					'caption'  => _t('_modzzz_formations_featured_cost_per_day'),
                    'content' => $iPerDayCost .' '. $this->_oConfig->getPurchaseCurrency(),
                ), 
                'status' => array(
                    'type' => 'custom',
                    'name' => 'status',
					'caption'  => _t('_modzzz_formations_featured_status'),
                    'content' => ($aDataEntry['featured']) ? _t('_modzzz_formations_featured_until') .' '. $this->_oTemplate->filterCustomDate($aDataEntry['featured_expiry_date']) : _t('_modzzz_formations_not_featured'), 
                ), 
                'quantity' => array(
                    'caption'  => _t('_modzzz_formations_caption_num_featured_days'),
                    'type'   => 'text',
                    'name' => 'quantity',
                    'required' => true,
                    'checker' => array (  
                        'func'   => 'Preg',
                        'params' => array('/^[0-9]+$/'),
                        'error'  => _t('_modzzz_formations_caption_err_featured_days'),
                    ),
                ),
                'submit' => array(
                    'type'  => 'submit',
                    'value' => ($aDataEntry['featured']) ? _t('_modzzz_formations_extend_featured') : _t('_modzzz_formations_get_featured'),
        ...
        if ($oForm->isSubmittedAndValid() && $oForm->getCleanValue('quantity')) { 
			...
			$this->initializeCheckout($iFormationId, $fCost, $oForm->getCleanValue('quantity'), true, $sTitle);  
			return;   
        } else {
        ...
        $this->_oTemplate->pageCode(_t('_modzzz_formations_purchase_featured')); 
    }
 
    function actionPaypalFeaturedProcess($iProfileId, $iFormationId) {
        $sPostData = '';
        $sPageContent = '';
        ...
        if($aData) {
        	...
        	if((int)$aResponse['code'] !== 0){
 				$this->actionPurchaseFeatured(_t('_modzzz_formations_purchase_failed')); 
 				return;
			}
			...
			if($aData['txn_type'] != 'web_accept') {
				$this->actionPurchaseFeatured($iFormationId, _t('_modzzz_formations_purchase_failed'));
			}else { 
				$fAmount = $this->_getReceivedAmount($aData);
			
				if($this->_oDb->isExistFeaturedTransaction($iProfileId, $aData['txn_id'])) {
					$this -> actionPurchaseFeatured($iFormationId, _t('_modzzz_formations_transaction_completed_already')); 
				} else {
					if( $this->_oDb->saveFeaturedTransactionRecord($iProfileId, $iFormationId,  $iQuantity, $fAmount, $aData['txn_id'], 'Paypal Purchase')) {

						$this->_oDb->updateFeaturedEntryExpiration($iFormationId, $iQuantity); 
			    
					} else {
						$this -> actionPurchaseFeatured($iFormationId, _t('_modzzz_formations_purchase_fail'));
					}
				}
			}
    ...
    function isAllowedPurchaseFeaturedOLD ($aDataEntry, $isPerformAction = false) {
		...
		if(getParam("modzzz_formations_buy_featured")!='on')
			return false;
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_PURCHASE_FEATURED, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;        
    } 

    function isMembershipEnabledForImages () {
        return ($this->_isMembershipEnabledFor ('BX_PHOTOS_ADD') && $this->_isMembershipEnabledFor ('BX_FORMATIONS_PHOTOS_ADD'));
    }

    function isMembershipEnabledForVideos () {
        return $this->_isMembershipEnabledFor ('BX_VIDEOS_ADD');
        return ($this->_isMembershipEnabledFor ('BX_VIDEOS_ADD') && $this->_isMembershipEnabledFor ('BX_FORMATIONS_VIDEOS_ADD')); 
    }

    function isMembershipEnabledForSounds () {
        return ($this->_isMembershipEnabledFor ('BX_SOUNDS_ADD') && $this->_isMembershipEnabledFor ('BX_FORMATIONS_SOUNDS_ADD'));
    }

    function isMembershipEnabledForFiles () {
        return ($this->_isMembershipEnabledFor ('BX_FILES_ADD') && $this->_isMembershipEnabledFor ('BX_FORMATIONS_FILES_ADD'));
    }
 
    function _isMembershipEnabledFor ($sMembershipActionConstant) { 
        defineMembershipActions (array('photos add', 'sounds add', 'videos add', 'files add', 'formations photos add', 'formations sounds add', 'formations videos add', 'formations files add'));
    ...
    function isAllowedPremium (&$aDataEntry, $isPerformAction = false) {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_PURCHASE, $isPerformAction); 
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;     
    } 
    ...
    function actionPremium ($iEntryId) {
        ...
        $GLOBALS['oTopMenu']->setCustomVar($this->_sPrefix.'_view_uri', $aDataEntry[$this->_oDb->_sFieldUri]);
 		$GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
			_t('_'.$this->_sPrefix) => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'home/',
			$aDataEntry[$this->_oDb->_sFieldTitle] => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri],
			_t('_modzzz_formations_page_title_premium') => '',
		));
        ...
        $this->_oTemplate->addCss ('main.css');
        $this->_oTemplate->addCss ('forms_extra.css');
        $this->_oTemplate->pageCode(_t('_modzzz_formations_page_title_premium'));
	}
    ...
    function isAllowedRelist (&$aDataEntry, $isPerformAction = false) {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_RELIST, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;     
    } 
    ...
    function actionRelist ($iEntryId) {
        ...
        $GLOBALS['oTopMenu']->setCustomVar($this->_sPrefix.'_view_uri', $aDataEntry[$this->_oDb->_sFieldUri]);
  		$GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
			_t('_'.$this->_sPrefix) => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'home/',
			$aDataEntry[$this->_oDb->_sFieldTitle] => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri],
			_t('_modzzz_formations_page_title_relist') => '',
		));
        ...
        $this->_oTemplate->addCss ('main.css');
        $this->_oTemplate->addCss ('forms_extra.css');
        $this->_oTemplate->pageCode(_t('_modzzz_formations_page_title_relist'));
	}
    ...
    function isAllowedExtend (&$aDataEntry, $isPerformAction = false) {
        ...
        $this->_defineActions();
        $aCheck = checkAction($this->_iProfileId, BX_FORMATIONS_EXTEND, $isPerformAction);
        return $aCheck[CHECK_ACTION_RESULT] == CHECK_ACTION_RESULT_ALLOWED;     
    } 
    ...
    function actionExtend ($iEntryId) {
        ...
        $GLOBALS['oTopMenu']->setCustomVar($this->_sPrefix.'_view_uri', $aDataEntry[$this->_oDb->_sFieldUri]);
  		$GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
			_t('_'.$this->_sPrefix) => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'home/',
			$aDataEntry[$this->_oDb->_sFieldTitle] => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri],
			_t('_modzzz_formations_page_title_extend') => '',
		));
        ...
        $this->_oTemplate->addCss ('main.css');
        $this->_oTemplate->addCss ('forms_extra.css');
        $this->_oTemplate->pageCode(_t('_modzzz_formations_page_title_extend'));
	}
    ...
    function _actionExtend ($iEntryId) {
		...
		$aForm = array(
            ...
            'inputs' => array( 
                ...
                'submit' => array(
                    'type'  => 'submit',
                    'value' => _t('_modzzz_formations_extend'),
                    'name'  => 'submit_package',
                ),
            ),
        );
	...
	function actionMapMultiple($iCenterLat=0, $iCenterLng=0, $iRadius=0){
		...
		$iCount = (int)getParam('modzzz_formations_map_count');
		$sMeasurementType = getParam('modzzz_formations_measurement_type');
		...
		$query = sprintf("SELECT `p`.`title`, `p`.`uri`, `p`.`address1`,  `p`.`city`, `p`.`country`,`p`.`company_id`, `p`.`vacancies`, `p`.`role`,  `p`.`thumb`, `m`.`lat`, `m`.`lng`, ( %s * acos( cos( radians('%s') ) * cos( radians( `m`.`lat` ) ) * cos( radians( `m`.`lng` ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( `m`.`lat` ) ) ) ) AS distance FROM `modzzz_formations_profiles` AS `m` INNER JOIN `modzzz_formations_main` AS `p` ON (`p`.`id` = `m`.`id`) WHERE `m`.`failed` = 0 AND `p`.`status` = 'approved' HAVING distance < '%s' ORDER BY distance LIMIT 0 , %s",
		  $iCalcValue,
		...
		while ($row = @mysql_fetch_assoc($result)){
			  ...
			  $sRole = ($row['role']) ? $row['role'] : _t('_modzzz_formations_na'); 
			  $sRole = '<b>'._t('_modzzz_formations_role').'</b>: '.$sRole ;
			 
			  $iVacancies = ($row['vacancies']) ? $row['vacancies'] : 1;  
			  $sVacancies = '<b>'._t('_modzzz_formations_vacancies').'</b> [ '.$iVacancies.' ]';
			  
			  if($row['company_id']){ 
					$aCompany = $this->_oDb->getCompanyEntryById($row['company_id']);
					$sCompanyName = $aCompany['company_name'];
					$sCompanyUrl = 'm/formations/companyview/' . $aCompany['company_uri'];  
			  }
			 
			  $sCompany = '<b>'._t('_modzzz_formations_company').'</b>: <a href="'. $sCompanyUrl .'">'. $sCompanyName .'</a>, '. $sVacancies;
			 
			  $newnode->setAttribute("url", 'm/formations/view/'.$row['uri']);
	...
	function actionMapSingle($iItemId){
		...
		$query = sprintf("SELECT `p`.`title`, `p`.`uri`, `p`.`company_id`, `p`.`role`, `p`.`vacancies`, `p`.`address1`,  `p`.`city`, `p`.`country`, `p`.`thumb`, `m`.`lat`, `m`.`lng`, `p`.`city` FROM `modzzz_formations_profiles` AS `m` INNER JOIN `modzzz_formations_main` AS `p` ON (`p`.`id` = `m`.`id`) WHERE `m`.`failed` = 0 AND `p`.`status` = 'approved' AND `m`.`id` = '%s'", 
    ...
    function actionEmbed ($sUri) {
        ...
        $GLOBALS['oTopMenu']->setCustomVar($this->_sPrefix.'_view_uri', $aDataEntry[$this->_oDb->_sFieldUri]);
  		$GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
			_t('_'.$this->_sPrefix) => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'home/',
			$aDataEntry[$this->_oDb->_sFieldTitle] => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aDataEntry[$this->_oDb->_sFieldUri],
			_t('_modzzz_formations_page_title_embed_video') => '',
		));

        bx_import ('EmbedForm', $this->_aModule);
		$oForm = new BxFormationsEmbedForm ($this, $aDataEntry);
        $oForm->initChecker();
        ...
        echo $oForm->getCode ();
        $this->_oTemplate->addCss ('main.css');
        $this->_oTemplate->pageCode(_t('_modzzz_formations_page_title_embed_video') .' - '. $aDataEntry[$this->_oDb->_sFieldTitle]);
    }

	function isAllowedSeeker(){
		return (getParam('modzzz_formations_seeker_activated') == 'on') ? true : false; 
	}
	...
	function serviceSearch() {
		...
		$aFormationPostTypeList = $oProfileFields->convertValues4Input('#!FormationPostType');
        ...
        $aForm = array(
            ...
            'inputs' => array(
				...
				// Freddy Ajout Formation/Stage---post_type
				 ...
				 'Type' => array( 
                    ...
                    'name' => 'Type',
					 ...
					 'values'=> array(
					            'all' => _t('_modzzz_formations_all'),
								'provider'=>_t('_modzzz_formations_provider'),
								'seeker'=>_t('_modzzz_formations_seeker'),
								
							),
							
                    'caption' => _t('_modzzz_formations_form_caption_post_type'), 
                'attrs' => array (
                        'style' => 'width:135px',
                    ), 
                ), 
                ...
                'Parent' => array(
                    ...
                    'name' => 'parent',
					'values'=> $aCategories,
                    'caption' => _t('_modzzz_formations_parent_categories'),
					'attrs' => array(
        ...
        if ($oForm->isSubmittedAndValid ()) {
            ...
            // Freddy Ajout Formation/Stage---post_type 
			//$o = new BxFormationsSearchResult('search', $oForm->getCleanValue('Keyword'), $oForm->getCleanValue('Category'), $oForm->getCleanValue('City'), $oForm->getCleanValue('State'), $oForm->getCleanValue('Country') );
			$o = new BxFormationsSearchResult('search', $oForm->getCleanValue('Keyword'), $oForm->getCleanValue('Category'), $oForm->getCleanValue('City'), $oForm->getCleanValue('State'), $oForm->getCleanValue('Country'), $oForm->getCleanValue('Type'), $oForm->getCleanValue('Parent')  );
    ...
    function serviceGetMemberMenuItemAddContent ()
    {
        if (!$this->isAllowedAdd())
            return '';
        return parent::_serviceGetMemberMenuItem (_t('_modzzz_formations_single'), _t('_modzzz_formations_single'), 'credit-card', false, '&filter=add_formation');
    }
   ...
   /**
    ...
    function serviceMapInstall()
    {
        ...
        return BxDolService::call('wmap', 'part_install', array('formations', array(
            'part' => 'formations',
            'title' => '_modzzz_formations',
            'title_singular' => '_modzzz_formations_single',
            'icon' => 'modules/modzzz/formations/|map_marker.png',
            'icon_site' => 'credit-card',
            'join_table' => 'modzzz_formations_main',
            ...
            'join_field_privacy' => 'allow_view_formation_to',
            'permalink' => 'modules/?r=formations/view/',
        )));
    }
    ...
    {
        $aParams = array(
            'txt_object' => '_modzzz_formations_wall_object',
            'txt_added_new_single' => '_modzzz_formations_wall_added_new',
            'txt_added_new_plural' => '_modzzz_formations_wall_added_new_items',
            'txt_privacy_view_event' => 'view_formation',
    ...
    function actionPayFeature ($iEntryId) {
		...
		if(getParam('modzzz_formations_featured_credits')=='on'){ 
			...
			$sBuyCreditsLink = BX_DOL_URL_ROOT . $oCredit->_oConfig->getBaseUri() . 'purchase_credits'; 
			$iMoneyBalance = (int)$oCredit->getMemberCredits($this->_iProfileId);
			$iFeaturedCost = (int)$oCredit->_oDb->getActionValue('modzzz_formations', 'feature');
			if($iMoneyBalance < $iFeaturedCost){
				$sCode = _t('_modzzz_formations_msg_insufficient_credits_feature', number_format($iMoneyBalance,0), number_format($iFeaturedCost,0));
				$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyCreditsLink);
 			}
		}elseif(getParam('modzzz_formations_featured_points')=='on'){
			...
			$sBuyPointsLink = BX_DOL_URL_ROOT . $oPoint->_oConfig->getBaseUri() . 'purchase_points'; 
			$iMoneyBalance = $oPoint->_oDb->getMemberPoints($this->_iProfileId);
			$iFeaturedCost = (int)$oPoint->_oDb->getActionValue($this->_iProfileId, 'modzzz_formations', 'feature');
			if($iMoneyBalance < $iFeaturedCost){
				$sCode = _t('_modzzz_formations_msg_insufficient_points_feature', number_format($iMoneyBalance,0), number_format($iFeaturedCost,0));
				$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyPointsLink);
 			}
		}
		...
		if($sCode) {
			...
			$aVarsPopup = array (
				'title' => _t('_modzzz_formations_page_title_feature_item'),
				'content' => $sCode,
			);        
		...
		$aCustomForm = array(
			...
			'inputs' => array (
				...
				'header_info' => array(
					'type' => 'block_header',
					'caption' => _t("_modzzz_formations_form_caption_feature", $sTitle),
				), 
				'money_balance' => array(
					'type' => 'custom',
					'caption' => $sMoneyBalanceLabelC,
				...
				'feature_cost' => array(
					'type' => 'custom',
 					'caption' => _t("_modzzz_formations_form_caption_cost_to_feature"),
					'content' => number_format($iFeaturedCost,0) .' '. $sMoneyTypeC .' '. _t("_modzzz_formations_form_per_day"),
				),   
                'quantity' => array(
                    'caption'  => _t('_modzzz_formations_caption_num_featured_days'),
                    'type'   => 'select',
		...
		if ( $oForm->isSubmittedAndValid() ) {
			...
			if(getParam('modzzz_formations_featured_credits')=='on'){ 
				...
				if($iMoneyBalance < $iTotalCost){
					$sCode = _t('_modzzz_formations_msg_insufficient_credits_feature', number_format($iMoneyBalance,0), number_format($iTotalCost,0));
					$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyCreditsLink);
					echo MsgBox($sCode);
					return;
				}
				...
				if($iMoneyBalance < $iTotalCost){
					$sCode = _t('_modzzz_formations_msg_insufficient_points_feature', number_format($iMoneyBalance,0), number_format($iTotalCost,0));
					$sCode .= '&nbsp;'._t('_modzzz_formations_buy_link', $sBuyPointsLink);
					echo MsgBox($sCode);
					return;
				}
			...
			$sResultMsg = ($bSuccess) ? _t("_modzzz_formations_msg_feature_success") : _t("_modzzz_formations_msg_feature_failure"); 
    ...
    function isAllowedPurchaseFeatured ($iEntryId, $isPerformAction = false) {
		...
		if(getParam("modzzz_formations_featured_credits")!='on' && getParam("modzzz_formations_featured_points")!='on')
			return false;
    ...
    function _actionUploadLogoIcon ( $iEntryId=0 ) {
		...
		$iIconWidth = (int)getParam("modzzz_formations_icon_width");
		$iIconHeight = (int)getParam("modzzz_formations_icon_height"); 
		$sIcon = "";
