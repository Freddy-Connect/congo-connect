...
/*
 * Formations module Data
 */
class BxFormationsDb extends BxDolTwigModuleDb {	

	/*
	 * Constructor.
	 */
	function BxFormationsDb(&$oConfig) {
        ...
        $this->_sFieldDailyFormation = 'today_formation';
        ...
        $this->_sFieldAllowViewTo = 'allow_view_formation_to'; 
	...
	function getBusinessZip($iAuthorId=0) {
	 ...
	 // Freddy nombre des formations favorite et candidatures  pour un membre
  /* function CountFavoriteMember($iProfileId){
	...
	*/
	 ...
	 function CountFavoriteMember($iProfileId){
	    $iProfileId = (int)$iProfileId;
		return $this->getOne ("SELECT COUNT(`id_entry`) FROM `" . $this->_sPrefix . $this->_sTableFavorite. "` AS  `f` RIGHT JOIN `modzzz_formations_main` AS  `j` ON  `f`.`id_entry` =  `j`.`id`
		 WHERE `j`.`status`='approved' and  `f`.`id_profile`=$iProfileId LIMIT 1"); 
	
	} 
	
	
	function CountFormationApplyMember($iProfileId){
	    $iProfileId = (int)$iProfileId;
		return $this->getOne ("SELECT COUNT(`formation_id`) FROM `" . $this->_sPrefix . "apply`  WHERE `member_id`=$iProfileId LIMIT 1"); 
   ...
   //////////////////////////////////////////////////
	...
	function getRecruteurApplicationCount ($iProfileId ) {
        return $this->getOne ("SELECT  COUNT(`a`.`formation_id`) FROM  `modzzz_formations_apply` AS  `a` RIGHT JOIN  `modzzz_formations_main` AS  `j` ON  `a`.`formation_id` =  `j`.`id`  and `j`.`author_id` = '$iProfileId'");
    }
	
	function getRecruteur () {
        return $this->getAll ("SELECT DISTINCT  `b`.`id` ,`b`.`title`,`b`.`uri`, `b`.`author_id`,`b`.`thumb`,`b`.`city`,`b`.`zip`,`b`.`country`,`b`.`state` FROM  `modzzz_listing_main` AS  `b` INNER JOIN  `modzzz_formations_main` AS  `j` ON  `b`.`id` =  `j`.`company_id` 
		ORDER BY `j`.`created` DESC LIMIT 15");
    }
	
	function getGoToYourSpace($iAuthorId){ 
		return $this->getOne("SELECT COUNT(`id`) FROM `modzzz_formations_main` WHERE `author_id`= '$iAuthorId' AND `status`='approved'");  
	}
	...
	///////////// freddy integration classified , formations, events
	...
	function getModzzzxFormationsCount($iId){ 
		return $this->getOne("SELECT COUNT(`id`) FROM `modzzz_formations_main` WHERE `company_id`='$iId' AND `status`='approved'");  
	}
	...
	// Freddy ajout 
	function getModzzzCountFormationsFormation($iAuthorId){ 
		return $this->getOne("SELECT COUNT(`id`) FROM `modzzz_formations_main` WHERE `author_id`= '$iAuthorId' AND `post_type`='provider' AND `status`='approved' ");  
	}
	
	
	
	// Freddy nombre total des formation  d'une ecole
	function getFormationByBusiness($iId){ 
		return $this->getOne("SELECT COUNT(`id`) FROM `modzzz_formations_main` WHERE `company_id`='$iId' AND `status`='approved'");  
	}
	
	function getBusinessFromationById($iAuthorId){ 
		$iAuthorId = (int)$iAuthorId;
		return $this->getAll ("SELECT `id`,`title`,`uri` FROM `modzzz_formations_main` WHERE `company_type`='listing' AND `author_id`=$iAuthorId  AND `status`='approved' ORDER BY `created` DESC"); 
	}  
	////////
	
	
	function getCentreFormations () {
        return $this->getAll ("SELECT DISTINCT  `b`.`id` ,`b`.`title`,`b`.`uri`, `b`.`author_id`,`b`.`thumb`,`b`.`city`,`b`.`zip`,`b`.`country`,`b`.`state` FROM  `modzzz_listing_main` AS  `b` RIGHT JOIN  `modzzz_formations_main` AS  `j` ON  `b`.`id` =  `j`.`company_id` 
		ORDER BY `j`.`created` DESC LIMIT 6");
    }
	
	
	
	
	function getModzzzCountFormationsStage($iAuthorId){ 
		return $this->getOne("SELECT COUNT(`id`) FROM `modzzz_formations_main` WHERE `author_id`= '$iAuthorId' AND `post_type`='seeker' AND `status`='approved' ");  
	}
	

	function getBusinessFormationsById($iAuthorId){ 
		$iAuthorId = (int)$iAuthorId;
		return $this->getAll ("SELECT `id`,`title`,`uri` FROM `modzzz_formations_main` WHERE `company_type`='listing' AND `author_id`=$iAuthorId AND `post_type`='provider' AND `status`='approved' ORDER BY `created` DESC"); 
	}  
	
	function getBusinessStageById($iAuthorId){ 
		$iAuthorId = (int)$iAuthorId;
		return $this->getAll ("SELECT `id`,`title`,`uri` FROM `modzzz_formations_main` WHERE `company_type`='listing' AND `author_id`=$iAuthorId AND `post_type`='seeker' AND `status`='approved' ORDER BY `created` DESC"); 
	}  
	////
	...
	function getNumberList($iStart, $iEnd, $iIncrement=1){
		...
		//Freddy 06/07/2015  Obtenir le nombre d'application par formation
	function getApplicationsCount($iFormation_id)
	{
		return (int)$this->getOne("SELECT COUNT(`id`) FROM `" . $this->_sPrefix . "apply` WHERE `formation_id`='$iFormation_id'");
	}
  
   
  
    function isFormationAdmin($iEntryId, $iProfileId) {
   ...
   function getCompanyName($iId) {
    ...
    function addFormationAdmin($iEntryId, $aProfileIds) {
    ...
    function removeFormationAdmin($iEntryId, $aProfileIds) {
		if (!$aProfileIds) 
			return false;
        if (is_array($aProfileIds)) {
   ...
   function deleteCompanyEntryByIdAndOwner ($iId, $iOwner, $isAdmin) {
    ...
    function getCompanyList($iAuthorId=0) {
		 ...
		 $arr = array();
		//Freddy
		 //$arr['0'] = _t('_modzzz_formations_select_company');
		 foreach($aCompanies as $aEachCompany){
			$arr['formation|'.$aEachCompany['id']] = $aEachCompany['company_name'];
		 }
  		  //Freddy
		// $arr['-99'] = _t('_modzzz_formations_add_company');
    ...
    function getMergedCompanyList($iAuthorId=0) {
        ...
        $arr = array();
		 ...
		 $aListings = $this->getAll ("SELECT `id`, `title` FROM `modzzz_schools_main` WHERE `status` = 'approved' AND `author_id` = '$iAuthorId' ORDER BY `title`");
		 */
		// FREDDY business remplacé par school
		//  $aListings = $this->getAll ("SELECT `id`, `title`, `status` FROM `modzzz_schools_main` WHERE  `author_id` = '$iAuthorId' ORDER BY `title`");
	...
	function getLocalCategoryCount($iParent, $sCountry='', $sState=''){
		...
		if (!$GLOBALS['logged']['admin']){ 
			if ($GLOBALS['logged']['member']){ 
			    ...
			    //$sExtraCheck = " AND formation.`membership_view_filter` IN ('', '$iMembershipId')"; 
			}else{
				//$sExtraCheck = "AND formation.`membership_view_filter`=''";
			}
		}
 
		if($sCountry)
			$sExtraCheck .= " AND formation.`country`='$sCountry'";

		if($sState)
			$sExtraCheck .= " AND formation.`state`='$sState'";

		return  $this->getOne("SELECT count(formation.`id`) FROM `" . $this->_sPrefix . "categ` cat LEFT JOIN `" . $this->_sPrefix . "main` formation ON cat.id=formation.`category_id` WHERE cat.`active` = 1 AND cat.`parent`=$iParent AND formation.`status`='approved' {$sExtraCheck}"); 
	}
 ...
 */
	...
	function getCategoryCount($iCategoryId){
		...
		return  $this->getOne("SELECT count(formation.`id`) FROM `" . $this->_sPrefix . "categ` cat LEFT JOIN `" . $this->_sPrefix . "main` formation ON cat.id=formation.`category_id` WHERE cat.`active` = 1 AND cat.`id`=$iCategoryId AND formation.`status`='approved' {$sExtraCheck}"); 
	}
	...
	function getFormationCount($iCompanyId)
	{
 	...
 	function postCompany($iEntryId, $iProfileId) {
		...
		$sCompanyUri = uriGenerate($sCompanyName, 'modzzz_formations_companies', 'company_uri'); 
	...
	function getCompanyIcon($iId, $sName, $bUrlOnly=false, $bLarge=false ) {
		...
		if($bLarge){
			$iWidth = getParam("modzzz_formations_company_image_width");
			$iWidth = getParam("modzzz_formations_company_image_height"); 
		}else{ 
			$iWidth = getParam("modzzz_formations_company_icon_width");
			$iWidth = getParam("modzzz_formations_company_icon_height"); 
		}
    ...
    function getDailyFormationItem () {
        $sWhere = " AND `{$this->_sFieldDailyFormation}` = '1' ";   
	...
	function markAsToday ($iId) {
		...
		$this->query ("UPDATE `" . $this->_sPrefix . $this->_sTableMain . "` SET `{$this->_sFieldDailyFormation}` = 0 WHERE `{$this->_sFieldId}` != $iId");
		
		return $this->query ("UPDATE `" . $this->_sPrefix . $this->_sTableMain . "` SET `{$this->_sFieldDailyFormation}` = (`{$this->_sFieldDailyFormation}` - 1)*(`{$this->_sFieldDailyFormation}` - 1) WHERE `{$this->_sFieldId}` = $iId LIMIT 1");   
    }
	...
	function saveApplication($iProfileId, $iEntryId, $sMessage,$sEmail, $sTelephone, $sResume, $iResumeId=0){
		...
		$this->query ("INSERT INTO `" . $this->_sPrefix . $this->_sTableApply . "` SET `member_id`=$iProfileId, `formation_id` =$iEntryId,  `resume_id` =$iResumeId, `letter`='$sMessage', `resume`='$sResume',  `telephone`='$sTelephone' , `Email`='$sEmail', `created`=$iTime"); 
	}
    ...
    function getApplications(&$aProfiles, $iEntryId, $iStart, $iMaxNum) {
        ...
        $aProfiles = $this->getAll ("SELECT SQL_CALC_FOUND_ROWS * FROM `" . $this->_sPrefix . $this->_sTableApply . "` WHERE `formation_id` = '$iEntryId'  ORDER BY `created` DESC LIMIT $iStart, $iMaxNum");
	...
	function hasApplied($iProfileId, $iEntryId){
		...
		return $this->getOne ("SELECT COUNT(`member_id`) FROM `" . $this->_sPrefix . $this->_sTableApply . "` WHERE `member_id`='$iProfileId' AND `formation_id` ='$iEntryId'");   
	}
	...
	function removeApplications($iEntryId, $iAppId=0){
		...
		$aEntries = $this->getAll ("SELECT `resume` FROM `" . $this->_sPrefix . $this->_sTableApply . "` WHERE `formation_id`=$iEntryId {$sQuery}"); 
		...
		$this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableApply . "` WHERE `formation_id`=$iEntryId {$sQuery}");   
	}
    ...
    function deleteEntryByIdAndOwner ($iId, $iOwner, $isAdmin) {
        if ($iRet = parent::deleteEntryByIdAndOwner ($iId, $iOwner, $isAdmin)) {
            ...
            $this->query ("DELETE FROM `" . $this->_sPrefix . "invoices` WHERE `formation_id` = $iId");
	...
	function processExpiringFormations() { 
		...
		$iNumNotifyDays = (int)getParam("modzzz_formations_email_expiring");
		if($iNumNotifyDays){
			$iNotifyTime = $SECONDS_IN_DAY * $iNumNotifyDays;
			$iTriggerTime = $iTime - $iExpireTime + $iNotifyTime;

			$aFormations = $this->getAll("SELECT `id`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND  `created` <= {$iTriggerTime} AND `pre_expire_notify`=0");
  
			foreach ($aFormations as $aEachFormation) {
				$iEntryId = $aEachFormation['id'];
				$iRecipientId = $aEachFormation['author_id'];
		  
				if(getParam("modzzz_formations_activate_expiring")=='on') { 
					$this->alertOnAction('modzzz_formations_expiring', $iEntryId, $iRecipientId, $iNumNotifyDays);
				}
	...
	function getFormationApplicants ($iEntryId) {
        return $this->getAll ("SELECT DISTINCT `p`.`ID`, `p`.`Email` FROM `" . $this->_sPrefix . $this->_sTableApplicants . "` AS `app` INNER JOIN `Profiles` as `p` ON (`app`.`formation_id` = '$iEntryId' AND `app`.`member_id` = `p`.`ID` AND `p`.`Status` = 'Active')");
    }    
	...
	function setInvoiceStatus($iItemId, $sStatus) {
 		 ...
 		 $this->query("UPDATE `" . $this->_sPrefix . "invoices` SET `invoice_status`='$sStatus' WHERE `formation_id`='$iItemId'"); 
	}
    ...
    function saveTransactionRecord($iBuyerId, $iFormationId, $sTransNo, $sTransType) {
        $iBuyerId    = (int)$iBuyerId;
        $iFormationId  = (int)$iFormationId; 
   
		$aDataEntry = $this->getEntryById($iFormationId);
		...
		$iOrderId = $this->lastId();
		 ...
		 //if order successful, update expiration date of formation
		 if($iOrderId){
			$aInvoice = $this->getInvoiceByNo($sInvoiceNo, false);
			$iDays = (int)$aInvoice['days'];
			$this->updateEntryExpiration($iFormationId, $iDays);

			$iPackageId = (int)$aInvoice['package_id'];
			$aPackage = $this->getPackageById($iPackageId); 
			$iFeatured = (int)$aPackage['featured'];
			if($iFeatured){
				$this->updateFeaturedStatus($iFormationId);
			}
		 }
	...
	function createInvoice($iEntryId, $iPackageId, $fPrice, $iDays, $sStatus='pending'){
		 ...
		 $iInvoiceActiveDays = (int)getParam('modzzz_formations_invoice_valid_days');
    ...
    function getInvoiceByNo($sInvoiceNo, $bCheckStatus=true) {  
        ...
        return $this->getRow("SELECT `invoice_no`, `price`, `days`, `formation_id`, `package_id`, `invoice_status`, `invoice_due_date`, `invoice_expiry_date`, `invoice_date` FROM `" . $this->_sPrefix . "invoices` WHERE `invoice_no` = '{$sInvoiceNo}' {$sStatusCheck}"); 
    }
	...
	/*featured functions*/ 
    function saveFeaturedTransactionRecord($iBuyerId, $iFormationId, $iQuantity, $fPrice, $sTransId, $sTransType) {
        $iBuyerId    = (int)$iBuyerId;
        $iFormationId  = (int)$iFormationId; 
		...
		if($bProcessed){
			$this->alertOnAction('modzzz_formations_featured_admin_notify', $iFormationId, $iBuyerId, $iQuantity, true);
  
			$this->alertOnAction('modzzz_formations_featured_buyer_notify', $iFormationId, $iBuyerId, $iQuantity);
		}
	...
	function processFormations(){ 
		$this->processExpiredFormations();
			
		$this->processExpiredInvoices();
 
		$this->processFeaturedFormation(); 
	}
	...
	function processExpiredInvoices() { 
		...
		$aFormation = $this->getAll("SELECT `formation_id` FROM `" . $this->_sPrefix . "invoices` WHERE `invoice_status`='pending' AND `invoice_expiry_date`>0 AND `invoice_expiry_date`<={$iExpireTime}");
		 
		foreach ($aFormation as $aEachFormation) {
			$iEntryId = (int)$aEachFormation['formation_id']; 
 
			$this->query("DELETE FROM `" . $this->_sPrefix . "main` WHERE `id`=$iEntryId");  
			
			$this->query("DELETE FROM `" . $this->_sPrefix . "invoices` WHERE `formation_id`=$iEntryId");  
 		}  
 
	}
 
	function processExpiredFormations() { 
 
 		$iTime = time();
 		$SECONDS_IN_DAY = 86400; 
		
        $bFreeListing = (getParam('modzzz_formations_paid_active')=='on') ? false : true;  
        $iNumActiveDays = (int)getParam("modzzz_formations_free_expired");
        if($bFreeListing && (!$iNumActiveDays))
            return; 

		if(getParam('modzzz_formations_activate_expired') == 'on') {
  			 
			$aFormation = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `featured`=0 AND `expiry_date`>0 AND `expiry_date`<={$iTime}");
			 
			foreach ($aFormation as $aEachFormation) {
				$iEntryId = (int)$aEachFormation['id']; 
				$iRecipientId = (int)$aEachFormation['author_id']; 

				$this->query("UPDATE `" . $this->_sPrefix . "main` SET `status`='expired'  WHERE `id`=$iEntryId");  
 
		 		$this->alertOnAction('modzzz_formations_expired', $iEntryId, $iRecipientId);
			}  
		}else{ 
		...
		if($iNumNotifyDays){
			...
			$aFormation = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `expiry_date` >= {$iTriggerTime} AND `expiry_date` < {$iTime}  AND `pre_expire_notify`=0");
		...
		$iNumNotifyDays = (int)getParam("modzzz_formations_email_expired");
		if($iNumNotifyDays){
			$iNotifyTime = $SECONDS_IN_DAY * $iNumNotifyDays;
			$iTriggerTime = $iTime - $iNotifyTime;

			$aFormation = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='expired' AND `expiry_date` <= {$iNotifyTime} AND `post_expire_notify`=0");
			...
			foreach ($aFormation as $aEachFormation) {
				...
				if(getParam("modzzz_formations_activate_expired")=='on') { 
					$this->alertOnAction('modzzz_formations_post_expired', $iEntryId, $iRecipientId, $iNumNotifyDays);
				}
			}
		}
	...
	function processFeaturedFormation(){
		
		if(getParam('modzzz_formations_buy_featured') != 'on') return;
			 
		$iTime = time();
   
        $aFormation = $this->getAll("SELECT `id`, `author_id`, `featured`, `featured_expiry_date` FROM `" . $this->_sPrefix . "main` WHERE `featured`=1 AND `featured_expiry_date`>0 AND `featured_expiry_date` <= $iTime"); 

		foreach($aFormation as $aEachList){
  
			$iFormationId = (int)$aEachList['id'];
			$iRecipientId = (int)$aEachList['author_id'];

			$this->alertOnAction('modzzz_formations_featured_expire_notify', $iFormationId, $iRecipientId  );
		
	        $this->query("UPDATE `" . $this->_sPrefix . "main` SET `featured`=0, `featured_expiry_date`=0, `featured_date`=0  WHERE `id`=$iFormationId"); 
		}

	}

	function alertOnAction($sTemplate, $iFormationId, $iRecipientId=0, $iDays=0, $bAdmin=false) {
	...
	function getFormationList($iAuthorId){
		$iAuthorId = (int)$iAuthorId;

		$aList = $this->getAll("SELECT `id`, `title` FROM `modzzz_formations_main` WHERE `post_type`='provider' AND `status`='approved' AND `author_id`=$iAuthorId");  
	...
	function hasFormationListings($iProfileId){
		$iProfileId = (int)$iProfileId;
	...
	function deductPayment($sAction, $sPaymentType, $iProfileId, $iEntryId, $iDays, $iPrice){
		...
		if($sPaymentType=="points") {  
			$oPoint = BxDolModule::getInstance('BxPointModule'); 
 			$oPoint->assignPoints($iProfileId, 'modzzz_formations', $sAction, "subtract", time(), abs($iPrice)); 
		}
		
		//integration with credits
		if($sPaymentType=="credits") {  
			$oCredit = BxDolModule::getInstance('BxCreditModule'); 
 			$oCredit->assignCredits($iProfileId, 'modzzz_formations', $sAction, "subtract", time(), abs($iPrice));  
		} 
	...
	function getLogo($iId, $sName, $bUrlOnly=false, $bUseAsIcon=false){
		...
		if($bUseAsIcon){
			...
			$iWidth = getParam("modzzz_formations_icon_width");
			$iHeight = getParam("modzzz_formations_icon_height");
		}
