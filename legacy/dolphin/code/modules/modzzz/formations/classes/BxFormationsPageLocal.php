...
class BxFormationsPageLocal extends BxDolTwigPageMain {
 		var $_oDb;
        var $_oConfig; 
		var $sCountry; 
		var $sState; 
    ...
    function BxFormationsPageLocal(&$oMain, $sCountry, $sState, $sCategory) {
		...
		if($sCountry)
			parent::BxDolTwigPageMain('modzzz_formations_local_state', $oMain); 
		else	
			parent::BxDolTwigPageMain('modzzz_formations_local', $oMain); 

	}
 
    function getBlockCode_StateFormations() {
  
		if($this->sState){ 	 
			
			$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'local/'. $this->sCountry .'/'. $this->sState . ($this->sCategory  ? '/'. $this->sCategory : '') . '?';

			$sFormations = $this->ajaxBrowse('local_state', $this->oDb->getParam('modzzz_formations_perpage_main_recent'), array(), $this->sCountry, $this->sState, $this->sCategory);
			...
			$sFormations = $this->ajaxBrowse('local_country', $this->oDb->getParam('modzzz_formations_perpage_main_recent'), array(), $this->sCountry,   $this->sCategory); 
		} 
   ...
   function getBlockCode_Categories() {
		...
		if($this->sState){  
			...
			$sItemLabel  = _t('_modzzz_formations_category_drilldown_for_item', $sItemName);
    ...
    function getBlockCode_States() {
	    ...
	    return $this->oTemplate->parseHtmlByName('formations_states', $aVars);   
	}
    ...
    function getBlockCode_Region() {
	    ...
	    return $this->oTemplate->parseHtmlByName('formations_regions', $aVars);  
	}
