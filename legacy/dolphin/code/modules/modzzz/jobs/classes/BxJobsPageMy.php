...
class BxJobsPageMy extends BxDolPageView {	
    ...
    function getBlockCode_Owner() {        
        ...
        $aMenu = array(
		 ...
		 _t('_modzzz_jobs_block_submenu_orders') => array('href' => $sBaseUrl . '&filter=my_orders', 'active' => 'my_orders' == $_REQUEST['filter']),
		 			
		_t('_modzzz_jobs_block_submenu_pending_invoices') => array('href' => $sBaseUrl . '&filter=my_pending_invoices', 'active' => 'my_pending_invoices' == $_REQUEST['filter']),
 ...
 $Application_count = $this->_oDb->getApplicationsCount($aEachJob['id']);
    ...
    function getBlockCode_Expired() {
        ...
        if (!$sForm)
            return MsgBox(_t('_Empty_jobs_expire'));
    ...
    function getBlockCode_Pending() {
        ...
        if (!$sForm)
            return MsgBox(_t('_Empty_jobs_en_attente'));
    ...
    function getBlockCode_Invoices() {
        ...
        if (!$sForm)
            return MsgBox(_t('_Empty_jobs_commande_attente'));
    ...
    function getBlockCode_Orders() {
        ...
        if (!$sForm)
            return MsgBox(_t('_Empty_jobs_invoices'));
