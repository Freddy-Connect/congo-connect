...
class BxJobsModule extends BxDolTwigModule {
    ...
    function actionPopupLetter ($iId) {
		...
		$aVarsPopup = array (
            'title' => _t('_modzzz_jobs_cover_letter'),
            'content' => $sContent,
        );        
    ...
    function _actionApplications ($sUri, $iPerPage, $sUrlBrowse, $sTitle) {
        ...
        foreach ($aProfiles as $aEachApplicant) { 
			...
			if($aEachApplicant['resume']){
				$sDownloadLink = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'download_resume/' . $aEachApplicant['job_id'] .'/'. $aEachApplicant['resume'];
    ...
    function _actionApply ($iEntryId, $sTitle, $sMsgSent) {
        ...
        if ($oForm->isSubmittedAndValid ()) {
			...
			$sAttachInboxMessage = ($sResume) ? _t('_modzzz_jobs_download_resume_online_at', $sDownloadLink) : '';
			...
			if(getParam('modzzz_resume_job_connect')=='on'){
				...
				if($iResumeId){
					...
					$sAttachInboxMessage .= _t('_modzzz_jobs_view_resume_online_at') . $sResumeLink;
				} 
			}
