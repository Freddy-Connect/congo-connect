...
class BxJobsDb extends BxDolTwigModuleDb {	
   ...
   function CountFavoriteMember($iProfileId){
		...
		return $this->getOne ("SELECT COUNT(`id_entry`) FROM `" . $this->_sPrefix . $this->_sTableFavorite. "` AS  `f` RIGHT JOIN `modzzz_jobs_main` AS  `j` ON  `f`.`id_entry` =  `j`.`id`
	...
	function CountJobApplyMember($iProfileId){
	    $iProfileId = (int)$iProfileId;
		return $this->getOne ("SELECT COUNT(`a`.`job_id`) FROM  `modzzz_jobs_apply` AS  `a` RIGHT JOIN `modzzz_jobs_main` AS  `j` ON  `a`.`job_id` =  `j`.`id`
		WHERE `j`.`status`='approved' and `member_id`=$iProfileId LIMIT 1 "); 
	} 
   ...
   //////////////////////////////////////////////////
	...
	function getRecruteurApplicationCount ($iProfileId ) {
        return $this->getOne ("SELECT  COUNT(`a`.`job_id`) FROM  `modzzz_jobs_apply` AS  `a` RIGHT JOIN  `modzzz_jobs_main` AS  `j` ON  `a`.`job_id` =  `j`.`id`  and `j`.`author_id` = '$iProfileId' AND `j`.`status`='approved'");
    }
	
	function getRecruteur () {
        return $this->getAll ("SELECT DISTINCT `b`.`icon` , `b`.`id` ,`b`.`title`,`b`.`uri`, `b`.`author_id`,`b`.`thumb`,`b`.`city`,`b`.`zip`,`b`.`country`,`b`.`state`,`b`.`employees_count`,`b`.`category_id`,`b`.`parent_category_id` FROM  `modzzz_listing_main` AS  `b` INNER JOIN  `modzzz_jobs_main` AS  `j` ON  `b`.`id` =  `j`.`company_id` WHERE  `j`.`status`='approved'
