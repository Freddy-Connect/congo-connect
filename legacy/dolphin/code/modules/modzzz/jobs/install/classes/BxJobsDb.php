...
class BxJobsDb extends BxDolTwigModuleDb {	
   ...
   //////////////////////////////////////////////////
	...
	function getRecruteur () {
        return $this->getAll ("SELECT DISTINCT  `b`.`id` ,`b`.`title`,`b`.`uri`, `b`.`author_id`,`b`.`thumb`,`b`.`city`,`b`.`zip`,`b`.`country`,`b`.`state`,`b`.`employees_count`,`b`.`category_id`,`b`.`parent_category_id` FROM  `modzzz_listing_main` AS  `b` RIGHT JOIN  `modzzz_jobs_main` AS  `j` ON  `b`.`id` =  `j`.`company_id` 
 ...
 */
	...
	function saveApplication($iProfileId, $iEntryId, $sMessage, $sResume, $iResumeId=0){
		
		$iTime = time();
		
		$this->query ("INSERT INTO `" . $this->_sPrefix . $this->_sTableApply . "` SET `member_id`=$iProfileId, `job_id` =$iEntryId,  `resume_id` =$iResumeId, `letter`='$sMessage', `resume`='$sResume', `created`=$iTime"); 
	}
