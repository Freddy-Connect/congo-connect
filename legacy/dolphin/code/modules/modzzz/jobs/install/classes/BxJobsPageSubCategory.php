...
class BxJobsPageSubCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxJobsPageSubCategory(&$oMain, $sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_jobs_subcategory', $oMain);
    ...
    function getBlockCode_SubCategoryJobs() {

		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'subcategories/'.$this->aCategoryInfo['uri'].'?';
 
        $sCode = $this->ajaxBrowse('subcategories', $this->oDb->getParam('modzzz_jobs_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
