...
class BxJobsCompanyPageView extends BxDolTwigPageView {	

	function BxJobsCompanyPageView(&$oMain, &$aDataEntry) {
		parent::BxDolTwigPageView('modzzz_jobs_company_view', $oMain, $aDataEntry);
    ...
    function getBlockCode_Rate() {
        modzzz_jobs_import('CompanyVoting');
        $o = new BxJobsCompanyVoting ('modzzz_jobs_company', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedCompanyRate($this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_jobs_import('CompanyCmts');
        $o = new BxJobsCompanyCmts ('modzzz_jobs_company', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $aInfo = array (
                ...
                'TitleEdit' => $this->_oMain->isAllowedCompanyEdit($this->aDataEntry) ? _t('_modzzz_jobs_action_title_edit') : '',
                'TitleDelete' => $this->_oMain->isAllowedCompanyDelete($this->aDataEntry) ? _t('_modzzz_jobs_action_title_delete') : '',
 				'TitleInvite' => $this->_oMain->isAllowedCompanySendInvitation($this->aDataEntry) ? _t('_modzzz_jobs_action_title_invite') : '',
                'TitleShare' => $this->_oMain->isAllowedShare($this->aDataEntry) ? _t('_modzzz_jobs_action_title_share') : '',
                'TitleBroadcast' => $this->_oMain->isAllowedCompanyBroadcast($this->aDataEntry) ? _t('_modzzz_jobs_action_title_broadcast') : '',
                'AddToFeatured' => $this->_oMain->isAllowedCompanyMarkAsFeatured($this->aDataEntry) ? ($this->aDataEntry['featured'] ? _t('_modzzz_jobs_action_remove_from_featured') : _t('_modzzz_jobs_action_add_to_featured')) : '' 
			);

            if (!$aInfo['TitleEdit'] && !$aInfo['TitleDelete'] && !$aInfo['TitleInvite'] && !$aInfo['TitleShare'] && !$aInfo['TitleBroadcast'] && !$aInfo['AddToFeatured']) 
                return '';

            return $oFunctions->genObjectsActions($aInfo, 'modzzz_jobs_company');
        } 
	...
	function getBlockCode_Jobs() {   
		...
		return $this->ajaxBrowse('company_jobs', $this->_oDb->getParam('modzzz_jobs_perpage_main_recent'),array(),$this->aDataEntry['id']); 
	}
