...
class BxJobsTemplate extends BxDolTwigTemplate {
    ...
    function unit ($aData, $sTemplateName, &$oVotingView, $isShort=false) {
	 ...
	 /*
		...
		*/
		...
		//////Fin freddy job location
		
		// Freddy pour afficher les candidatures que lorsque le count est superier à zero
		...
		$aMax_Salary = '&nbsp;'. '-' . '&nbsp;'. $aData['max_salary'] . '€' ;  
		}
		elseif ($aData['max_salary']){
			$aMax_Salary = '&nbsp;'. $aData['max_salary'] . '€' ;  
	    }
		else{
			$aMax_Salary = _t('_modzzz_jobs_upwards');
		}
		
		
		// Freddy Post_Type
		if ($aData['post_type']=='seeker'){
			
			 $aPost_Type_Stage =  '<i class="sys-icon user"></i>'.'&nbsp;' ._t('_modzzz_jobs_post_type_stage');
		}
		 else{
			 $aPost_Type_Stage =  '<i class="sys-icon building"></i>' ;
		}
		...
		//// Freddy add
			...
			'AddToFavorite' => $this->_oMain->isAllowedMarkAsFavorite($aData) ? ($this->_oMain->isFavorite($this->_oMain->_iProfileId, $aData['id']) ? _t('_modzzz_jobs_action_supprimer_favorite') :  _t('_modzzz_jobs_action_add_to_favorite')) : '',
			'Sauvegardee' => $this->_oMain->isAllowedMarkAsFavorite($aData) ? ($this->_oMain->isFavorite($this->_oMain->_iProfileId, $aData['id']) ? '<i class="sys-icon heart"></i>' . ' ' .  _t('_modzzz_jobs_action_afficher_job_favorite') : '') : '',
			 ...
			 /* 'bx_if:Application_count' => array( 
								'condition' => $application_count >=1,
								
								'content' => array(
             'Application_count'=> $application_count,
			 'Application_caption'=> _t('_modzzz_jobs_form_unit_view_applications') ? _t('_modzzz_jobs_form_unit_view_applications') : _t('_modzzz_jobs_Be_one_of_the_first_ten_applicants'),
								), 
							),
				*/
		...
		//////////////////////Fin Freddy add
			...
			'bx_if:salary' => array( 
				...
				'content' => array(
					//'salary_minimum' => $sCurrency .''. number_format($aData['min_salary'], 2),
					'salary_minimum' =>  $aMin_Salary ? $aMin_Salary : '' ,
					//'salary_maximum' => ($aData['max_salary']) ? $sCurrency .''. number_format($aData['max_salary'], 2) : _t('_modzzz_jobs_upwards'),
					//'salary_maximum' => $aMax_Salary ? $aMax_Salary : _t('_modzzz_jobs_upwards'),
					'salary_maximum' => $aMax_Salary,
			...
			*/
			...
			'bx_if:salary' => array( 
				'condition' => ($aData['max_salary'] || $aData['min_salary']),
				'content' => array(
					'salary_minimum' => ($aData['min_salary']) ? $sCurrency .''. number_format($aData['min_salary'], 2) : '-',
					'salary_maximum' => ($aData['max_salary']) ? $sCurrency .''. number_format($aData['max_salary'], 2) : _t('_modzzz_jobs_upwards'),
					'salary_type' => _t($GLOBALS['aPreValues']['JobSalaryType'][$aData['salary_type']]['LKey']),
				) 
			), 
			...
			'bx_if:negotiable_salary' => array( 
				'condition' => ($aData['max_salary'] &&  $aData['min_salary']),
				'content' => array(
					'negotiable_salary' => _t('_modzzz_jobs_Salary_Negotiable'),
    ...
    function blockDesc (&$aDataEntry) {
		...
		///Freddy add   Ne pas Afficher Apply
		 $aProfile = getProfileInfo($this->_oMain->_iProfileId);
          if($this->_oDb->hasApplied($aProfile['ID'],$aDataEntry['id']))
		  { 
		  $smessage_applicant = '<i class="sys-icon hand-peace-o"></i>'. _t('_modzzz_jobs_already_applied_for') ? '<i class="sys-icon user"></i>' . _t('_modzzz_jobs_already_applied_for') : '';
		  }
		  ///////////////////
		...
		$aVars = array (
			...
			////// Freddy add
			 'Application_count' => $sApplication_count = $this->_oDb->getApplicationsCount($aDataEntry['id']),
    			'bx_if:skills' => array( 
			...
			/* Freddy commentaire
			'bx_if:salary' => array( 
				'condition' => ($aDataEntry['max_salary'] || $aDataEntry['min_salary']),
				'content' => array(
					'salary_minimum' => $aDataEntry['currency'] .''. number_format($aDataEntry['min_salary'], 2),
					'salary_maximum' => ($aDataEntry['max_salary']) ? $aDataEntry['currency'] .''. number_format($aDataEntry['max_salary'], 2) : _t('_modzzz_jobs_upwards'),
					'salary_type' => _t($GLOBALS['aPreValues']['JobSalaryType'][$aDataEntry['salary_type']]['LKey']),
				) 
			), 
			*/ 
