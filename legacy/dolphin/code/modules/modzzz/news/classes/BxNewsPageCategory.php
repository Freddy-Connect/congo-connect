...
class BxNewsPageCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxNewsPageCategory(&$oMain,$sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_news_category', $oMain);
		...
		if(isset($_GET['ajax']) && ($_GET['ajax']=='sort')){
			echo $this->getBlockCode_CategoryNews(); 
			exit;
		} 
    ...
    function getBlockCode_CategoryNews() {

 		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories/'.$this->aCategoryInfo['uri'].$this->oMain->getConcatenator();

        $sCode = $this->ajaxBrowse('categories', $this->oDb->getParam('modzzz_news_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
