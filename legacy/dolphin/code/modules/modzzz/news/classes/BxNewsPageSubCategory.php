...
class BxNewsPageSubCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxNewsPageSubCategory(&$oMain, $sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_news_subcategory', $oMain);
    ...
    function getBlockCode_SubCategoryNews() {

 		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'subcategories/'.$this->aCategoryInfo['uri'].$this->oMain->getConcatenator();

        $sCode = $this->ajaxBrowse('subcategories', $this->oDb->getParam('modzzz_news_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
