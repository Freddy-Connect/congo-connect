 ...
 * News module Data
 */
class BxNewsDb extends BxDolTwigModuleDb {	
	...
	function BxNewsDb(&$oConfig) {
        ...
        $this->_sFieldLetter = 'letter';  

        $this->_sTableAdmins = 'admins';
        $this->_sFieldAllowViewTo = 'allow_view_news_to';
	}
    ...
    function getEntriesByMonth ($iYear, $iMonth, $iNextYear, $iNextMonth) {
		...
		return $this->getAll ("SELECT *, DAYOFMONTH(FROM_UNIXTIME(`{$this->_sFieldPublished}`)) AS `Day`
            ...
            WHERE `{$this->_sFieldPublished}` >= UNIX_TIMESTAMP('$iYear-$iMonth-1') AND `{$this->_sFieldPublished}` < UNIX_TIMESTAMP('$iNextYear-$iNextMonth-1') AND `{$this->_sFieldStatus}` = 'approved' AND `language` IN (0, $ilangId)"); 
    }
	...
	function countMonthPosts($iYear, $iMonth=-1){
		...
		return (int)db_value("SELECT COUNT(`id`) FROM `" . $this->_sPrefix . "main` WHERE  YEAR(FROM_UNIXTIME(`when`))='{$iYear}' {$sSqlExtra} AND `status`='approved' AND `language` IN (0, $ilangId)"); 
	}
	...
	function getSubscribeCount($iEntryId){ 
		...
		return (int)$this->getOne("SELECT COUNT(DISTINCT `subscriber_id`) FROM `sys_sbs_entries` WHERE `object_id`='$iEntryId' AND `subscription_id` IN (SELECT `id` FROM `sys_sbs_types` WHERE `unit`='modzzz_news')"); 
	}
	...
	function SaveCategory($iParent=0){
		...
		$this->query("INSERT INTO `" . $this->_sPrefix . "categ` SET $sExtraAdd `uri`='$sCatURI', `name`='$sCategory'");
	...
	function sqlAfter(){
		...
		if(mysql_num_rows(mysql_query("SHOW TABLES LIKE 'bx_news_comments_bak'"))==0) return;

		$this->query("INSERT INTO `modzzz_news_cmts` SELECT * FROM `bx_news_comments_bak`");
		$this->query("INSERT INTO `modzzz_news_cmts_track` SELECT * FROM `bx_news_comments_track_bak`");
		$this->query("INSERT INTO `modzzz_news_rating` SELECT * FROM `bx_news_voting_bak`");
		$this->query("INSERT INTO `modzzz_news_rating_track` SELECT * FROM `bx_news_voting_track_bak`");
		$this->query("INSERT INTO `modzzz_news_views_track` SELECT * FROM `bx_news_views_track_bak`");
		 

		$this->query("  
		INSERT INTO `modzzz_news_main` 
		...
		$this->query("
		UPDATE `modzzz_news_main` SET
			allow_view_news_to=3, 		
			allow_comment_to=3, 		
			allow_rate_to=3 
		");
 
		$this->query("INSERT INTO `modzzz_news_categ` (`parent`, `name`, `uri`, `icon`, `active`) VALUES (0, 'Main', 'main', '', 1)"); 

		$aCategories = $this->getAll("SELECT DISTINCT `Category` FROM `sys_categories` WHERE `Type`='bx_news' AND `ID`=0");
		foreach($aCategories as $aEachCategory){
		 
			$sCategory = process_db_input($aEachCategory['Category']);
			$sCategoryUri = strtolower(uriGenerate($aEachCategory['Category'], 'modzzz_news_categ', 'uri'));

			$this->query("INSERT INTO `modzzz_news_categ` (`parent`, `name`, `uri`, `icon`, `active`) VALUES (1, '$sCategory', '$sCategoryUri', '', 1)"); 
		 
			$iSubCategoryId = $this->lastId();

			$this->query("UPDATE `modzzz_news_main` SET `parent_category_id`=1, `category_id`=$iSubCategoryId WHERE `categories` LIKE '%$sCategory%' AND `parent_category_id`=0"); 
		}

		$iAdminId = (int)$this->getOne("SELECT `ID` FROM `Profiles` WHERE `Role`=3 LIMIT 1");
		$this->query("UPDATE `modzzz_news_main` SET `author_id`=$iAdminId WHERE `author_id`=0");
