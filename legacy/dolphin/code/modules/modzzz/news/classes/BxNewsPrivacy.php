...
class BxNewsPrivacy extends BxDolPrivacy {
	 ...
	 * Constructor
	 */
    function BxNewsPrivacy(&$oModule) {
        $this->oModule = $oModule;
	    parent::BxDolPrivacy($oModule->_oDb->getPrefix() . 'main', 'id', 'author_id');
    }

	/**
	 * Check whethere viewer is a member of dynamic news.
	 *
	 * @param mixed $mixedNewsId dynamic news ID.
	 * @param integer $iObjectOwnerId object owner ID.
	 * @param integer $iViewerId viewer ID.
	 * @return boolean result of operation.
	 */
    function isDynamicGroupMember($mixedNewsId, $iObjectOwnerId, $iViewerId, $iObjectId) {

        $aDataEntry = array ('id' => $iObjectId, 'author_id' => $iObjectOwnerId);
        if ('a' == $mixedNewsId) // admins only
