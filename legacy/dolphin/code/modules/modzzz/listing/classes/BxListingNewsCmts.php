...
class BxListingNewsCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxListingNewsCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aNewsEntry = $oMain->_oDb->getNewsEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableNews, $aNewsEntry);
     }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aNewsEntry = $oMain->_oDb->getNewsEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aNewsEntry['listing_id']); 
