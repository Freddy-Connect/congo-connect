...
class BxListingNewsFormAdd extends BxDolFormMedia {

    var $_oMain, $_oDb;

    function BxListingNewsFormAdd ($oMain, $iProfileId, $iListingId = 0, $iEntryId = 0, $iThumb = 0) { 
        $this->_oMain = $oMain;
        $this->_oDb = $oMain->_oDb; 
		$this->_oDb->_sTableMediaPrefix = $this->_oDb->_sTableNewsMediaPrefix;
 		
		bx_import ('SubPrivacy', $this->_oMain->_aModule);
		$GLOBALS['oBxListingModule']->_oSubPrivacy = new BxListingSubPrivacy($this->_oMain, $this->_oDb->_sTableNews); 
      ...
      if (BxDolRequest::serviceExists('videos', 'perform_video_upload', 'Uploader'))
        ...
        $aInputPrivacyComment = $GLOBALS['oBxListingModule']->_oPrivacy->getGroupChooser($iProfileId, 'listing', 'comment');
        ...
        $aInputPrivacyRate = $GLOBALS['oBxListingModule']->_oPrivacy->getGroupChooser($iProfileId, 'listing', 'rate');
