...
class BxListingPageLocal extends BxDolTwigPageMain {
 		var $_oDb;
        var $_oConfig; 
		var $sCountry; 
		var $sState; 
    ...
    function BxListingPageLocal(&$oMain, $sCountry, $sState, $sCategory) {
		...
		if($sCountry)
			parent::BxDolTwigPageMain('modzzz_listing_local_state', $oMain); 
		else	
			parent::BxDolTwigPageMain('modzzz_listing_local', $oMain); 

	}
 
    function getBlockCode_StateListings() {
		...
		if($this->sState){ 	 
			...
			$sListings = $this->ajaxBrowse('local_state', $this->oDb->getParam('modzzz_listing_perpage_main_recent'), array(), $this->sCountry, $this->sState, $this->sCategory);
			...
			$sListings = $this->ajaxBrowse('local_country', $this->oDb->getParam('modzzz_listing_perpage_main_recent'), array(), $this->sCountry,   $this->sCategory); 
		} 
    ...
    function getBlockCode_Categories() {
		...
		if($this->sState){  
			...
			$sItemLabel  = _t('_modzzz_listing_category_drilldown_for_item', $sItemName);
    ...
    function getBlockCode_States() {
		...
		$aVars['bx_repeat:entries'] = array();        
  		foreach($aStates as $aEachState){
			...
			$aVars['bx_repeat:entries'][] = array(
				...
				'bx_if:selstate' => array( 
				     // Modif freddy afficher region seulement avec au moins un Business listing
 	    ...
 	    $aBlock = array($this->oTemplate->parseHtmlByName('block_states_local', $aVars)); 
    ...
    function getBlockCode_Region() {
	    ...
	    return $this->oTemplate->parseHtmlByName('listing_regions', $aVars);  
	}
