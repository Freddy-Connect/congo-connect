...
class BxListingEventPageBrowse extends BxDolTwigPageMain {

    function BxListingEventPageBrowse(&$oMain, $sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_listing_events_browse', $oMain);
	}
    ...
    function getBlockCode_Browse () {
		...
		if($this->sUri){
			$this->sUrlStart = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() .  'event/browse/' . $this->sUri;
			$this->sUrlStart .= (false === strpos($this->sUrlStart, '?') ? '?' : '&'); 

  			$sBrowseBy = 'events'; 
		}else{
			$this->sUrlStart = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() .  'events/browse/' . $this->sUri;
			$this->sUrlStart .= (false === strpos($this->sUrlStart, '?') ? '?' : '&'); 

			$sBrowseBy = 'events_main'; 
		}

        return $this->ajaxBrowseSubProfile(
            'event',
			$sBrowseBy,   
            $this->oDb->getParam('modzzz_listing_perpage_browse_subitems'), 
