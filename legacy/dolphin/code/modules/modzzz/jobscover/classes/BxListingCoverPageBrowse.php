...
class BxJobsCoverPageBrowse extends BxDolTwigPageMain {

    function BxJobsCoverPageBrowse(&$oMain, $sUri) {
        ...
        $this->sSearchResultClassName = 'BxJobsCoverSearchResult';
        $this->sFilterName = 'jobscover_filter';
		parent::BxDolTwigPageMain('modzzz_jobscover_browse', $oMain);
        ...
        if ($_POST['action_cover'] && is_array($_POST['entry'])) {
            ...
            foreach ($_POST['entry'] as $iMediaId) {
                ...
                if ($this->oDb->activateCover($aMediaEntry['jobs_id'], $iMediaId)) {
    ...
    function getBlockCode_Browse () {
        ...
        $o = new BxJobsCoverPageMain ($this->oMain);
        ...
        $aBlock = $o->ajaxBrowse(
            'covers',
            getParam('modzzz_jobscover_perpage_browse'), 
            array(), $this->sUri, true, false, false 
        );  
 
		if($aBlock[0] == '') $aBlock[0] = MsgBox(_t('_Empty'));

		if(getParam("modzzz_jobscover_active")!='on')
