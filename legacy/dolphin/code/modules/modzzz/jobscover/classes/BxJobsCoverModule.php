...
class BxJobsCoverModule extends BxDolTwigModule {
	...
	function serviceCoverBlock($aDataEntry) {
		...
		///freddy Business Listing
		$oBusinessListing = BxDolModule::getInstance('BxListingModule');
		//////////////////
    	...
    	if(!empty($aProfileThumbnail) && is_array($aProfileThumbnail)) {
 		...
 		if($aDataEntry['icon']){ 
		    $sThumbnail = $oBusinessListing->_oDb->getLogoBusinessListing($aDataEntry['id'], $aDataEntry['icon'], true);
