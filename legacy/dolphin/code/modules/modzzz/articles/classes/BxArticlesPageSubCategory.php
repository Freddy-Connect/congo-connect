...
class BxArticlesPageSubCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxArticlesPageSubCategory(&$oMain, $sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_articles_subcategory', $oMain);
    ...
    function getBlockCode_SubCategoryArticles() {

 		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'subcategories/'.$this->aCategoryInfo['uri'].$this->oMain->getConcatenator();

        $sCode = $this->ajaxBrowse('subcategories', $this->oDb->getParam('modzzz_articles_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
