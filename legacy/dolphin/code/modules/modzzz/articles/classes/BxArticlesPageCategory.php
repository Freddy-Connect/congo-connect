...
class BxArticlesPageCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxArticlesPageCategory(&$oMain,$sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_articles_category', $oMain);
		...
		if(isset($_GET['ajax']) && ($_GET['ajax']=='sort')){
			echo $this->getBlockCode_CategoryArticles(); 
			exit;
		} 
    ...
    function getSubCategories() { 
	    ...
	    return $this->oTemplate->parseHtmlByName('block_subcategories', $aVars);  
	}
 
    function getBlockCode_CategoryArticles() {

 		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories/'.$this->aCategoryInfo['uri'].$this->oMain->getConcatenator();

        $sCode = $this->ajaxBrowse('categories', $this->oDb->getParam('modzzz_articles_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
