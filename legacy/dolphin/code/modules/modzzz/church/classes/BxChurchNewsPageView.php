...
class BxChurchNewsPageView extends BxDolTwigPageView {	

    function BxChurchNewsPageView(&$oNewsMain, &$aNews) {
        parent::BxDolTwigPageView('modzzz_church_news_view', $oNewsMain, $aNews);
	
        $this->_oDb->_sTableMediaPrefix = $this->_oDb->_sTableNewsMediaPrefix; 
	}

   	function getBlockCode_InfoOLD() {
    ...
    function getBlockCode_Rate() {
        modzzz_church_import('NewsVoting');
        $o = new BxChurchNewsVoting ('modzzz_church_news', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableNews, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_church_import('NewsCmts');
        $o = new BxChurchNewsCmts ('modzzz_church_news', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableNews,$this->aDataEntry) ? _t('_modzzz_church_action_upload_photos') : '',  
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableNews,$this->aDataEntry) ? _t('_modzzz_church_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableNews,$this->aDataEntry) ? _t('_modzzz_church_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableNews,$this->aDataEntry) ? _t('_modzzz_church_action_upload_files') : '', 
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_church_news');
        } 
