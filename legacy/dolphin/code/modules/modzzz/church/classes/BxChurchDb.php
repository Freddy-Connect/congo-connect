 ...
 * Church module Data
 */
class BxChurchDb extends BxDolTwigModuleDb {	
	...
	function BxChurchDb(&$oConfig) {
        ...
        $this->_sEventPrefix = 'modzzz_church_event';
        $this->_sTableEventMediaPrefix = 'event_';

		/********Sermon ************/
        $this->_sTableSermon = 'sermon_main';
        $this->_sSermonPrefix = 'modzzz_church_sermon';
        $this->_sTableSermonMediaPrefix = 'sermon_';

		/********Staff ************/
        $this->_sTableStaff = 'staff_main';
        $this->_sStaffPrefix = 'modzzz_church_staff';
        $this->_sTableStaffMediaPrefix = 'staff_';

		/********Doctrine ************/
        $this->_sTableDoctrine = 'doctrine_main';
        $this->_sDoctrinePrefix = 'modzzz_church_doctrine';
        $this->_sTableDoctrineMediaPrefix = 'doctrine_';
 
		/********News ************/
        $this->_sTableNews = 'news_main';
        $this->_sNewsPrefix = 'modzzz_church_news';
        $this->_sTableNewsMediaPrefix = 'news_';
 
		/********Members ************/
		$this->_sTableMembers = 'members_main'; 
        $this->_sMembersPrefix = 'modzzz_church_members';
	    $this->_sTableMembersMediaPrefix = 'members_';

		/********Ministries ************/
		$this->_sTableMinistries = 'ministries_main'; 
        $this->_sMinistriesPrefix = 'modzzz_church_ministries';
	    $this->_sTableMinistriesMediaPrefix = 'ministries_';

		/********Branches ************/
		$this->_sTableBranches = 'branches_main'; 
        $this->_sBranchesPrefix = 'modzzz_church_branches';
	    $this->_sTableBranchesMediaPrefix = 'branches_';
 
		/********Review ************/
        $this->_sTableReview = 'review_main';
        $this->_sReviewPrefix = 'modzzz_church_review';
        ...
        $this->_sFaqPrefix = 'modzzz_church_faq';
		/********[END] Faq ************/
        ...
        $this->_sFieldAllowViewTo = 'allow_view_church_to';
	}
	...
	function setInvoiceStatus($iItemId, $sStatus) {
 		 ...
 		 $this->query("UPDATE `" . $this->_sPrefix . "invoices` SET `invoice_status`='$sStatus' WHERE `church_id`='$iItemId'"); 
	}
 	...
 	function isOwnerAdmin($iProfileId) {
 		...
 		return $this->getOne("SELECT `ID` FROM `Profiles` WHERE `ID`='$iProfileId' AND `Role`=3 AND `Status`='Active'");  
	}
	...
	function saveClaimRequest($iEntryId, $iProfileId, $sClaimText) {
		...
		$aAllEntries = $this->query("INSERT INTO `" . $this->_sPrefix . "claim` SET `church_id`='$iEntryId', `member_id`='$iProfileId', `message`='$sClaimText', `processed`=0, `claim_date`=$iTime");  

	}

 	function getClaimById($iEntryId) { 
		$aClaim = $this->getRow("SELECT `id`,`church_id`,`member_id`,`message`,`processed`,`claim_date`,`assign_date` FROM `" . $this->_sPrefix . "claim` WHERE `id`='$iEntryId'");   
	...
	function assignClaim($iClaimId, $isAdmin=false) { 
		...
		$this->alertOnAction('modzzz_church_claim_assign', $iEntryId, $iClaimantId );  
	}
    ...
    function deleteEntryByIdAndOwner ($iId, $iOwner, $isAdmin) {
        if ($iRet = parent::deleteEntryByIdAndOwner ($iId, $iOwner, $isAdmin)) {
            ...
            $this->deleteEntryMediaAll ($iId, 'files');
 			...
 			$this->query("DELETE FROM `" . $this->_sPrefix . "claim` WHERE `church_id`='$iId'");  
            $this->query ("DELETE FROM `" . $this->_sPrefix . "invoices` WHERE `church_id` = $iId");
    ...
    function insertProfileLocation ($iAuthorId, $iId, $fLat, $fLng, $iMapZoom, $sMapType, $sAddress, $sCountry, $iPrivacy = 0, $isFailed = 0) {
        $sPrivacyUpdate = '';
        $sPrivacyInsert = "`allow_view_location_to` = '" . BX_CHURCH_DEFAULT_PRIVACY . "',";
        if ($iPrivacy) {
    ...
    function saveTransactionRecord($iBuyerId, $iChurchId, $sTransNo, $sTransType) {
        $iBuyerId    = (int)$iBuyerId;
        $iChurchId  = (int)$iChurchId; 
   
		$aDataEntry = $this->getEntryById($iChurchId);
		...
		$iOrderId = $this->lastId();
		 ...
		 //if order successful, update expiration date of church
		 if($iOrderId){
			$aInvoice = $this->getInvoiceByNo($sInvoiceNo);
			$iDays = (int)$aInvoice['days'];
			$this->updateEntryExpiration($iChurchId, $iDays);

			$iPackageId = (int)$aInvoice['package_id'];
			$aPackage = $this->getPackageById($iPackageId); 
			$iFeatured = (int)$aPackage['featured'];
			if($iFeatured){
				$this->updateFeaturedStatus($iChurchId);
			}
		 }
	...
	function createInvoice($iEntryId, $iPackageId, $fPrice, $iDays, $sStatus='pending'){
		 ...
		 $iInvoiceActiveDays = (int)getParam('modzzz_church_invoice_valid_days');
    ...
    function getInvoiceByNo($sInvoiceNo) {  
        return $this->getRow("SELECT `invoice_no`, `price`, `days`, `church_id`, `package_id`, `invoice_status`, `invoice_due_date`, `invoice_expiry_date`, `invoice_date` FROM `" . $this->_sPrefix . "invoices` WHERE `invoice_no` = '{$sInvoiceNo}'"); 
    }
   ...
   /** category functions*/
	...
	function getParentCategoryCount($iParent){
		...
		if (!$GLOBALS['logged']['admin']){ 
			if ($GLOBALS['logged']['member']){ 
			    ...
			    $sExtraCheck = " AND church.`membership_view_filter` IN ('', '$iMembershipId')"; 
			}else{
				$sExtraCheck = "AND church.`membership_view_filter`=''";
			}
		}
 
		return  $this->getOne("SELECT count(church.`id`) FROM `" . $this->_sPrefix . "categ` cat LEFT JOIN `" . $this->_sPrefix . "main` church ON cat.id=church.`category_id` WHERE cat.`active` = 1 AND cat.`parent`=$iParent AND church.`status`='approved' {$sExtraCheck}"); 
	}
	...
	function getCategoryCount($iCategoryId){
		...
		return  $this->getOne("SELECT count(church.`id`) FROM `" . $this->_sPrefix . "categ` cat LEFT JOIN `" . $this->_sPrefix . "main` church ON cat.id=church.`category_id` WHERE cat.`active` = 1 AND cat.`id`=$iCategoryId AND church.`status`='approved' {$sExtraCheck}"); 
	}
	...
	/*featured functions*/ 
    function saveFeaturedTransactionRecord($iBuyerId, $iChurchId, $iQuantity, $fPrice, $sTransId, $sTransType) {
        $iBuyerId    = (int)$iBuyerId;
        $iChurchId  = (int)$iChurchId; 
		...
		if($iOrderId){
			$this->alertOnAction('modzzz_church_featured_admin_notify', $iChurchId, $iBuyerId, $iQuantity, true);

			$this->alertOnAction('modzzz_church_featured_buyer_notify', $iChurchId, $iBuyerId, $iQuantity);
		}
	...
	function processChurches(){ 
		$this->processExpiredChurches();
			
		$this->processExpiredInvoices();
 
		$this->processFeaturedChurches();
	}
	...
	function processExpiredInvoices() { 
		...
		$aChurches = $this->getAll("SELECT `church_id` FROM `" . $this->_sPrefix . "invoices` WHERE `invoice_status`='pending' AND `invoice_expiry_date`>0 AND `invoice_expiry_date`<={$iExpireTime}");
		 
		foreach ($aChurches as $aEachChurch) {
			$iEntryId = (int)$aEachChurch['church_id']; 
 
			$this->query("DELETE FROM `" . $this->_sPrefix . "main` WHERE `id`=$iEntryId");  
			
			$this->query("DELETE FROM `" . $this->_sPrefix . "invoices` WHERE `church_id`=$iEntryId");  
 		}  
 
	}


	function processExpiredChurches() { 
 
 		$iTime = time();
 		$SECONDS_IN_DAY = 86400; 
		
        $bFreeChurch = (getParam('modzzz_church_paid_active')=='on') ? false : true;  
        $iNumActiveDays = (int)getParam("modzzz_church_free_expired");
        if($bFreeChurch && (!$iNumActiveDays))
            return; 

		if(getParam('modzzz_church_activate_expired') == 'on') {
  			 
			$aChurches = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `expiry_date`>0 AND `expiry_date`<={$iTime}");
			 
			foreach ($aChurches as $aEachChurch) {
				$iEntryId = (int)$aEachChurch['id']; 
				$iRecipientId = (int)$aEachChurch['author_id']; 

				$this->query("UPDATE `" . $this->_sPrefix . "main` SET `status`='expired'  WHERE `id`=$iEntryId");  
 
		 		$this->alertOnAction('modzzz_church_expired', $iEntryId, $iRecipientId);
			}  
		}else{ 
  			$this->query("UPDATE `" . $this->_sPrefix . "main` SET `status`='expired' WHERE  `status`='approved' AND `expiry_date`>0 AND `expiry_date`<=$iTime");  
		}
  
		$iNumNotifyDays = (int)getParam("modzzz_church_email_expiring");
		if($iNumNotifyDays){
			$iNotifyTime = $SECONDS_IN_DAY * $iNumNotifyDays;
			$iTriggerTime = $iTime - $iNotifyTime;

			$aChurches = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='approved' AND `expiry_date` >= {$iTriggerTime} AND `expiry_date` < {$iTime}  AND `pre_expire_notify`=0");
			 
			foreach ($aChurches as $aEachChurch) {
				$iEntryId = $aEachChurch['id'];
				$iRecipientId = $aEachChurch['author_id'];
		 
				$this->query("UPDATE `" . $this->_sPrefix . "main` SET `pre_expire_notify`=1 WHERE `id` = $iEntryId");

				if(getParam("modzzz_church_activate_expiring")=='on') { 
					$this->alertOnAction('modzzz_church_expiring', $iEntryId, $iRecipientId, $iNumNotifyDays);
				}
			}
		}
 			
		$iNumNotifyDays = (int)getParam("modzzz_church_email_expired");
		if($iNumNotifyDays){
			$iNotifyTime = $SECONDS_IN_DAY * $iNumNotifyDays;
			$iTriggerTime = $iTime - $iNotifyTime;

			$aChurches = $this->getAll("SELECT `id`, `expiry_date`, `author_id` FROM `" . $this->_sPrefix . "main` WHERE `status`='expired' AND `expiry_date` <= {$iNotifyTime} AND `post_expire_notify`=0");
			...
			foreach ($aChurches as $aEachChurch) {
				...
				if(getParam("modzzz_church_activate_expired")=='on') { 
					$this->alertOnAction('modzzz_church_post_expired', $iEntryId, $iRecipientId, $iNumNotifyDays);
				}
			}
		}
	...
	function processFeaturedChurches(){
        ...
        $aChurches = $this->getAll("SELECT `id`, `author_id`, `featured`, `featured_expiry_date` FROM `" . $this->_sPrefix . "main` WHERE `featured`=1 AND `featured_expiry_date`>0 AND `featured_expiry_date` <= $iTime"); 

		foreach($aChurches as $aEachList){
  
			$iChurchId = (int)$aEachList['id'];
			$iRecipientId = (int)$aEachList['author_id'];

			$this->alertOnAction('modzzz_church_featured_expire_notify', $iChurchId, $iRecipientId  );
		
	        $this->query("UPDATE `" . $this->_sPrefix . "main` SET `featured`=0, `featured_expiry_date`=0, `featured_date`=0  WHERE `id`=$iChurchId"); 
		}

	}

	function alertOnAction($sTemplate, $iChurchId, $iRecipientId=0, $iDays=0, $bAdmin=false) {
 	...
 	function getLatestComments($iLimit=5){
		...
		return $this->getAll("SELECT `cmt_object_id`,`cmt_author_id`,`cmt_text`, UNIX_TIMESTAMP(`cmt_time`) AS `date` FROM `" . $this->_sPrefix . "cmts` cmt INNER JOIN `" . $this->_sPrefix . "main` m  ON cmt.`cmt_object_id`= m.`id` WHERE m.`allow_view_church_to` = '". BX_DOL_PG_ALL ."' AND `cmt_text` NOT LIKE '<object%' ORDER BY `cmt_time` DESC LIMIT $iLimit"); 
	}  
    ...
    function deleteNewsByIdAndOwner ($iId, $iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableNews . "` WHERE `id` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
    ...
    function deleteNews ($iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        
		$aNews = $this->getAllSubItems('news', $iChurchId);
  
		if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableNews . "` WHERE `church_id`=$iChurchId $sWhere")))
            return false;
    ...
    function deleteEventByIdAndOwner ($iId, $iChurchId, $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableEvent . "` WHERE `id` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
    ...
    function deleteEvents ($iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
       
		$aEvents = $this->getAllSubItems('event', $iChurchId);

		if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableEvent . "` WHERE `church_id`=$iChurchId $sWhere")))
            return false;
	...
	/***** begin] Doctrine **************************************/
    function getDoctrineEntryById($iId) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableDoctrine . "` WHERE `id` = $iId LIMIT 1");
    }
 
    function getDoctrineEntryByUri($sUri) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableDoctrine . "` WHERE `uri` = '$sUri' LIMIT 1");
    }
 
    function deleteDoctrineByIdAndOwner ($iId, $iChurchId, $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableDoctrine . "` WHERE `id` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
 
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableDoctrineMediaPrefix . "files` WHERE `entry_id` = $iId");
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableDoctrineMediaPrefix . "images` WHERE `entry_id` = $iId");
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableDoctrineMediaPrefix . "videos` WHERE `entry_id` = $iId");
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableDoctrineMediaPrefix . "sounds` WHERE `entry_id` = $iId");

        return true;
    } 

    function deleteDoctrines ($iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
       
		$aDoctrines = $this->getAllSubItems('doctrine', $iChurchId);

		if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableDoctrine . "` WHERE `church_id`=$iChurchId $sWhere")))
            return false;
    ...
    function deleteStaffByIdAndOwner ($iId, $iChurchId, $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableStaff . "` WHERE `id` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
    ...
    function deleteStaffs ($iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
       
		$aStaffs = $this->getAllSubItems('staff', $iChurchId);

		if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableStaff . "` WHERE `church_id`=$iChurchId $sWhere")))
            return false;
	...
	/***** Sermon **************************************/
    function getSermonEntryById($iId) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableSermon . "` WHERE `id` = $iId LIMIT 1");
    }
 
    function getSermonEntryByUri($sUri) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableSermon . "` WHERE `uri` = '$sUri' LIMIT 1");
    }
 
    function deleteSermonByIdAndOwner ($iId, $iChurchId, $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableSermon . "` WHERE `id` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
 
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableSermonMediaPrefix . "files` WHERE `entry_id` = $iId");
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableSermonMediaPrefix . "images` WHERE `entry_id` = $iId");
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableSermonMediaPrefix . "videos` WHERE `entry_id` = $iId");
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableSermonMediaPrefix . "sounds` WHERE `entry_id` = $iId");

        return true;
    } 

    function deleteSermons ($iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
       
		$aSermons = $this->getAllSubItems('sermon', $iChurchId);

		if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableSermon . "` WHERE `church_id`=$iChurchId $sWhere")))
            return false;
	...
	/***** MEMBERS **************************************/
 	...
 	function getMembers($iEntryId, $iLimit=0){
		...
		return $this->getAll("SELECT `id`, `church_id`, `uri`, `title`, `desc`, `position`,  `thumb` FROM `" . $this->_sPrefix . $this->_sTableMembers . "` WHERE `church_id`={$iEntryId} {$sQuery}");
	}
    ...
    function deleteMembersByIdAndOwner ($iId, $iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableMembers . "` WHERE `id` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
    ...
    function deleteMembers ($iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        
		$aMembers = $this->getAllSubItems('members', $iChurchId);
  
		if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableMembers . "` WHERE `church_id`=$iChurchId $sWhere")))
            return false;
	...
	/***** Ministries **************************************/
 	...
 	function getMinistries($iEntryId, $iLimit=0){
		
		if($iLimit)
			$sQuery = "LIMIT 0, {$iLimit}";

		return $this->getAll("SELECT `id`, `church_id`, `uri`, `title`, `desc`, `position`,  `thumb` FROM `" . $this->_sPrefix . $this->_sTableMinistries . "` WHERE `church_id`={$iEntryId} {$sQuery}");
	}

    function getMinistriesEntryById($iId) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableMinistries . "` WHERE `id` = $iId LIMIT 1");
    }
 
    function getMinistriesEntryByUri($sUri) {
         return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableMinistries . "` WHERE `uri` = '$sUri' LIMIT 1");
    }
 
    function deleteMinistriesByIdAndOwner ($iId, $iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableMinistries . "` WHERE `id` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
 
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableMinistriesMediaPrefix . "images` WHERE `entry_id` = $iId");
 
        return true;
    } 
 
    function deleteMinistries ($iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        
		$aMinistries = $this->getAllSubItems('ministries', $iChurchId);
  
		if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableMinistries . "` WHERE `church_id`=$iChurchId $sWhere")))
            return false;
	...
	/***** Branches **************************************/
 	...
 	function getBranches($iEntryId, $iLimit=0){
		...
		return $this->getAll("SELECT `id`, `church_id`, `uri`, `title`, `desc`, `position`,  `thumb` FROM `" . $this->_sPrefix . $this->_sTableBranches . "` WHERE `church_id`={$iEntryId} {$sQuery}");
	}
    ...
    function deleteBranchesByIdAndOwner ($iId, $iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableBranches . "` WHERE `id` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
    ...
    function deleteBranches ($iChurchId,  $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `author_id` = '$iOwner' ";
        
		$aBranches = $this->getAllSubItems('branches', $iChurchId);
  
		if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableBranches . "` WHERE `church_id`=$iChurchId $sWhere")))
            return false;
    ...
    function deleteReviewByIdAndOwner ($iId, $iChurchId, $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `{$this->_sFieldAuthorId}` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableReview . "` WHERE `{$this->_sFieldId}` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
	...
	function getReviews($sType='', $sqlLimit='', $iEntryId=0) {
		...
		if($iEntryId){ 
			$sIdQuery = ' AND `church_id`='.$iEntryId;
		}
    ...
    function getFaqByChurch($iChurchId) {
         return $this->getOne ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableFaq . "` WHERE `church_id` = '$iChurchId' LIMIT 1");
    }
  
    function deleteFaqByIdAndOwner ($iId, $iChurchId, $iOwner, $isAdmin) {
        $sWhere = '';
        if (!$isAdmin) 
            $sWhere = " AND `{$this->_sFieldAuthorId}` = '$iOwner' ";
        if (!($iRet = $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableFaq . "` WHERE `{$this->_sFieldId}` = $iId AND `church_id`=$iChurchId $sWhere LIMIT 1")))
            return false;
        $this->query ("DELETE FROM `" . $this->_sPrefix . $this->_sTableFaqItems . "` WHERE `church_id` = $iChurchId");
	...
	function getFaqItems($iChurchId, $sqlLimit='') {
		$iChurchId = (int)$iChurchId; 
	 
		$aAllEntries = $this->getAll("SELECT `id`, `faq_id`, `question`, `answer` FROM `" . $this->_sPrefix . $this->_sTableFaqItems . "` WHERE `church_id`={$iChurchId} ORDER BY `question` ASC {$sqlLimit}");

		return $aAllEntries;   
	}
 
 	function addFaqItems($iFaqId, $iChurchId){
		$iCreated = time();
		...
		if(is_array($_POST['question'])){ 
			foreach($_POST['question'] as $iKey=>$sQuestion){
				...
				if($sAnswer){
					$this->query("INSERT INTO `" . $this->_sPrefix . $this->_sTableFaqItems . "` SET `faq_id`=$iFaqId, `church_id`='$iChurchId', `question`='$sQuestion', `answer`='$sAnswer', `created`=$iCreated");
				} 
 			} 
		} 
	}
    ...
    function isSubProfileFan($sTable, $iSubEntryId, $iProfileId, $isConfirmed) {
        ...
        $iEntryId = (int)$this->getOne ("SELECT `church_id` FROM `" . $this->_sPrefix . $sTable . "` WHERE `id` = '$iSubEntryId' LIMIT 1");
	...
	function getAllSubItems($sSubItem, $iChurchId){
		$aSubItems = array();
		$iChurchId = (int)$iChurchId;

		switch($sSubItem){ 
			case 'event':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableEvent . "` WHERE `church_id`=$iChurchId");
			break;  
			case 'branches':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableBranches . "` WHERE `church_id`=$iChurchId");
			break;  
			case 'doctrine':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableDoctrine . "` WHERE `church_id`=$iChurchId");
			break;  
			case 'members':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableMembers . "` WHERE `church_id`=$iChurchId");
			break;  
			case 'ministries':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableMinistries . "` WHERE `church_id`=$iChurchId");
			break;  
			case 'news':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableNews . "` WHERE `church_id`=$iChurchId");
			break;  
			case 'sermon':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableSermon . "` WHERE `church_id`=$iChurchId");
			break;  
			case 'staff':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableStaff . "` WHERE `church_id`=$iChurchId");
			break;  
 			case 'faq':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableFaq . "` WHERE `church_id`=$iChurchId");
			break; 
			case 'review':
				$aSubItems = $this->getAll ("SELECT `id` FROM `" . $this->_sPrefix . $this->_sTableReview . "` WHERE `church_id`=$iChurchId");
			break; 
		}
    ...
    //[BEGIN] DONATION
    function generatePaymentUrl($iEntryId, $fTotalCost, $sCustomStr, $iProfileId=0) {
        ...
        return  $this ->_oConfig->getPurchaseBaseUrl() . '?cmd=_xclick'
                ...
                . '&return=' .  $this->_oConfig->getDonationUrl() . $iProfileId .'/'. $iEntryId
				. '&business='  . $sPaypalEmail
                . '&custom='  . $sCustomStr  
                . '&item_name=' . _t('_modzzz_church_paypal_donation_desc',$aEntry['title'])
    ...
    function saveDonationRecord($iEntryId, $iDonorId, $iAmount, $sTransId, $sPaypal, $sCustomStr, $sTransType) {
		...
		if($iDonorId){
			...
			$sFirstName = ($aCustom[1]) ? $aCustom[1] : $aProfile['FirstName'];
			$sLastName = ($aCustom[2]) ? $aCustom[2] : $aProfile['LastName'];
		}else{
        ...
        $bProcessed = $this->query("INSERT INTO `" . $this->_sPrefix . "paypal_trans` SET `church_id` = '{$iEntryId}', `donor_id` = '{$iDonorId}', `paypal` =  '{$sPaypal}', `trans_id` =  '{$sTransId}', `created`  = UNIX_TIMESTAMP(), `trans_type`  = '{$sTransType}', `amount` = '{$iAmount}', `anonymous` = {$bAnonymous}, `first_name`='$sFirstName', `last_name`='$sLastName'
        "); 
	...
	function onDonationAlert($sItem, $iEntryId, $iHero, $iOwner, $aData, $fAmount=0) {
		...
		if($bAnonymous){  
 			$sHeroName = _t('_modzzz_church_someone'); 
		...
		$aChurch = $this->getEntryById($iEntryId);
 
		$sChurchUrl = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/'.$aChurch['uri'];
		...
		$aPlus['ChurchName'] = $sChurchTitle; 
		$aPlus['ChurchLink'] = $sChurchUrl;
		$aPlus['Amount'] = $this->_oConfig->getCurrencySign() . $fAmount;
		...
		switch($sItem){
			...
			case "thankyou":
				...
				$sTemplate = "modzzz_church_donation_thanks";
				$aTemplate = $oEmailTemplate->getTemplate($sTemplate, $iHero);
				$sMessage = $aTemplate['Body'];
				$sSubject = $aTemplate['Subject'];   
				$sSubject = str_replace("<SiteName>", $aPlus['SiteName'], $sSubject);
				$sSubject = str_replace("<ChurchName>", $aPlus['ChurchName'], $sSubject);
		  
				$vMailRes = sendMail( $sHeroEmail, $sSubject, $sMessage, $iHero, $aPlus, 'html' ); 
			break; 	 
			case "notify":
	 
				$sTemplate = "modzzz_church_donation_notify";
