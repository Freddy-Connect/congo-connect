...
class BxChurchReviewPageView extends BxDolTwigPageView {	

    function BxChurchReviewPageView(&$oReviewsMain, &$aReview) {
        parent::BxDolTwigPageView('modzzz_church_reviews_view', $oReviewsMain, $aReview);
    ...
    function getBlockCode_Rate() {
        modzzz_church_import('ReviewVoting');
        $o = new BxChurchReviewVoting ('modzzz_church_review', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableReview, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_church_import('ReviewCmts');
        $o = new BxChurchReviewCmts ('modzzz_church_review', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
			...
			$iEntryId = $aReviewEntry['church_id'];
            ...
            $this->aInfo = array (
                ...
                'TitleEdit' => $this->_oMain->isAllowedSubEdit($aDataEntry, $aReviewEntry) ? _t('_modzzz_church_action_title_edit') : '',
                'TitleDelete' => $this->_oMain->isAllowedSubDelete($aDataEntry, $aReviewEntry) ? _t('_modzzz_church_action_title_delete') : '',
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_church_review');
        } 
