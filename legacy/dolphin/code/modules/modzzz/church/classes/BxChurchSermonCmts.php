...
class BxChurchSermonCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxChurchSermonCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aSermonEntry = $oMain->_oDb->getSermonEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableSermon, $aSermonEntry);
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aSermonEntry = $oMain->_oDb->getSermonEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aSermonEntry['church_id']); 
