...
class BxChurchEventPageView extends BxDolTwigPageView {	

    function BxChurchEventPageView(&$oEventsMain, &$aEvent) {
        parent::BxDolTwigPageView('modzzz_church_events_view', $oEventsMain, $aEvent);
    ...
    function getBlockCode_Rate() {
        modzzz_church_import('EventVoting');
        $o = new BxChurchEventVoting ('modzzz_church_event', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableEvent, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_church_import('EventCmts');
        $o = new BxChurchEventCmts ('modzzz_church_event', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableEvent,$this->aDataEntry) ? _t('_modzzz_church_action_upload_photos') : '', 
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableEvent,$this->aDataEntry) ? _t('_modzzz_church_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableEvent,$this->aDataEntry) ? _t('_modzzz_church_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableEvent,$this->aDataEntry) ? _t('_modzzz_church_action_upload_files') : '', 
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_church_event');
        } 
