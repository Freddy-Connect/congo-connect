...
class BxChurchMinistriesPageView extends BxDolTwigPageView {	

    function BxChurchMinistriesPageView(&$oMinistriesMain, &$aMinistries) {
        parent::BxDolTwigPageView('modzzz_church_ministries_view', $oMinistriesMain, $aMinistries);
    ...
    function getBlockCode_Rate() {
        modzzz_church_import('MinistriesVoting');
        $o = new BxChurchMinistriesVoting ('modzzz_church_ministries', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableMinistries, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_church_import('MinistriesCmts');
        $o = new BxChurchMinistriesCmts ('modzzz_church_ministries', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
			...
			$aMinistriesEntry = $this->_oDb->getMinistriesEntryById($this->aDataEntry['id']);
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableMinistries,$this->aDataEntry) ? _t('_modzzz_church_action_upload_photos') : '', 
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableMinistries,$this->aDataEntry) ? _t('_modzzz_church_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableMinistries,$this->aDataEntry) ? _t('_modzzz_church_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableMinistries,$this->aDataEntry) ? _t('_modzzz_church_action_upload_files') : '', 
             );
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_church_ministries');
        } 
