...
class BxChurchPageLocal extends BxDolTwigPageMain {
 		var $_oDb;
        var $_oConfig; 
		var $sCountry; 
		var $sState; 

    function BxChurchPageLocal(&$oMain, $sCountry, $sState) {
		...
		if($sCountry)
			parent::BxDolTwigPageMain('modzzz_church_local_state', $oMain); 
		else	
			parent::BxDolTwigPageMain('modzzz_church_local', $oMain); 

	}
 
    function getBlockCode_StateChurches() {
  
		if($this->sState){ 
			$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'local/'. $this->sCountry .'/'. $this->sState . '?';
			
			$sChurches = $this->ajaxBrowse('local_state', $this->oDb->getParam('modzzz_church_perpage_main_recent'), array(), $this->sState, false, false);
		}elseif($this->sCountry){
			$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'local/'. $this->sCountry . '?';

			$sChurches = $this->ajaxBrowse('local_country', $this->oDb->getParam('modzzz_church_perpage_main_recent'), array(), $this->sCountry, false, false);
		} 
    ...
    function getBlockCode_States() {
	    ...
	    $aStates = array($this->oTemplate->parseHtmlByName('church_states', $aVars)); 
    ...
    function getBlockCode_Region() {
	    ...
	    return $this->oTemplate->parseHtmlByName('church_regions', $aVars);  
	}
