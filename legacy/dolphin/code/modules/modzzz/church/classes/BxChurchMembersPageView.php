...
class BxChurchMembersPageView extends BxDolTwigPageView {	

    function BxChurchMembersPageView(&$oMembersMain, &$aMembers) {
        parent::BxDolTwigPageView('modzzz_church_members_view', $oMembersMain, $aMembers);
    ...
    function getBlockCode_Rate() {
        modzzz_church_import('MembersVoting');
        $o = new BxChurchMembersVoting ('modzzz_church_members', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableMembers, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_church_import('MembersCmts');
        $o = new BxChurchMembersCmts ('modzzz_church_members', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableMembers,$this->aDataEntry) ? _t('_modzzz_church_action_upload_photos') : '', 
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableMembers,$this->aDataEntry) ? _t('_modzzz_church_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableMembers,$this->aDataEntry) ? _t('_modzzz_church_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableMembers,$this->aDataEntry) ? _t('_modzzz_church_action_upload_files') : '', 
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_church_members');
        } 
