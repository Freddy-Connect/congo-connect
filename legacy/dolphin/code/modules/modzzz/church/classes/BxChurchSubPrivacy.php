...
class BxChurchSubPrivacy extends BxDolPrivacy {
	 ...
	 * Constructor
	 */
    function BxChurchSubPrivacy(&$oModule, $sTable) {
	...
	/**
	 * Check whethere viewer is a member of dynamic church.
	 *
	 * @param mixed $mixedChurchId dynamic church ID.
	 * @param integer $iObjectOwnerId object owner ID.
	 * @param integer $iViewerId viewer ID.
	 * @return boolean result of operation.
	 */
    function isDynamicGroupMember($mixedChurchId, $iObjectOwnerId, $iViewerId, $iObjectId) {
   
        $aDataEntry = array ('id' => $iObjectId, 'author_id' => $iObjectOwnerId);
        if ('f' == $mixedChurchId){  // fans only                       
            return $this->oModule->isSubProfileFan ($this->sTable, $aDataEntry, $iViewerId, true); 
 
        }elseif ('a' == $mixedChurchId){// admins only
