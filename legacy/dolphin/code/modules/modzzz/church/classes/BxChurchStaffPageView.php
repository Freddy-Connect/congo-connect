...
class BxChurchStaffPageView extends BxDolTwigPageView {	

    function BxChurchStaffPageView(&$oStaffsMain, &$aStaff) {
        parent::BxDolTwigPageView('modzzz_church_staffs_view', $oStaffsMain, $aStaff);
    ...
    function getBlockCode_Rate() {
        modzzz_church_import('StaffVoting');
        $o = new BxChurchStaffVoting ('modzzz_church_staff', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableStaff, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_church_import('StaffCmts');
        $o = new BxChurchStaffCmts ('modzzz_church_staff', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableStaff,$this->aDataEntry) ? _t('_modzzz_church_action_upload_photos') : '', 
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableStaff,$this->aDataEntry) ? _t('_modzzz_church_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableStaff,$this->aDataEntry) ? _t('_modzzz_church_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableStaff,$this->aDataEntry) ? _t('_modzzz_church_action_upload_files') : '', 
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_church_staff');
        } 
