...
class BxChurchMinistriesPageBrowse extends BxDolTwigPageMain {

    function BxChurchMinistriesPageBrowse(&$oMain, $sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_church_ministries_browse', $oMain);
	}
  
    function getBlockCode_Browse () {
 
        $this->sUrlStart = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() .  'ministries/browse/' . $this->sUri . '?';
        ...
        return $this->ajaxBrowseSubProfile(
            'ministries',
            'ministries',
            $this->oDb->getParam('modzzz_church_perpage_browse_subitems'), 
