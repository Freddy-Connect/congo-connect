...
class BxChurchSermonPageView extends BxDolTwigPageView {	

    function BxChurchSermonPageView(&$oSermonsMain, &$aSermon) {
        parent::BxDolTwigPageView('modzzz_church_sermons_view', $oSermonsMain, $aSermon);
    ...
    function getBlockCode_Rate() {
        modzzz_church_import('SermonVoting');
        $o = new BxChurchSermonVoting ('modzzz_church_sermon', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRateSubProfile($this->_oDb->_sTableSermon, $this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        modzzz_church_import('SermonCmts');
        $o = new BxChurchSermonCmts ('modzzz_church_sermon', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
			...
			$aSermonEntry = $this->_oDb->getSermonEntryById($this->aDataEntry['id']);
            ...
            $this->aInfo = array (
				...
				'TitleUploadPhotos' => $this->_oMain->isAllowedUploadPhotosSubProfile($this->_oDb->_sTableSermon,$this->aDataEntry) ? _t('_modzzz_church_action_upload_photos') : '', 
				'TitleUploadVideos' => $this->_oMain->isAllowedUploadVideosSubProfile($this->_oDb->_sTableSermon,$this->aDataEntry) ? _t('_modzzz_church_action_upload_videos') : '', 
				'TitleUploadSounds' => $this->_oMain->isAllowedUploadSoundsSubProfile($this->_oDb->_sTableSermon,$this->aDataEntry) ? _t('_modzzz_church_action_upload_sounds') : '', 
				'TitleUploadFiles' => $this->_oMain->isAllowedUploadFilesSubProfile($this->_oDb->_sTableSermon,$this->aDataEntry) ? _t('_modzzz_church_action_upload_files') : '', 
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'modzzz_church_sermon');
        } 
