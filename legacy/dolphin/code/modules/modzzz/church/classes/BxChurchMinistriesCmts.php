...
class BxChurchMinistriesCmts extends BxTemplCmtsView {
 

	/**
	 * Constructor
	 */
	function BxChurchMinistriesCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
        if (!parent::isPostReplyAllowed())
            return false;
        $oMain = $this->getMain();  
		$aMinistryEntry = $oMain->_oDb->getMinistriesEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableMinistries, $aMinistryEntry);
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain(); 
		$aMinistryEntry = $oMain->_oDb->getMinistriesEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aMinistryEntry['church_id']);
