...
class BxChurchBranchesFormAdd extends BxDolFormMedia {

    var $_oMain, $_oDb;

    function BxChurchBranchesFormAdd ($oMain, $iProfileId, $iChurchId = 0, $iEntryId = 0, $iThumb = 0) { 
        $this->_oMain = $oMain;
        $this->_oDb = $oMain->_oDb; 
		$this->_oDb->_sTableMediaPrefix = $this->_oDb->_sTableBranchesMediaPrefix;
         
		bx_import ('SubPrivacy', $this->_oMain->_aModule);
		$GLOBALS['oBxChurchModule']->_oSubPrivacy = new BxChurchSubPrivacy($this->_oMain, $this->_oDb->_sTableBranches); 
		...
		if($iChurchId) {
			$aDataEntry = $this->_oDb->getBranchesEntryById($iChurchId); 
        ...
        $aCustomForm = array(
            ...
            'inputs' => array(
                ...
                'title' => array(
                    'type' => 'text',
                    'name' => 'title',
                    'caption' => _t('_modzzz_church_form_caption_branch_name'),
                    'required' => true,
                    'checker' => array (
                        'func' => 'length',
                        'params' => array(3,100),
