...
class BxChurchPageCategory extends BxDolTwigPageMain {

	var $aCategoryInfo = array();
	var $sCategoryUri;

    function BxChurchPageCategory(&$oMain,$sUri) {
		...
		parent::BxDolTwigPageMain('modzzz_church_category', $oMain);
		...
		if(isset($_GET['ajax']) && ($_GET['ajax']=='sort')){
			echo $this->getBlockCode_CategoryChurch(); 
			exit;
		} 
    ...
    function getBlockCode_CategoryChurch() {

		$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories/'.$this->aCategoryInfo['uri'].'?';

        $sCode = $this->ajaxBrowse('categories', $this->oDb->getParam('modzzz_church_perpage_main_recent'),array(), $this->aCategoryInfo['uri'], false, false);  
