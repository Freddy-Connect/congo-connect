...
class BxGroupsVenuePageView extends BxDolTwigPageView {	

    function BxGroupsVenuePageView(&$oVenuesMain, &$aVenue) {
        parent::BxDolTwigPageView('bx_groups_venues_view', $oVenuesMain, $aVenue);
	...
	function _blockFiles ($aReadyMedia, $iAuthorId = 0) {        
        ...
        foreach ($aReadyMedia as $iMediaId) {        
            ...
            $aInputBtnDownload = array (
                ...
                'value' => _t ('_download'), 
                'attrs' => array(
                    'onclick' => "window.open ('" . BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . "venue/download/".$this->aDataEntry[$this->_oDb->_sFieldId]."/{$iMediaId}','_self');",
                ),
            );
    ...
    function getBlockCode_Rate() {
        bx_groups_import('VenueVoting');
        $o = new BxGroupsVenueVoting ('bx_groups_venue', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRate($this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        bx_groups_import('VenueCmts');
        $o = new BxGroupsVenueCmts ('bx_groups_venue', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
	...
	function _blockCustomDisplay($aDataEntry, $sType) {
		...
		$sFields = $this->_oTemplate->blockCustomSubItemFields($aDataEntry, 'Venue', $aAllow);
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
            ...
            $this->aInfo = array (
                ...
                'URI' => $this->aDataEntry['uri'],
                 'TitleEdit' => $this->_oMain->isAllowedSubEdit($aDataEntry, $aVenueEntry) ? _t('_bx_groups_action_title_edit') : '',
                'TitleDelete' => $this->_oMain->isAllowedSubDelete($aDataEntry, $aVenueEntry) ? _t('_bx_groups_action_title_delete') : ''
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'bx_groups_venue');
        } 
