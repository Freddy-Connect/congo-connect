...
class BxGroupsSponsorPageView extends BxDolTwigPageView {	

    function BxGroupsSponsorPageView(&$oSponsorsMain, &$aSponsor) {
        parent::BxDolTwigPageView('bx_groups_sponsors_view', $oSponsorsMain, $aSponsor);
	...
	function _blockFiles ($aReadyMedia, $iAuthorId = 0) {        
        ...
        foreach ($aReadyMedia as $iMediaId) {        
            ...
            $aInputBtnDownload = array (
                ...
                'value' => _t ('_download'), 
                'attrs' => array(
                    'onclick' => "window.open ('" . BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . "sponsor/download/".$this->aDataEntry[$this->_oDb->_sFieldId]."/{$iMediaId}','_self');",
                ),
            );
    ...
    function getBlockCode_Rate() {
        bx_groups_import('SponsorVoting');
        $o = new BxGroupsSponsorVoting ('bx_groups_sponsor', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRate($this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        bx_groups_import('SponsorCmts');
        $o = new BxGroupsSponsorCmts ('bx_groups_sponsor', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
			...
			$iEntryId = $aSponsorEntry['group_id'];
            ...
            $this->aInfo = array (
                ...
                'TitleEdit' => $this->_oMain->isAllowedSubEdit($aDataEntry, $aSponsorEntry) ? _t('_bx_groups_action_title_edit') : '',
                'TitleDelete' => $this->_oMain->isAllowedSubDelete($aDataEntry, $aSponsorEntry) ? _t('_bx_groups_action_title_delete') : ''
            );

            if (!$this->aInfo['TitleEdit'] && !$this->aInfo['TitleDelete'])
                return '';

            return $oFunctions->genObjectsActions($this->aInfo, 'bx_groups_sponsor');
        } 
