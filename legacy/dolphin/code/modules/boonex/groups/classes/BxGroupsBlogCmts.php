...
class BxGroupsBlogCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxGroupsBlogCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aBlogEntry = $oMain->_oDb->getBlogEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableBlog, $aBlogEntry); 
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aBlogEntry = $oMain->_oDb->getBlogEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aBlogEntry['group_id']); 
