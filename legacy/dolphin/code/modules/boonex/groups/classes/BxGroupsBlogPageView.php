...
class BxGroupsBlogPageView extends BxDolTwigPageView {	

    function BxGroupsBlogPageView(&$oBlogsMain, &$aBlog) {
        parent::BxDolTwigPageView('bx_groups_blogs_view', $oBlogsMain, $aBlog);
	...
	function getBlockCode_Info() {
        return array($this->_oTemplate->blockSubProfileInfo ('blog', $this->aDataEntry));
    }
	...
	function _blockFiles ($aReadyMedia, $iAuthorId = 0) {        
        ...
        foreach ($aReadyMedia as $iMediaId) {        
            ...
            $aInputBtnDownload = array (
                ...
                'value' => _t ('_download'), 
                'attrs' => array(
                    'onclick' => "window.open ('" . BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . "blog/download/".$this->aDataEntry[$this->_oDb->_sFieldId]."/{$iMediaId}','_self');",
                ),
            );
    ...
    function getBlockCode_Rate() {
        bx_groups_import('BlogVoting');
        $o = new BxGroupsBlogVoting ('bx_groups_blog', (int)$this->aDataEntry['id']);
    	if (!$o->isEnabled()) return '';
        return array($o->getBigVoting ($this->_oMain->isAllowedRate($this->aDataEntry)));
    }        

    function getBlockCode_Comments() {    
        bx_groups_import('BlogCmts');
        $o = new BxGroupsBlogCmts ('bx_groups_blog', (int)$this->aDataEntry['id']);
        if (!$o->isEnabled()) 
            return '';
    ...
    function getBlockCode_Actions() {
        ...
        if ($this->_oMain->_iProfileId || $this->_oMain->isAdmin()) {
			...
			$aBlogEntry = $this->_oDb->getBlogEntryById($this->aDataEntry['id']);
			$iEntryId = $aBlogEntry['group_id'];
            ...
            $this->aInfo = array (
                ...
                'URI' => $this->aDataEntry['uri'],
                 'TitleEdit' => $this->_oMain->isAllowedSubEdit($aDataEntry, $aBlogEntry) ? _t('_bx_groups_action_title_edit') : '',
                'TitleDelete' => $this->_oMain->isAllowedSubDelete($aDataEntry, $aBlogEntry) ? _t('_bx_groups_action_title_delete') : ''
            ...
            return $oFunctions->genObjectsActions($this->aInfo, 'bx_groups_blog');
        } 
