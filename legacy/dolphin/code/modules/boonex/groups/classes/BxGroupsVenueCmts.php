...
class BxGroupsVenueCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxGroupsVenueCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aVenueEntry = $oMain->_oDb->getVenueEntryById($this->getId ()); 
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableVenue, $aVenueEntry); 
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aVenueEntry = $oMain->_oDb->getVenueEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aVenueEntry['group_id']); 
