...
class BxGroupsPageMy extends BxDolPageView
{
    ...
    function getBlockCode_Joined() {
        ...
        bx_groups_import ('SearchResult');
        $o = new BxGroupsSearchResult('joined', process_db_input ($this->_aProfile['NickName'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION));
        $o->aCurrent['rss'] = 0;
 
        $o->sBrowseUrl = "browse/user/". process_db_input ($this->_aProfile['NickName'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION);
        $o->aCurrent['title'] = _t('_bx_groups_block_joined_groups');
        ...
        if ($s = $o->processing()) {
            ...
            return DesignBoxContent(_t('_bx_groups_block_joined_groups'), MsgBox(_t('_Empty')), 1);
        }
    }
