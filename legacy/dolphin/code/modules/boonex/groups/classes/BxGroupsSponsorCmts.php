...
class BxGroupsSponsorCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
	function BxGroupsSponsorCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
		...
		$aSponsorEntry = $oMain->_oDb->getSponsorEntryById($this->getId ());
        return $oMain->isAllowedCommentsSubProfile($oMain->_oDb->_sTableSponsor, $aSponsorEntry); 
    }

    function isEditAllowedAll () {
        $oMain = $this->getMain();
		$aSponsorEntry = $oMain->_oDb->getSponsorEntryById($this->getId ());
        $aDataEntry = $oMain->_oDb->getEntryById($aSponsorEntry['group_id']); 
