...
class BxEventsPageMy extends BxDolPageView {
	...
	function getBlockCode_Owner() {
        ...
        case 'pending_events':
            ...
            break; 
        case 'group_events':
            $sContent = $this->getBlockCode_LinkedGroupEvents ();
            break; 
        ...
        $aMenu = array(
			...
			_t('_bx_events_block_submenu_pending_invoices') => array('href' => $sBaseUrl . '&bx_events_filter=my_pending_invoices', 'active' => 'my_pending_invoices' == $_REQUEST['bx_events_filter']),
            _t('_bx_events_block_submenu_orders') => array('href' => $sBaseUrl . '&bx_events_filter=my_orders', 'active' => 'my_orders' == $_REQUEST['bx_events_filter']),
		...
		if(getParam('bx_groups_boonex_events')=='on'){
			$aMenu[_t('_bx_events_block_submenu_group_events')] = array('href' => $sBaseUrl . '&bx_events_filter=group_events', 'active' => 'group_events' == bx_get('bx_events_filter')); 
		}
    ...
    function getBlockCode_Joined() {
        ...
        bx_events_import ('SearchResult');
        $o = new BxEventsSearchResult('joined', process_db_input ($this->_aProfile['NickName'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION));
        $o->aCurrent['rss'] = 0;
 
        $o->sBrowseUrl = "browse/user/". process_db_input ($this->_aProfile['NickName'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION);
        $o->aCurrent['title'] = _t('_bx_events_block_joined_events');
        ...
        if ($s = $o->processing()) {
			...
			$s = ($s) ? $s : MsgBox(_t('_Empty'));
            return $s;
        } else {
            return DesignBoxContent(_t('_bx_events_block_joined_events'), MsgBox(_t('_Empty')), 1);
        }
    }
	...
	function getBlockCode_Pending() {
        $sForm = $this->_oMain->_manageEntries ('my_pending', '', false, 'bx_events_my_pending', array(
                'action_delete' => '_bx_events_admin_delete',
        ), 'bx_events_my_pending', true, 7); 
        $aVars = array ('form' => $sForm, 'id' => 'bx_events_my_pending');
        return $this->_oTemplate->parseHtmlByName('my_events_manage', $aVars);
    }
 
	function getBlockCode_LinkedGroupEvents() {
        $sForm = $this->_oMain->_manageEntries ('upcoming_group_events', process_db_input ($this->_aProfile['NickName'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION), false, 'bx_events_my_group_events', array(), 'bx_events_my_group_events', true, 7, false, 'unit'); 
        $aVars = array ('form' => $sForm, 'id' => 'bx_events_my_group_events');
        return $this->_oTemplate->parseHtmlByName('my_events_manage', $aVars);
    }
  
	function getBlockCode_GroupEvents() {
        $sForm = $this->_oMain->_manageEntries ('upcoming_group_events', process_db_input ($this->_aProfile['NickName'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION), false, 'bx_events_my_group_events', array(), 'bx_events_my_group_events', true, 7, false, 'unit'); 
         
        if (!$sForm)
            return DesignBoxContent(_t('_bx_events_block_submenu_group_events'), MsgBox(_t('_Empty')), 1);
        
		$aVars = array ('form' => $sForm, 'id' => 'bx_events_my_group_events');
        $s = $this->_oTemplate->parseHtmlByName('my_events_manage', $aVars); 
 
		return DesignBoxContent(_t('_bx_events_block_submenu_group_events'), $s, 1); 
    }
 
    function getBlockCode_Invoices() {
 
        $sForm = $this->_oMain->_manageOrders ('invoice', '', false, 'bx_events_pending_user_form', array(
            'action_delete' => '_bx_events_admin_delete',
        ), 'bx_events_my_pending', false, 7);
        if (!$sForm)
            return MsgBox(_t('_Empty'));
        $aVars = array ('form' => $sForm, 'id' => 'bx_events_my_pending_invoices');
        return $this->_oTemplate->parseHtmlByName('my_events_manage', $aVars); 
    }

    function getBlockCode_Orders() {
        $sForm = $this->_oMain->_manageOrders ('order', '', false, 'bx_events_pending_user_form', array(), 'bx_events_my_orders', false, 7, false);
        if (!$sForm)
            return MsgBox(_t('_Empty'));
        $aVars = array ('form' => $sForm, 'id' => 'bx_events_my_orders');
        return $this->_oTemplate->parseHtmlByName('my_events_manage', $aVars); 
    }
