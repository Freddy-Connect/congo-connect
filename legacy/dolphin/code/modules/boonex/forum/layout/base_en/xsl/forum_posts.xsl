...
-->
            <xsl:if test="0 = topic/locked">
                <btn href="javascript:void(0);" onclick="return f.postReply('{forum/id}','{topic/id}')" icon="{/root/urls/img}btn_icon_reply.gif">Reply</btn>				
            </xsl:if>
...
<!--
            <btn href="{$rw_rss_topic}{topic/uri}{$rw_rss_topic_ext}" onclick="" icon="{/root/urls/img}btn_icon_rss.gif">RSS</btn>

            <btn href="{$rw_topic}{topic/uri}{$rw_topic_ext}" onclick="" icon="{/root/urls/img}btn_icon_plink.gif">#</btn>
-->
        </xsl:if>
    </xsl:variable>
                    ...
                    <div class="bx-def-bc-padding">
                        ...
                        <xsl:if test="forum/id != 0 and topic/id != 0 and 0 = topic/locked">
                            <div class="forum_reply_button bx-def-margin-top clearfix">
                                <a class="bx-btn" href="javascript:void(0);" onclick="return f.postReply('{forum/id}','{topic/id}')" icon="{/root/urls/img}btn_icon_reply.gif"><i>Reply ›</i></a>
                            </div>
                        </xsl:if>
        ...
        <span class="forum_stat bx-def-font-grayed">
            ...
            <span id="report_{@id}" class="forum_post_actions_report_button">
                <xsl:if test="'' = vote_user_point and user/@name != /root/logininfo/username">
                    <a title="report this post" href="javascript:void(0);" onclick="if (f.report({@id}, true)) return f.voteBad({@id});">Report</a>
                    <span class="forum_bullet"></span>
                </xsl:if>
            </span>

            <xsl:if test="'' = vote_user_point and user/@name != /root/logininfo/username">
                <a href="javascript:void(0);" onclick="return f.voteBad({@id}, true);">Bury</a>
                <span class="forum_bullet"></span>
            </xsl:if>
            ...
            <span class="forum_post_actions_rate" id="rate_{@id}">
                <span class="forum_post_actions_rate_text">
                    ...
                    <span id="points_{@id}">
                        <xsl:call-template name="replace_hash">
                            <xsl:with-param name="s" select="string('# likes')"/>
