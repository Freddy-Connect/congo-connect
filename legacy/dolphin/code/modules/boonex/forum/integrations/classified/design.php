...
if (isset($_GET['action']) && 'goto' == $_GET['action'] && $_GET['forum_id']) {    
    ...
    $GLOBALS['oTopMenu']->setCustomVar('modzzz_classified_view_uri', $aForum['forum_uri']);
...
elseif (isset($_GET['action']) && 'goto' == $_GET['action'] && $_GET['topic_id']) {    
    ...
    $GLOBALS['oTopMenu']->setCustomVar('modzzz_classified_view_uri', $aTopic['forum_uri']);
    $GLOBALS['oTopMenu']->setCustomSubHeader($aTopic['forum_title']);
} else {
    $GLOBALS['oTopMenu']->setCustomVar('modzzz_classified_view_uri', '../');
}

if ((isset($_GET['action']) && 'goto' == $_GET['action'] && $_GET['forum_id']) || (isset($_GET['action']) && 'goto' == $_GET['action'] && $_GET['topic_id'])) { 
    $oModuleMain = BxDolModule::getInstance('BxClassifiedModule'); 
    if ($oModuleMain && $aForum) {
        ...
        $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
            _t('_modzzz_classified') => BX_DOL_URL_ROOT . $oModuleMain->_oConfig->getBaseUri() . 'home/',
            $aForum['forum_title'] => BX_DOL_URL_ROOT . $oModuleMain->_oConfig->getBaseUri() . 'view/' . $aForum['forum_uri'],
            _t('_modzzz_classified_menu_view_forum') => '',
        )); 
	}
}
...
// do not show forum index page - always select Classified category at least 

if (!isset($_GET['action']) && !isset($_POST['action'])) {
    $_GET['action'] = 'goto';
    $_GET['cat_id'] = 'Classified';
}
