jQuery(function ($) {
    ...
    var fixSortable = function () {
        if (!$.browser.msie)
            return;
        //this is fix for ie
        $('#column-manager').NestedSortableDestroy();
        $('#column-manager').NestedSortable({
            accept: 'sortable',
            helperclass: 'ns-helper',
            opacity: .8,
            handle: '.ns-title',
            ...
            onChange: function (serialized) {
				menu_serialized = serialized[0].hash;
                $('#btn-save-menu-col').attr('disabled', false);
            }
        });
    };
    $('#column-manager').NestedSortable({
		disableNesting  : 'sortable',
        accept: 'sortable',
        helperclass: 'ns-helper',
        opacity: .8,
        handle: '.ns-title',
    ...
    // $('#btn-save-menu-col').attr('disabled', true);
    $('#form-menu-col').submit(function () {
        // $('#btn-save-menu-col').attr('disabled', true);

        var jsonData = {};
            $( "input[class^='sel-data']" ).each(function() {
                //alert($(this).attr('data-id'));
                //alert($(this).is(':checked'));
                jsonData[$(this).attr('data-id')] = $(this).is(':checked');
                
            });
            // alert(jsonData);
            var jsonData = JSON.stringify(jsonData);
            if(menu_serialized == undefined)
            {
                menu_serialized = false;
            }
        
        $.ajax({
            type: 'POST',
            url: $(this).attr('action')+'/'+jsonData,
            dataType: 'json',
   ...
   /* Column manager end */
    ...
    $('.edit-menu').live('click', function () {
        ...
        gbox.show({
            type: 'ajax',
            url: _BASE_URL + 'modules/?r=giveme/editmenuitem&id=' + menu_id,
            buttons: {
                'Save': function () {
                    $.ajax({
                        type: 'POST',
    ...
    $('.delete-menu').live('click', function () {
        ...
        gbox.show({
            ...
            buttons: {
                'Yes': function () {
                    $.post(_BASE_URL + 'modules/?r=giveme/deletemenuitem', param, function (data) {
                        if (data.success) {
                            gbox.hide();
                            li.remove();
                        } else {
            ...
            buttons: {
                'Save': function () {
                    ...
                    if (group_title == '') {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: _BASE_URL + 'modules/?r=giveme/addmenugroup',
                            ...
                            success: function (data) {
                                ...
                                switch (data.status) {
                                    case 1:
                                        gbox.hide();
                                        $('#menu-group').append('<li><a href="' + _BASE_URL + 'modules/?r=giveme/administration/footer&group_id=' + data.id + '">' + group_title + '</a></li>');
                                        break;
                                    case 2:
    ...
    $('#edit-group').click(function () {
        ...
        inputgroup.focus().select().keydown(function (e) {
            if (e.which == 13) {
                ...
                $.ajax({
                    type: 'POST',
                    url: _BASE_URL + 'modules/?r=giveme/editmenugroup',
    ...
    $('#delete-group').click(function () {
        ...
        gbox.show({
            ...
            buttons: {
                'Yes': function () {
                    $.post(_BASE_URL + 'modules/?r=giveme/deletemenugroup', param, function (data) {
                        if (data.success) {
                            window.location = _BASE_URL + 'modules/?r=giveme/administration/footer';
                        } else {
                            gbox.show({
                                content: 'Failed to delete this menu.'
                            });
    ...
    /* add footer content
 ------------------------------------------------------------------------- */
$('#footer-content-form').submit(function () {         
    if ($('#footer-content').val() == '') {
        ...
        $.ajax({
            ...
            error: function () {
                gbox.show({
                    content: 'Footer content error. Please try again.',
                    autohide: 1000
                });
            },
            success: function (data) {
                gbox.show({
                    content: 'Footer content updated.',
                    autohide: 1000
                });
            }
        });
