...
function validateEduForm()
{
  ...
  // ---- Detect education type
  var val = ($('#type option:selected').text() || '').toLowerCase();
  var isNoDiploma = (
    val.indexOf('aucun diplôme') !== -1 ||
    val.indexOf('autodidacte') !== -1 ||
    val.indexOf('formation informelle') !== -1 ||
    val.indexOf('no formal diploma') !== -1 ||
    val.indexOf('self-taught') !== -1 ||
    val.indexOf('informal') !== -1
  );

  var isFormalEducation = !isNoDiploma;

  // Hide all errors first
  $('#school_name-error, #field_of_study-error, #period-required-error, #period-order-error').hide();

  // 1) REQUIRED only for formal education
  if (isFormalEducation) {
    ...
    if (($('#field_of_study').val() || '').trim() === '') {
      $('#field_of_study-error').show();
      errors.push('field_of_study');
    }
  }

  // 2) Period: required only for formal, BUT order check for everyone
  var startRaw = ($('#startDate').val() || '').trim();
  var endRaw   = ($('#endDate').val() || '').trim();

  var start = parseInt(startRaw, 10);
  var end   = parseInt(endRaw, 10);

  if (isFormalEducation) {
    if (!startRaw || !endRaw || isNaN(start) || isNaN(end)) {
      $('#period-required-error').show();
      errors.push('period');
    }
  }

  if (!isNaN(start) && !isNaN(end) && start > end) {
    $('#period-order-error').show();
    errors.push('period');
  }
...
// =========================
// Education: dynamic labels/placeholders (formal vs informal)
// =========================
function initEduDynamicUI()
{
  // Determine if current selection is "Aucun diplôme / autodidacte / informal"
  var txt = ($('#type option:selected').text() || '').toLowerCase();
  var isNoDiploma = (
    txt.indexOf('aucun diplôme') !== -1 ||
    txt.indexOf('autodidacte') !== -1 ||
    txt.indexOf('formation informelle') !== -1 ||
    txt.indexOf('no formal diploma') !== -1 ||
    txt.indexOf('self-taught') !== -1 ||
    txt.indexOf('informal') !== -1
  );

  var isInformal = isNoDiploma;
  var isFormal = !isInformal;

  // Toggle required stars
  $('.period-label abbr.required').toggle(isFormal);
  $('.field-label abbr.required').toggle(isFormal);
  $('label[for="school_name"] abbr.required').toggle(isFormal);

  // Update labels (read from hidden spans to avoid broken HTML)
  var periodText = isInformal ? $('#period-label-informal').text() : $('#period-label-formal').text();
  if (periodText) $('#period-label-text').text(periodText);

  var fieldLabelText = isInformal ? $('#field-label-informal').text() : $('#field-label-formal').text();
  if (fieldLabelText) $('#field-label-text').text(fieldLabelText);

  // Update placeholders
  var fieldPh = isInformal ? $('#edu-field-placeholder-informal').text() : $('#edu-field-placeholder-formal').text();
  if (fieldPh) $('#field_of_study').attr('placeholder', fieldPh);
  
    // ✅ ICI : Placeholder description
  var descPh = isInformal
    ? $('#edu-desc-placeholder-informal').text()
    : $('#edu-desc-placeholder-formal').text();

  if (descPh) {
    $('#description')
      .data('current-placeholder', descPh)
      .attr('placeholder', descPh);
  }
  ...
  var descPh = isInformal ? $('#edu-desc-placeholder-informal').text() : $('#edu-desc-placeholder-formal').text();
  if (descPh) {
    $('#description')
      .data('current-placeholder', descPh)
      .attr('placeholder', descPh);
  ...
  // Optionnel: rendre school name non-required visuellement/HTML
  $('#school_name').prop('required', isFormal);
  $('#field_of_study').prop('required', isFormal);
}
...
// Gestion focus/blur sans casser les placeholders dynamiques
$(document).on('focus', '#description', function () {
  this.placeholder = '';
});

$(document).on('blur', '#description', function () {
  var ph = $(this).data('current-placeholder') || '';
  this.placeholder = ph;
});
			...
			success:function(data)
{
	...
	$('.prof_edu_main').html(data);
// Re-init dynamic labels/placeholders after AJAX injection
initEduDynamicUI();


    // IMPORTANT: lancer le toggle après injection
    toggleEducationFields();
}
/**********Fin modif chatgpt 20 janvier 2026   */
		});
		return false;
	}
...
/* ajout 20 janvier 2026 */
// -------------------------------
// Hide / show optional education fields (FR + EN)
// Works even when the form is loaded by AJAX
// -------------------------------
function toggleEducationFields() {
  ...
  var txt = ($select.find('option:selected').text() || '').toLowerCase();

  var hideValues = [
    '_abs_plinkin_Aucun_diplome_Autodidacte',
    '_abs_plinkin_No_formal_diploma_Self_taught_Informal',
    '_abs_plinkin_Autodidacte'
  ];

  var hideByText =
    txt.includes('aucun diplôme') ||
    txt.includes('autodidacte') ||
    txt.includes('formation informelle') ||
    txt.includes('no formal diploma') ||
    txt.includes('self-taught') ||
    txt.includes('informal');

  var shouldHide = (hideValues.indexOf(val) !== -1) || hideByText;

  if (shouldHide) {
    $('.edu-optional').stop(true, true).slideUp(200);
  } else {
    $('.edu-optional').stop(true, true).slideDown(200);
  }

  // ✅ Important : met à jour labels/placeholders après le hide/show
  initEduDynamicUI();
}

/*** Fin function toggleEducationFields() ***/





// Delegated event (works after AJAX HTML injection)
$(document).on('change', '#type', function () {
    toggleEducationFields();
});
...
/* ===========================
   J243 Education Modal (JS)
   - remplace l'ouverture derrière
   - Add + Edit via getedueditform
   - stopImmediatePropagation() pour bloquer anciens handlers
   - scroll restore (anti-freeze)
   =========================== */
/*
(function($){
  ...
  function ensureModalEdu(){
    ...
    var html = ''
      ...
      + '  <div id="j243ExpModal" role="dialog" aria-modal="true">'
      + '    <div class="j243ExpHeader">'
      + '      <h3 id="j243ExpTitle">Ajouter une formation</h3>'
      + '      <button type="button" class="j243ExpClose" aria-label="Fermer">×</button>'
      + '    </div>'
      + '    <div class="j243ExpBody">'
      + '      <div class="j243ExpLoading">Chargement...</div>'
    ...
    // Cancel button closes
    $(document).on('click', '#j243ExpOverlay .edu-form-cancel', function(e){
      e.preventDefault();
      closeModalEdu();
      return false;
    });
  ...
  // ==== DELETE FIX: always delete the edu_id from the modal form (not from the page)
$(document).on('click', '#j243ExpOverlay .edu-form-delete', function(e){
  e.preventDefault();
  e.stopPropagation();
  e.stopImmediatePropagation(); // bloque l'ancien handler

  var $form = $(this).closest('form.Education_Form');
  var eduid = $.trim($form.find('input[name="edu_id"]').val() || '');

  if(!eduid || eduid === '0'){
    alert("Erreur: ID de la formation introuvable.");
    return false;
  }
  ...
  $('#j243ExpOverlay')
    ...
    // IMPORTANT: cacher le form derrière si le module l'a montré
    $('.prof_education_form').hide().empty();
  }
 ...
 function normalizeFormUI(){
  var $content = $('#j243ExpOverlay .j243ExpContent');
  var $form = $content.find('form.Education_Form').first();
  if(!$form.length) return;

  // --- Detect mode edit/add via hidden edu_id (dans prof_education_form.html)
  var expIdVal = $.trim($form.find('input[name="edu_id"]').val() || '');
  var isEdit = (expIdVal !== '' && expIdVal !== '0');

  // --- Show delete only on edit
  var $deleteBtn = $form.find('.edu-form-delete');
  if($deleteBtn.length){
    if(isEdit){
      $deleteBtn.css('display','inline-block');
    }else{
 ...
 function loadEduForm(eduid){
  var $title = $('#j243ExpModalTitle');

  var title = eduid ? 'Modifier la formation' : 'Ajouter une formation' ;
  // fallback: if translations are not loaded, use data-* attributes if present
  if(title === 'Modifier la formation' || title === 'Ajouter une formation '){
    title = eduid ? ($title.data('title-edit') || title) : ($title.data('title-add') || title);
  }
 openModalEdu(title);
$('#j243ExpOverlay')
  .addClass('active')
...
setLoading(true);
    ...
    $.ajax({
      url: 'modules/?r=linkedin_profile/getedueditform',
      type: 'post',
      data: { eduid: (eduid ? eduid : '') },
      success: function(data){
      setLoading(false);
      $('#j243ExpOverlay .j243ExpContent').html(data);
      // If we opened the form in ADD mode, force edu_id empty so UI doesn't think it's EDIT mode
      if(!eduid){
        $('#j243ExpOverlay').find('input[name="edu_id"]').val('');
      }
      normalizeFormUI();
    },
      error: function(){
  ...
  // Edit Education (pencil)
  $(document).on('click', '.scroll_edit, .scroll_con_edit', function(e){
    e.preventDefault();
    e.stopPropagation();
    if(e.stopImmediatePropagation) e.stopImmediatePropagation();

    var eduid = $(this).closest('.each_container').attr('data-eduid') ||
                $(this).parents('.each_container').attr('data-eduid');

    $('.prof_education_form').hide().empty();
    if(eduid) loadEduForm(eduid);
    return false;
  });

  // Edit education (click on box)
  $(document).on('click', '.edu_info', function(e){
    // seulement si c'est dans une education
    var eduid = $(this).closest('.each_container').attr('data-eduid');
    if(!eduid) return;
  ...
  $(document).ajaxSuccess(function(event, xhr, settings){
    ...
    // Le module poste généralement sur postedu
    if(settings.url.indexOf('modules/?r=linkedin_profile/postedu') !== -1){
      closeModalEdu();
    }
  });
  ...
  if(settings.url.indexOf('modules/?r=linkedin_profile/removeedu') !== -1){
    closeModalEdu();
  }
});
